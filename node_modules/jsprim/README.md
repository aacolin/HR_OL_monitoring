...
columns and the last element contains the remaining object .  For e***REMOVED***ample:
...
This is similar to `flattenObject` e***REMOVED***cept that instead of returning an array,
...
For e***REMOVED***ample, `pluck(obj, "foo.bar.baz")` is roughly equivalent to
`obj.foo.bar.baz`, e***REMOVED***cept that:

1. If traversal fails, the resulting value is undefined, and no error is
   thrown.  For e***REMOVED***ample, `pluck({}, "foo.bar")` is just undefined.
2. If "obj" has property "key" directly (without traversing), the
   corresponding property is returned.  For e***REMOVED***ample,
...
### startsWith(str, prefi***REMOVED***)

Returns true if the given string starts with the given prefi***REMOVED*** and false
otherwise.


### endsWith(str, suffi***REMOVED***)

Returns true if the given string ends with the given suffi***REMOVED*** and false
otherwise.
...
------------------ | ------- | ------- | ---------------------------
base               | number  | 10      | numeric base (radi***REMOVED***) to use, in the range 2 to 36
...
allowPrefi***REMOVED***        | boolean | false   | whether to interpret the prefi***REMOVED***es `0b` (base 2), `0o` (base 8), `0t` (base 10), or `0***REMOVED***` (base 16)
...
Note that if `base` is unspecified, and `allowPrefi***REMOVED***` or `leadingZeroIsOctal`
are, then the leading characters can change the default base from 10. If `base`
is e***REMOVED***plicitly specified and `allowPrefi***REMOVED***` is true, then the prefi***REMOVED*** will only be
accepted if it matches the specified base. `base` and `leadingZeroIsOctal`
cannot be used together.

**Conte***REMOVED***t:** It's tricky to parse integers with JavaScript's built-in facilities
for several reasons:

- `parseInt()` and `Number()` by default allow the base to be specified in the
  input string by a prefi***REMOVED*** (e.g., `0***REMOVED***` for he***REMOVED***).
...
- Some older JavaScript engines always interpret a leading 0 as indicating
  octal, which can be surprising when parsing input from users who e***REMOVED***pect a
  leading zero to be insignificant.

While each of these may be desirable in some conte***REMOVED***ts, there are also times when
...
Parses a date e***REMOVED***pressed as a string, as either a number of milliseconds since
...
### e***REMOVED***traProperties(object, allowed)

Check an object for une***REMOVED***pected properties.  Accepts the object to check, and an
array of allowed property name strings.  If e***REMOVED***tra properties are detected, an
array of e***REMOVED***tra property names is returned.  If no properties other than those
