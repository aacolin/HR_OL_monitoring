...
test('stringify()', function (t) {
    ...
    t.test('adds query prefi***REMOVED***', function (st) {
        st.equal(qs.stringify({ a: 'b' }, { addQueryPrefi***REMOVED***: true }), '?a=b');
        st.end();
    });

    t.test('with query prefi***REMOVED***, outputs blank string given an empty object', function (st) {
        st.equal(qs.stringify({}, { addQueryPrefi***REMOVED***: true }), '');
        st.end();
    });
    ...
    t.test('stringifies an array with mi***REMOVED***ed objects and primitives', function (st) {
        st.equal(
    ...
    t.test('does not crash when parsing circular references', function (st) {
        ...
        var arr = ['a'];
        st.doesNotThrow(
            function () { qs.stringify({ ***REMOVED***: arr, y: arr }); },
    ...
    t.test('supports custom representations when filter=function', function (st) {
        ...
        var filterFunc = function (prefi***REMOVED***, value) {
            calls += 1;
            if (calls === 1) {
                st.equal(prefi***REMOVED***, '', 'prefi***REMOVED*** is empty');
                st.equal(value, obj);
            } else if (prefi***REMOVED*** === 'c') {
                return void 0;
            } else if (value instanceof Date) {
                st.equal(prefi***REMOVED***, 'e[f]');
    ...
    t.test('respects an e***REMOVED***plicit charset of utf-8 (the default)', function (st) {
    ...
    t.test('strictNullHandling works with custom filter', function (st) {
        var filter = function (prefi***REMOVED***, value) {
            return value;
        };
