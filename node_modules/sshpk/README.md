...
node -- no `ssh-keygen` or other e***REMOVED***ternal dependencies.

Supports RSA, DSA, ECDSA (nistp-\*) and ED25519 key types, in PEM (PKCS#1, 
PKCS#8) and OpenSSH formats.

This library has been e***REMOVED***tracted from
...
[Ale***REMOVED*** Wilson](https://github.com/arekinath).
...
More e***REMOVED***amples: converting between formats:
...
- `options` -- Optional Object, e***REMOVED***tra options, with keys:
...
one, e***REMOVED***cept that the `verify()` function takes only the target signature as an
argument.
...
Creates a Diffie-Hellman key e***REMOVED***change object initialized with this key and all
necessary parameters. This has the same API as a `crypto.DiffieHellman`
instance, e***REMOVED***cept that functions take `Key` and `PrivateKey` objects as
...
E***REMOVED***tract just the public part of this private key, and return it as a `Key`
object.
...
one, e***REMOVED***cept that the `sign()` function takes no arguments, and returns a
`Signature` object.
...
to avoid degenerate keys that lead to a weak Diffie-Hellman e***REMOVED***change).
...
- `format` -- Optional String, format to use, valid options are `he***REMOVED***` and
              `base64`. If this `Fingerprint` uses the `md5` algorithm, the
              default format is `he***REMOVED***`. Otherwise, the default is `base64`.
...
to access basic metadata about certificates, e***REMOVED***tract public keys from them, and
also to generate simple self-signed certificates from an e***REMOVED***isting key.
...
Parameters
 ...
 - `format` -- a String, format to use, one of `'openssh'`, `'pem'` (X.509 in a
               PEM wrapper), or `'***REMOVED***509'` (raw DER encoded)
 ...
 * `'codeSigning'` -- key can be used to sign e***REMOVED***ecutable code
 * `'keyEncryption'` -- key can be used to encrypt other keys
 * `'encryption'` -- key can be used to encrypt data (only applies for RSA)
 * `'keyAgreement'` -- key can be used for key e***REMOVED***change protocols such as
                       Diffie-Hellman
...
### `Certificate#getE***REMOVED***tension(nameOrOid)`

Retrieves information about a certificate e***REMOVED***tension, if present, or returns
`undefined` if not. The string argument `nameOrOid` should be either the OID
(for X509 e***REMOVED***tensions) or the name (for OpenSSH e***REMOVED***tensions) of the e***REMOVED***tension
to retrieve.

The object returned will have the following properties:

 * `format` -- String, set to either `'***REMOVED***509'` or `'openssh'`
 * `name` or `oid` -- String, only one set based on value of `format`
 * `data` -- Buffer, the raw data inside the e***REMOVED***tension

### `Certificate#getE***REMOVED***tensions()`

Returns an Array of all present certificate e***REMOVED***tensions, in the same manner and
format as `getE***REMOVED***tension()`.

### `Certificate#isE***REMOVED***pired([when])`

Tests whether the Certificate is currently e***REMOVED***pired (i.e. the `validFrom` and
...
Parameters
 ...
 - `when` -- optional Date, if specified, tests whether the Certificate was or
             will be e***REMOVED***pired at the specified time instead of now
...
Parameters
 ...
 - `format` -- an optional String, output format, one of `'openssh'`, `'pem'` or
               `'***REMOVED***509'`. Defaults to `'***REMOVED***509'`.
...
| `street` | `2.5.4.9` |
| `***REMOVED***500UniqueIdentifier` | `2.5.4.45` |
| `role` | `2.5.4.72` |
...
of the DN contain an attribute of this name, an e***REMOVED***ception is thrown unless
