...
The pug loader is responsible for loading the depenendencies of a given pug file.  It adds `fullPath` and `str` properties to every `Include` and `E***REMOVED***tends` node.  It also adds an `ast` property to any `Include` nodes that are loading pug and any `E***REMOVED***tends` nodes.  It then recursively loads the dependencies of any of those included files.
...
- `le***REMOVED***` (function): **(required)** the le***REMOVED***er used
...
Callback used by `pug-load` to resolve the full path of an included or e***REMOVED***tended file given the path of the source file.
...
var ast = load(parse(le***REMOVED***(str, 'bar.pug'), 'bar.pug'), {
  le***REMOVED***: le***REMOVED***,
  parse: parse,
