  ...
  return walk(ast, function(node) {
    if (node.str === undefined) {
      if (
        ...
        node.type === 'E***REMOVED***tends'
      ) {
        var file = node.file;
        if (file.type !== 'FileReference') {
          throw new Error('E***REMOVED***pected file.type to be "FileReference"');
        }
        var path, str, raw;
        try {
        ...
        file.str = str;
        file.raw = raw;
        if (node.type === 'E***REMOVED***tends' || node.type === 'Include') {
...
load.string = function loadString(src, options) {
  ...
  var tokens = options.le***REMOVED***(src, options);
...
load.resolve = function resolve(filename, source, options) {
  ...
  if (filename[0] !== '/' && !source)
    throw new Error(
      'the "filename" option is required to use includes and e***REMOVED***tends with "relative" paths'
    );

  if (filename[0] === '/' && !options.basedir)
    throw new Error(
      'the "basedir" option is required to use includes and e***REMOVED***tends with "absolute" paths'
    );
...
load.validateOptions = function validateOptions(options) {
  ...
  /* istanbul ignore if */
  if (typeof options.le***REMOVED*** !== 'function') {
    throw new TypeError('options.le***REMOVED*** must be a function');
  }
  /* istanbul ignore if */
