 ...
 * - [`e***REMOVED***plain`](https://mongoosejs.com/docs/api/aggregate.html#Aggregate.prototype.e***REMOVED***plain())
 * - `fieldsAsRaw`
...
/**
 * Get/set the model that this aggregation will e***REMOVED***ecute on.
 *
 * #### E***REMOVED***ample:
 *
 ...
 * #### E***REMOVED***ample:
 *
 *     // adding new fields based on e***REMOVED***isting fields
 *     aggregate.addFields({
 *         newField: '$b.nested'
 *       , plusTen: { $add: ['$val', 10]}
 *       , sub: {
 ...
 *
 * Mongoose query [selection synta***REMOVED***](https://mongoosejs.com/docs/api/query.html#Query.prototype.select()) is also supported.
 *
 * #### E***REMOVED***ample:
 *
 *     // include a, include b, e***REMOVED***clude _id
 ...
 * @memberOf Aggregate
 * @instance
 * @param {Number} num number of records to skip before ne***REMOVED***t stage
 * @return {Aggregate}
 * @api public
 */
 ...
 * @memberOf Aggregate
 * @instance
 * @param {Number} num ma***REMOVED***imum number of records to pass to the ne***REMOVED***t stage
 * @return {Aggregate}
 * @api public
 */
 ...
 * If you are passing in an object the strings in your e***REMOVED***pression will not be altered.
 *
 * #### E***REMOVED***ample:
 *
 *     aggregate.replaceRoot("user");
 *
 *     aggregate.replaceRoot({ ***REMOVED***: { $concat: ['$this', '$that'] } });
 *
 ...
 *      aggregate.graphLookup({ from: 'courses', startWith: '$prerequisite', connectFromField: 'prerequisite', connectToField: 'name', as: 'prerequisites', ma***REMOVED***Depth: 3 }) // this will recursively search the 'courses' collection up to 3 prerequisites
 *
...
Aggregate.prototype.sort = function(arg) {
  ...
  if (getConstructorName(arg) === 'Object') {
    ...
    Object.keys(arg).forEach(function(field) {
      // If sorting by te***REMOVED***t score, skip coercing into 1/-1
      if (arg[field] instanceof Object && arg[field].$meta) {
        sort[field] = arg[field];
        return;
      }
      sort[field] = desc.inde***REMOVED***Of(arg[field]) === -1 ? 1 : -1;
    });
  } else if (arguments.length === 1 && typeof arg === 'string') {
    arg.split(/\s+/).forEach(function(field) {
      if (!field) {
 ...
 * If `thenE***REMOVED***pr` or `elseE***REMOVED***pr` is string, make sure it starts with $$, like `$$DESCEND`, `$$PRUNE` or `$$KEEP`.
 *
 * #### E***REMOVED***ample:
 *
 ...
 *     });
 *
 *     // $redact often comes with $cond operator, you can also use the following synta***REMOVED*** provided by mongoose
 *     await Model.aggregate(pipeline).redact({ $eq: [ '$level', 5 ] }, '$$PRUNE', '$$DESCEND');
 *
 * @param {Object} e***REMOVED***pression redact options or conditional e***REMOVED***pression
 * @param {String|Object} [thenE***REMOVED***pr] true case for the condition
 * @param {String|Object} [elseE***REMOVED***pr] false case for the condition
 * @return {Aggregate} this
 * @see $redact https://www.mongodb.com/docs/manual/reference/operator/aggregation/redact/
 * @api public
 */

Aggregate.prototype.redact = function(e***REMOVED***pression, thenE***REMOVED***pr, elseE***REMOVED***pr) {
  if (arguments.length === 3) {
    if ((typeof thenE***REMOVED***pr === 'string' && !validRedactStringValues.has(thenE***REMOVED***pr)) ||
      (typeof elseE***REMOVED***pr === 'string' && !validRedactStringValues.has(elseE***REMOVED***pr))) {
      throw new Error('If thenE***REMOVED***pr or elseE***REMOVED***pr is string, it must be either $$DESCEND, $$PRUNE or $$KEEP');
    }

    e***REMOVED***pression = {
      $cond: {
        if: e***REMOVED***pression,
        then: thenE***REMOVED***pr,
        else: elseE***REMOVED***pr
      }
    };
  ...
  return this.append({ $redact: e***REMOVED***pression });
};

/**
 * E***REMOVED***ecute the aggregation with e***REMOVED***plain
 *
 * #### E***REMOVED***ample:
 *
 *     Model.aggregate(..).e***REMOVED***plain()
 *
 * @param {String} [verbosity]
 * @return {Promise}
 */

Aggregate.prototype.e***REMOVED***plain = async function e***REMOVED***plain(verbosity) {
  if (typeof verbosity === 'function' || typeof arguments[1] === 'function') {
    throw new MongooseError('Aggregate.prototype.e***REMOVED***plain() no longer accepts a callback');
  }
  ...
  await new Promise((resolve, reject) => {
    model.hooks.e***REMOVED***ecPre('aggregate', this, error => {
      if (error) {
        const _opts = { error: error };
        return model.hooks.e***REMOVED***ecPost('aggregate', this, [null], _opts, error => {
          reject(error);
        });
      } else {
        resolve();
  ...
  let result = null;
  try {
    result = await cursor.e***REMOVED***plain(verbosity);
  } catch (error) {
    await new Promise((resolve, reject) => {
      const _opts = { error: error };
      model.hooks.e***REMOVED***ecPost('aggregate', this, [null], _opts, error => {
        if (error) {
          return reject(error);
        }
        return resolve();
  ...
  await new Promise((resolve, reject) => {
    model.hooks.e***REMOVED***ecPost('aggregate', this, [result], _opts, error => {
      if (error) {
        return reject(error);
      }
      return resolve();
 ...
 * #### E***REMOVED***ample:
 *
 *     Model.aggregate(..).hint({ qty: 1, category: 1 }).e***REMOVED***ec();
 *
 * @param {Object|String} value a hint object or the inde***REMOVED*** name
 * @return {Aggregate} this
 * @see mongodb https://www.mongodb.com/docs/manual/reference/command/aggregate/
 */
 ...
 * @param {Number} [options.ma***REMOVED***TimeMS] number limits the time this aggregation will run, see [MongoDB docs on `ma***REMOVED***TimeMS`](https://www.mongodb.com/docs/manual/reference/operator/meta/ma***REMOVED***TimeMS/)
...
/**
 * Sets the `cursor` option and e***REMOVED***ecutes this aggregation, returning an aggregation cursor.
 ...
 * @param {Boolean} [options.useMongooseAggCursor] use e***REMOVED***perimental mongoose-specific aggregation cursor (for `eachAsync()` and other query cursor semantics)
...
/**
 * Helper for [Atlas Te***REMOVED***t Search](https://www.mongodb.com/docs/atlas/atlas-search/tutorial/)'s
 * `$search` stage.
 *
 * #### E***REMOVED***ample:
 *
 ...
 *
 * @return {Array} The current pipeline similar to the operation that will be e***REMOVED***ecuted
 * @api public
 */
...
/**
 * E***REMOVED***ecutes the aggregate pipeline on the currently bound Model.
 *
 * #### E***REMOVED***ample:
 *     const result = await aggregate.e***REMOVED***ec();
 *
 * @return {Promise}
 * @api public
 */

Aggregate.prototype.e***REMOVED***ec = async function e***REMOVED***ec() {
  if (!this._model) {
    throw new Error('Aggregate not bound to any Model');
  }
  if (typeof arguments[0] === 'function') {
    throw new MongooseError('Aggregate.prototype.e***REMOVED***ec() no longer accepts a callback');
  }
  const model = this._model;
  const collection = this._model.collection;

  applyGlobalMa***REMOVED***TimeMS(this.options, model.db.options, model.base.options);
...
/**
 * Provides a Promise-like `then` function, which will call `.e***REMOVED***ec` without a callback
...
/**
 * E***REMOVED***ecutes the aggregation returning a `Promise` which will be
...
/**
 * E***REMOVED***ecutes the aggregate returning a `Promise` which will be
...
module.e***REMOVED***ports = Aggregate;
