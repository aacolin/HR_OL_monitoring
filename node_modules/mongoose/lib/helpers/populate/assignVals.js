...
module.e***REMOVED***ports = function assignVals(o) {
  // Options that aren't e***REMOVED***plicitly listed in `populateOptions`
  const userOptions = Object.assign({}, get(o, 'allOptions.options.options'), get(o, 'allOptions.options'));
  // `o.options` contains options e***REMOVED***plicitly listed in `populateOptions`, like
  // `match` and `limit`.
  ...
  let i;
  let setValueInde***REMOVED*** = 0;

  function setValue(val) {
    ++setValueInde***REMOVED***;
    if (count) {
      return val;
    }
    ...
    if (o.justOne === true && Array.isArray(val)) {
      ...
      return valueFilter(val, options, populateOptions, val == null ? val : _allIds[setValueInde***REMOVED*** - 1]);
    }
    return valueFilter(val, options, populateOptions, _allIds);
  }

  for (i = 0; i < docs.length; ++i) {
    setValueInde***REMOVED*** = 0;
    const _path = o.path.endsWith('.$*') ? o.path.slice(0, -3) : o.path;
    const e***REMOVED***istingVal = mpath.get(_path, docs[i], lookupLocalFields);
    if (e***REMOVED***istingVal == null && !getVirtual(o.originalModel.schema, _path)) {
      continue;
    }
    ...
    // If we're populating a map, the e***REMOVED***isting value will be an object, so
    // we need to transform again
    const originalSchema = o.originalModel.schema;
    const isDoc = get(docs[i], '$__', null) != null;
    let isMap = isDoc ?
      e***REMOVED***istingVal instanceof Map :
      utils.isPOJO(e***REMOVED***istingVal);
    // If we pass the first check, also make sure the local field's schematype
    // is map (re: gh-6460)
    isMap = isMap && get(originalSchema._getSchema(_path), '$isSchemaMap');
    if (!o.isVirtual && isMap) {
      const _keys = e***REMOVED***istingVal instanceof Map ?
        Array.from(e***REMOVED***istingVal.keys()) :
        Object.keys(e***REMOVED***istingVal);
    ...
    for (let j = 0; j < parts.length - 1; ++j) {
      ...
      // `foo` on the array.
      if (Array.isArray(cur) && !utils.isArrayInde***REMOVED***(parts[j])) {
        break;
      }
 ...
 *
 * background:
 * _ids are left in the query even when user e***REMOVED***cludes them so
...
function maybeRemoveId(subdoc, assignmentOpts) {
  if (subdoc != null && assignmentOpts.e***REMOVED***cludeId) {
