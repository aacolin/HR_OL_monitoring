...
module.e***REMOVED***ports = function getModelsMapForPopulate(model, docs, options) {
  let doc;
  ...
  for (let i = 0; i < len; i++) {
    ...
    // there's an e***REMOVED***plicit `ref` or `refPath` re: gh-8946
    if (schema != null &&
    ...
    if (Array.isArray(schema)) {
      ...
      for (const _schema of schemasArray) {
        ...
        for (const modelName of _modelNames) {
          if (modelNames.inde***REMOVED***Of(modelName) === -1) {
    ...
    } else if (schema && !schema[schemaMi***REMOVED***edSymbol]) {
      // Skip Mi***REMOVED***ed types because we e***REMOVED***plicitly don't do casting on those.
  ...
  function _getModelNames(doc, schema, modelNameFromQuery, model) {
    ...
    if (schema != null &&
        schema[schemaMi***REMOVED***edSymbol] &&
        !ref &&
        !refPath &&
        !modelNameFromQuery) {
    ...
    if (modelNameFromQuery) {
      ...
      if ((ref = get(schemaForCurrentDoc, 'options.ref')) != null) {
        if (schemaForCurrentDoc != null &&
          ...
          // Ensure correct conte***REMOVED***t for ref functions: subdoc, not top-level doc. See gh-8469
          modelNames = new Set();
...
function _virtualPopulate(model, docs, options, _virtualRes) {
  ...
  for (const doc of docs) {
    ...
    let localField;
    const virtualPrefi***REMOVED*** = _virtualRes.nestedSchemaPath ?
      _virtualRes.nestedSchemaPath + '.' : '';
    if (typeof options.localField === 'string') {
      localField = options.localField;
    } else if (typeof virtual.options.localField === 'function') {
      localField = virtualPrefi***REMOVED*** + virtual.options.localField.call(doc, doc);
    } else if (Array.isArray(virtual.options.localField)) {
      localField = virtual.options.localField.map(field => virtualPrefi***REMOVED*** + field);
    } else {
      localField = virtualPrefi***REMOVED*** + virtual.options.localField;
    }
...
function matchIdsToRefPaths(ids, refPaths, refPathToFind) {
  ...
  if (Array.isArray(ids) && Array.isArray(refPaths)) {
    return ids.flatMap((id, inde***REMOVED***) => matchIdsToRefPaths(id, refPaths[inde***REMOVED***], refPathToFind));
  }
  return [];
}
...
function _getLocalFieldValues(doc, localField, model, options, virtual, schema) {
  ...
  if (localFieldGetters.length !== 0 && getters) {
    ...
    if (Array.isArray(localFieldValue)) {
      ...
      return localFieldValue.map((localFieldArrVal, localFieldArrInde***REMOVED***) =>
        localFieldPath.applyGetters(localFieldArrVal, localFieldHydratedValue[localFieldArrInde***REMOVED***]));
    } else {
...
function convertTo_id(val, schema) {
  ...
  if (getConstructorName(val) === 'Object' &&
      // The intent here is we should only flatten the object if we e***REMOVED***pect
      // to get a Map in the end. Avoid doing this for mi***REMOVED***ed types.
      (schema == null || schema[schemaMi***REMOVED***edSymbol] == null)) {
    const ret = [];
