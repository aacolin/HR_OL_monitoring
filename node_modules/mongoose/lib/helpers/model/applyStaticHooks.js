...
module.e***REMOVED***ports = function applyStaticHooks(model, hooks, statics) {
  const kareemOptions = {
  ...
  hooks = hooks.filter(hook => {
    // If the custom static overwrites an e***REMOVED***isting middleware, don't apply
    // middleware to it by default. This avoids a potential backwards breaking
    // change with plugins like `mongoose-delete` that use statics to overwrite
    // built-in Mongoose functions.
    if (middlewareFunctions.inde***REMOVED***Of(hook.name) !== -1) {
      return !!hook.model;
    }
    ...
    if (hooks.hasHooks(key)) {
      ...
      model[key] = function() {
        ...
        return promiseOrCallback(cb, callback => {
          hooks.e***REMOVED***ecPre(key, model, args, function(err) {
            if (err != null) {
              return callback(err);
            }
            ...
            function post(error, res) {
              ...
              hooks.e***REMOVED***ecPost(key, model, [res], function(error) {
                if (error != null) {
                  return callback(error);
                }
                callback(null, res);
