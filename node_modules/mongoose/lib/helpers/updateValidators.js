...
module.e***REMOVED***ports = function(query, schema, castedDoc, options, callback) {
  ...
  const validatorsToE***REMOVED***ecute = [];
  const validationErrors = [];

  const alreadyValidated = [];

  const conte***REMOVED***t = query;
  function iter(i, v) {
    const schemaPath = schema._getSchema(updates[i]);
    if (schemaPath == null) {
      return;
    }
    if (schemaPath.instance === 'Mi***REMOVED***ed' && schemaPath.path !== updates[i]) {
      return;
    }

    if (v && Array.isArray(v.$in)) {
      v.$in.forEach((v, i) => {
        validatorsToE***REMOVED***ecute.push(function(callback) {
      ...
      if (schemaPath.$isMongooseDocumentArrayElement && v != null && v.$__ != null) {
        ...
        validatorsToE***REMOVED***ecute.push(function(callback) {
          ...
          }, conte***REMOVED***t, { updateValidator: true });
        });
      } else {
        validatorsToE***REMOVED***ecute.push(function(callback) {
    ...
    if (schemaPath && schemaPath.$isMongooseDocumentArray) {
      validatorsToE***REMOVED***ecute.push(function(callback) {
        schemaPath.doValidate(
          ...
          options && options.conte***REMOVED***t === 'query' ? query : null);
      });
    } else {
  ...
  if (callback != null) {
    let numValidators = validatorsToE***REMOVED***ecute.length;
    if (numValidators === 0) {
      return _done(callback);
    }
    for (const validator of validatorsToE***REMOVED***ecute) {
      validator(function() {
        if (--numValidators <= 0) {
          _done(callback);
        }
