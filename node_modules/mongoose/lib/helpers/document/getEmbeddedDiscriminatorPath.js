...
/**
 * Like `schema.path()`, e***REMOVED***cept with a document, because impossible to
...
module.e***REMOVED***ports = function getEmbeddedDiscriminatorPath(doc, path, options) {
  options = options || {};
  const typeOnly = options.typeOnly;
  const parts = Array.isArray(path) ?
    path :
    (path.inde***REMOVED***Of('.') === -1 ? [path] : path.split('.'));
  let schemaType = null;
  ...
  for (let i = 0; i < parts.length; ++i) {
    ...
    if (schemaType.instance === 'Mi***REMOVED***ed') {
