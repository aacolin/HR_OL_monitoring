...
const Mi***REMOVED***edSchema = require('./schema/mi***REMOVED***ed');
const ModifiedPathsSnapshot = require('./modifiedPathsSnapshot');
const ObjectE***REMOVED***pectedError = require('./error/objectE***REMOVED***pected');
...
const isPathE***REMOVED***cluded = require('./helpers/projection/isPathE***REMOVED***cluded');
...
const schemaMi***REMOVED***edSymbol = require('./schema/symbols').schemaMi***REMOVED***edSymbol;
...
function Document(obj, fields, skipId, options) {
  ...
  // Support `browserDocument.js` synta***REMOVED***
  ...
  // e***REMOVED***cluded fields
  if (utils.isPOJO(fields) && Object.keys(fields).length > 0) {
    e***REMOVED***clude = isE***REMOVED***clusive(fields);
    this.$__.selected = fields;
    this.$__.e***REMOVED***clude = e***REMOVED***clude;
  }

  const hasIncludedChildren = e***REMOVED***clude === false && fields ?
    $__hasIncludedChildren(fields) :
    null;

  if (this._doc == null) {
    this.$__buildDoc(obj, fields, skipId, e***REMOVED***clude, hasIncludedChildren, false);

    // By default, defaults get applied **before** setting initial values
    // Re: gh-6155
    if (defaults) {
      applyDefaults(this, fields, e***REMOVED***clude, hasIncludedChildren, true, null, {
  ...
  if (options.willInit && defaults) {
    ...
    applyDefaults(this, fields, e***REMOVED***clude, hasIncludedChildren, false, options.skipDefaults);
  }
  ...
  if (!this.$__.strictMode && obj) {
    ...
    keys.forEach(function(key) {
      // Avoid methods, virtuals, e***REMOVED***isting fields, and `$` keys. The latter is to avoid overwriting
      // Mongoose internals.
 ...
 * a new document or `updateOne()` to update an e***REMOVED***isting document.
 *
 * #### E***REMOVED***ample:
 *
 ...
 *
 * On the other hand, if you load an e***REMOVED***isting document from the database
...
/*!
 * Document e***REMOVED***poses the NodeJS event emitter API, so you can use
 * `on`, `once`, etc.
 */
utils.each(
  ['on', 'once', 'emit', 'listeners', 'removeListener', 'setMa***REMOVED***Listeners',
  ...
  function(emitterFn) {
    Document.prototype[emitterFn] = function() {
      ...
      if (!this.$__.emitter) {
        ...
        this.$__.emitter.setMa***REMOVED***Listeners(0);
      }
 ...
 * #### Note:
 *
 * This getter e***REMOVED***ists on all documents by default. The getter can be disabled by setting the `id` [option](https://mongoosejs.com/docs/guide.html#id) of its `Schema` to false at construction time.
 *
...
/**
 * A string containing the current operation that Mongoose is e***REMOVED***ecuting
...
function $applyDefaultsToNested(val, path, doc) {
  ...
  const pathPieces = path.inde***REMOVED***Of('.') === -1 ? [path] : path.split('.');
...
Document.prototype.$__buildDoc = function(obj, fields, skipId, e***REMOVED***clude, hasIncludedChildren) {
  const doc = {};
  ...
  for (; ii < plen; ++ii) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      // support e***REMOVED***cluding intermediary levels
      if (e***REMOVED***clude === true) {
        if (curPath in fields) {
          break;
        }
      } else if (e***REMOVED***clude === false && fields && !included) {
        if (curPath in fields) {
          included = true;
...
Document.prototype.$__init = function(doc, opts) {
  ...
  const hasIncludedChildren = this.$__.e***REMOVED***clude === false && this.$__.selected ?
    $__hasIncludedChildren(this.$__.selected) :
    null;

  applyDefaults(this, this.$__.selected, this.$__.e***REMOVED***clude, hasIncludedChildren, false, this.$__.skipDefaults);
  return this;
};
 ...
 * @param {String} [prefi***REMOVED***] Prefi***REMOVED*** to add to each path
 * @api private
 */

function init(self, obj, doc, opts, prefi***REMOVED***) {
  ...
  function _init(inde***REMOVED***) {
    i = keys[inde***REMOVED***];
    // avoid prototype pollution
    if (i === '__proto__' || i === 'constructor') {
      return;
    }
    path = prefi***REMOVED*** ? prefi***REMOVED*** + i : i;
...
Document.prototype.updateOne = function updateOne(doc, options, callback) {
  ...
  query.pre(function queryPreUpdateOne(cb) {
    self.constructor._middleware.e***REMOVED***ecPre('updateOne', self, [self], cb);
  });
  query.post(function queryPostUpdateOne(cb) {
    self.constructor._middleware.e***REMOVED***ecPost('updateOne', self, [self], {}, cb);
  });
  ...
  if (callback != null) {
    return query.e***REMOVED***ec(callback);
  }
...
/**
 * Overwrite all values in this document with the values of `obj`, e***REMOVED***cept
 * for immutable properties. Behaves similarly to `set()`, e***REMOVED***cept for it
...
Document.prototype.overwrite = function overwrite(obj) {
  ...
  for (const key of keys) {
    ...
    // E***REMOVED***plicitly skip version key
 ...
 * @param {Boolean} [options.merge=false] if true, setting a [nested path](https://mongoosejs.com/docs/subdocs.html#subdocuments-versus-nested-paths) will merge e***REMOVED***isting values rather than overwrite the whole object. So `doc.set('nested', { a: 1, b: 2 })` becomes `doc.set('nested.a', 1); doc.set('nested.b', 2);`
 * @return {Document} this
 * @method $set
 * @memberOf Document
 * @instance
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  if (path == null) {
    ...
    for (let i = 0; i < len; ++i) {
      key = keys[i];
      const pathName = prefi***REMOVED*** ? prefi***REMOVED*** + key : key;
      ...
      if (utils.isNonBuiltinObject(valForKey) && pathtype === 'nested') {
        ...
        if (pathtype === 'real' || pathtype === 'virtual') {
          ...
          if (pathtype === 'nested') {
            throw new ObjectE***REMOVED***pectedError(key, valForKey);
          } else {
  ...
  let parts = null;
  if (pathType === 'adhocOrUndefined') {
    parts = path.inde***REMOVED***Of('.') === -1 ? [path] : path.split('.');
  ...
  if (pathType === 'adhocOrUndefined' && strict) {
    // check for roots that are Mi***REMOVED***ed types
    let mi***REMOVED***ed;
    ...
    for (i = 0; i < parts.length; ++i) {
      ...
      if (schema instanceof Mi***REMOVED***edSchema) {
        // allow changes to sub paths of mi***REMOVED***ed types
        mi***REMOVED***ed = true;
        break;
      } else if (schema.$isSchemaMap && schema.$__schemaType instanceof Mi***REMOVED***edSchema && i < parts.length - 1) {
        // Map of mi***REMOVED***ed and not the last element in the path resolves to mi***REMOVED***ed
        mi***REMOVED***ed = true;
  ...
  // gh-4578, if setting a deeply nested path that doesn't e***REMOVED***ist yet, create it
  let cur = this._doc;
  let curPath = '';
  for (i = 0; i < parts.length - 1; ++i) {
    ...
    if (!cur) {
      this.$set(curPath, {});
      // Hack re: gh-5800. If nested field is not selected, it probably e***REMOVED***ists
      // so `MongoServerError: cannot use the part (nested of nested.num) to
      // traverse the element ({nested: null})` is not likely. If user gets
      // that error, its their fault for now. We should reconsider disallowing
      // modifying not selected paths for 6.***REMOVED***
  ...
  // When using the $set operator the path to the field must already e***REMOVED***ist.
  ...
  try {
    ...
    if (!refMatches || !schema.$isSingleNested || !val.$__) {
      ...
      // a single nested doc. That's to make sure we get the correct conte***REMOVED***t.
      // Otherwise we would double-call the setter, see gh-7196.
      let setterConte***REMOVED***t = this;
      if (this.$__schema.singleNestedPaths[path] != null && parts.length > 1) {
        setterConte***REMOVED***t = getDeepestSubdocumentForPath(this, parts, this.schema);
      }
      if (options != null && options.overwriteImmutable) {
        val = schema.applySetters(val, setterConte***REMOVED***t, false, priorVal, { overwriteImmutable: true });
      } else {
        val = schema.applySetters(val, setterConte***REMOVED***t, false, priorVal);
      }
    }
...
Document.prototype.$__set = function(pathToMark, path, options, constructing, parts, schema, val, priorVal) {
  ...
  for (; i < l; i++) {
    const ne***REMOVED***t = i + 1;
    const last = ne***REMOVED***t === l;
...
Document.prototype.get = function(path, type, options) {
  ...
  if (schema != null && schema.instance === 'Mi***REMOVED***ed') {
  ...
  const hasDot = path.inde***REMOVED***Of('.') !== -1;
  let obj = this._doc;
 ...
 *
 * _Very helpful when using [Mi***REMOVED***ed](https://mongoosejs.com/docs/schematypes.html#mi***REMOVED***ed) types._
 *
 * #### E***REMOVED***ample:
 *
 *     doc.mi***REMOVED***ed.type = 'changed';
 *     doc.markModified('mi***REMOVED***ed.type');
 *     doc.save() // changes to mi***REMOVED***ed.type are now persisted
 *
...
Document.prototype.$__saveInitialState = function $__saveInitialState(path) {
  ...
  if (savedState != null) {
    const firstDot = savedStatePath.inde***REMOVED***Of('.');
 ...
 * modified path is a path that you e***REMOVED***plicitly set, whether via `doc.foo = 'bar'`,
 ...
 *
 * @param {Object} [options]
 * @param {Boolean} [options.includeChildren=false] if true, returns children of modified paths as well. For e***REMOVED***ample, if false, the list of modified paths for `doc.colors = { primary: 'blue' };` will **not** contain `colors.primary`. If true, `modifiedPaths()` will return an array that contains `colors.primary`.
 * @return {String[]}
 * @api public
 */
...
Document.prototype.isModified = function(paths, options, modifiedPaths) {
  if (paths) {
    ...
    if (typeof paths === 'string') {
      paths = paths.inde***REMOVED***Of(' ') === -1 ? [paths] : paths.split(' ');
    }
    ...
    const isModifiedChild = paths.some(function(path) {
      return !!~modified.inde***REMOVED***Of(path);
    });
...
Document.prototype.$isDefault = function(path) {
  ...
  if (typeof path === 'string' && path.inde***REMOVED***Of(' ') === -1) {
 ...
 *     product.$isDeleted(); // false
 *     product.remove(); // will e***REMOVED***ecute a remove against the db
 *
 *
...
/**
 * Checks if `path` was e***REMOVED***plicitly selected. If no projection, always returns
 * true.
 *
 * #### E***REMOVED***ample:
 *
 *     Thing.findOne().select('nested.name').e***REMOVED***ec(function (err, doc) {
...
/**
 * E***REMOVED***ecutes registered validation rules for this document.
 *
 * #### Note:
 *
...
Document.prototype.validate = async function validate(pathsToValidate, options) {
  ...
  if (options && typeof options.pathsToSkip === 'string') {
    const isOnePathOnly = options.pathsToSkip.inde***REMOVED***Of(' ') === -1;
...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip, isNestedValidate) {
  ...
  if (!isNestedValidate) {
    ...
    for (const subdoc of subdocs) {
      if (subdoc.$basePath) {
        const fullPathToSubdoc = subdoc.$isSingleNested ? subdoc.$__pathRelativeToParent() : subdoc.$__fullPathWithInde***REMOVED***es();
        ...
        if (doc.$isModified(fullPathToSubdoc, null, modifiedPaths) &&
          ...
          if (subdoc.$isDocumentArrayElement && subdoc.__inde***REMOVED*** != null) {
            doValidateOptions[fullPathToSubdoc].inde***REMOVED*** = subdoc.__inde***REMOVED***;
          }
        }
      }
    }
...
Document.prototype.$__validate = function(pathsToValidate, options, callback) {
  ...
  if (paths.length === 0) {
    return immediate(function() {
      ...
      if (error) {
        return _this.$__schema.s.hooks.e***REMOVED***ecPost('validate:error', _this, [_this], { error: error }, function(error) {
          callback(error);
        });
      }
      callback(null, _this);
  ...
  function validatePath(path) {
    ...
    immediate(function() {
      ...
      // If setting a path under a mi***REMOVED***ed path, avoid using the mi***REMOVED***ed path validator (gh-10141)
      if (schemaType[schemaMi***REMOVED***edSymbol] != null && path !== schemaType.path) {
        return --total || complete();
      }
...
/**
 * E***REMOVED***ecutes registered validation rules (skipping asynchronous validators) for this document.
 *
 * #### Note:
 *
...
Document.prototype.validateSync = function(pathsToValidate, options) {
  ...
  if (typeof pathsToValidate === 'string') {
    const isOnePathOnly = pathsToValidate.inde***REMOVED***Of(' ') === -1;
    pathsToValidate = isOnePathOnly ? [pathsToValidate] : pathsToValidate.split(' ');
  } else if (typeof pathsToSkip === 'string' && pathsToSkip.inde***REMOVED***Of(' ') !== -1) {
 ...
 *     })
 *
 * @param {String} path the field to invalidate. For array elements, use the `array.i.field` synta***REMOVED***, where `i` is the 0-based inde***REMOVED*** in the array.
...
/**
 * Marks a path as valid, removing e***REMOVED***isting validation errors.
 *
...
function _checkImmutableSubpaths(subdoc, schematype, priorVal) {
  ...
  for (const key of Object.keys(schema.paths)) {
    ...
    // Calling immutableSetter with `oldVal` even though it e***REMOVED***pects `newVal`
...
Document.prototype.$toObject = function(options, json) {
  ...
  // If options do not e***REMOVED***ist or is not an object, set it to empty object
  ...
  if (typeof transformFunction === 'function') {
    const ***REMOVED***formed = transformFunction(this, ret, options);
    if (typeof ***REMOVED***formed !== 'undefined') {
      ret = ***REMOVED***formed;
    }
  }
 ...
 * #### Getters/Virtuals
 *
 * E***REMOVED***ample of only applying path getters
 *
 *     doc.toObject({ getters: true, virtuals: false })
 *
 * E***REMOVED***ample of only applying virtual getters
 *
 *     doc.toObject({ virtuals: true })
 *
 * E***REMOVED***ample of applying both path and virtual getters
 *
 ...
 * @param {Boolean} [options.versionKey=true] if false, e***REMOVED***clude the version key (`__v` by default) from the output
 * @param {Boolean} [options.flattenMaps=false] if true, convert Maps to POJOs. Useful if you want to `JSON.stringify()` the result of `toObject()`.
 * @param {Boolean} [options.flattenObjectIds=false] if true, convert any ObjectIds in the result to 24 character he***REMOVED*** strings.
 * @param {Boolean} [options.useProjection=false] - If true, omits fields that are e***REMOVED***cluded in this document's projection. Unless you specified a projection, this will omit any field that has `select: false` in the schema.
...
function applyVirtuals(self, json, options, toObjectOptions) {
  ...
  for (i = 0; i < numPaths; ++i) {
    ...
    // We may be applying virtuals to a nested object, for e***REMOVED***ample if calling
    ...
    if (assignPath.inde***REMOVED***Of('.') === -1 && assignPath === path) {
...
/**
 * Populates paths on an e***REMOVED***isting document.
 *
 * #### E***REMOVED***ample:
 *
...
Document.prototype.$assertPopulated = function $assertPopulated(path, values) {
  ...
  if (!this.$populated(path)) {
    throw new MongooseError(`E***REMOVED***pected path "${path}" to be populated`);
  }
 ...
 * #### E***REMOVED***ample:
 *
 *     Model.findOne().populate('author').e***REMOVED***ec(function (err, doc) {
 *       console.log(doc.author.name); // Dr.Seuss
...
Document.prototype.depopulate = function(path) {
  if (typeof path === 'string') {
    path = path.inde***REMOVED***Of(' ') === -1 ? [path] : path.split(' ');
  }
  ...
  for (const singlePath of path) {
    ...
    if (virtualKeys.inde***REMOVED***Of(singlePath) !== -1) {
...
Document.prototype.$__delta = function $__delta() {
  ...
  for (; d < len; ++d) {
    ...
    // its parents is e***REMOVED***cluded, don't treat this path as dirty.
    if (this.$isDefault(data.path) && this.$__.selected) {
      if (data.path.inde***REMOVED***Of('.') === -1 && isPathE***REMOVED***cluded(this.$__.selected, data.path)) {
        continue;
      }

      const pathsToCheck = parentPaths(data.path);
      if (pathsToCheck.find(path => isPathE***REMOVED***cluded(this.$__.isSelected, path))) {
        continue;
      }
    }
...
function operand(self, where, delta, data, val, op) {
  ...
  // path e***REMOVED***cluded from versioning?
...
function shouldSkipVersioning(self, path) {
  ...
  // Remove any array inde***REMOVED***es from the path
  path = path.replace(/\.\d+\./, '.');
...
module.e***REMOVED***ports = e***REMOVED***ports = Document;
