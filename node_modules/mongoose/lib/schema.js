...
const getInde***REMOVED***es = require('./helpers/schema/getInde***REMOVED***es');
...
const hasNumericSubpathRege***REMOVED*** = /\.\d+(\.|$)/;
 ...
 *     new Schema({ name: String }, { id: false, autoInde***REMOVED***: false })
 *
 * #### Options:
 *
 * - [autoInde***REMOVED***](https://mongoosejs.com/docs/guide.html#autoInde***REMOVED***): bool - defaults to null (which means use the connection's autoInde***REMOVED*** option)
 ...
 * - [skipVersioning](https://mongoosejs.com/docs/guide.html#skipVersioning): object - paths to e***REMOVED***clude from versioning
 ...
 *
 * - `e***REMOVED***cludeInde***REMOVED***es`: bool - defaults to `false`. If `true`, skip building inde***REMOVED***es on this schema's paths.
 *
 * #### Note:
 *
 * _When nesting schemas, (`children` in the e***REMOVED***ample above), always declare the child schema first before passing it into its parent._
 *
...
function Schema(obj, options) {
  ...
  this.inherits = {};
  this.callQueue = [];
  this._inde***REMOVED***es = [];
  this._searchInde***REMOVED***es = [];
...
Schema.prototype._clone = function _clone(Constructor) {
  ...
  s._inde***REMOVED***es = clone(this._inde***REMOVED***es);
  s._searchInde***REMOVED***es = clone(this._searchInde***REMOVED***es);
...
Schema.prototype.defaultOptions = function(options) {
  ...
  options = {
    ...
    strictQuery,
    bufferCommands: true,
    capped: false, // { size, ma***REMOVED***, autoInde***REMOVED***Id }
    versionKey: '__v',
    optimisticConcurrency: false,
    minimize: true,
    autoInde***REMOVED***: null,
...
/**
 * Inherit a Schema by applying a discriminator on an e***REMOVED***isting Schema.
 *
 *
 * #### E***REMOVED***ample:
 *
 ...
 *     // getters, setters, inde***REMOVED***es, methods, and statics.
 *     TurboManSchema.add(ToySchema).add({ year: Number });
 *
 * @param {Object|Schema} obj plain object with paths to add, or another schema
 * @param {String} [prefi***REMOVED***] path to prefi***REMOVED*** the newly added paths with
 * @return {Schema} the Schema instance
 * @api public
 */

Schema.prototype.add = function add(obj, prefi***REMOVED***) {
  ...
  if (obj._id === false && prefi***REMOVED*** == null) {
    this.options._id = false;
  }

  prefi***REMOVED*** = prefi***REMOVED*** || '';
  // avoid prototype pollution
  if (prefi***REMOVED*** === '__proto__.' || prefi***REMOVED*** === 'constructor.' || prefi***REMOVED*** === 'prototype.') {
    return this;
  }
  ...
  for (const key of keys) {
    ...
    const fullPath = prefi***REMOVED*** + key;
    const val = obj[key];
    ...
    if (!(isPOJO(val) || val instanceof SchemaTypeOptions)) {
      ...
      // E***REMOVED***amples: Schema instances, SchemaType instances
      if (prefi***REMOVED***) {
        this.nested[prefi***REMOVED***.substring(0, prefi***REMOVED***.length - 1)] = true;
      }
      this.path(prefi***REMOVED*** + key, val);
      if (val[0] != null && !(val[0].instanceOfSchema) && utils.isPOJO(val[0].discriminators)) {
        const schemaType = this.path(prefi***REMOVED*** + key);
        for (const key in val[0].discriminators) {
          schemaType.discriminator(key, val[0].discriminators[key]);
        }
      }
    } else if (Object.keys(val).length < 1) {
      // Special-case: {} always interpreted as Mi***REMOVED***ed path so leaf at this node
      if (prefi***REMOVED***) {
        this.nested[prefi***REMOVED***.substring(0, prefi***REMOVED***.length - 1)] = true;
      }
      this.path(fullPath, val); // mi***REMOVED***ed type
      ...
      if (isPOJO(_typeDef) && Object.keys(_typeDef).length > 0) {
        ...
        this.path(prefi***REMOVED*** + key, schemaWrappedPath);
      } else {
        // Either the type is non-POJO or we interpret it as Mi***REMOVED***ed anyway
        if (prefi***REMOVED***) {
          this.nested[prefi***REMOVED***.substring(0, prefi***REMOVED***.length - 1)] = true;
        }
        this.path(prefi***REMOVED*** + key, val);
  ...
  const aliasObj = Object.fromEntries(
    Object.entries(obj).map(([key]) => ([prefi***REMOVED*** + key, null]))
  );
...
/**
 * Remove an inde***REMOVED*** by name or inde***REMOVED*** specification.
 *
 * removeInde***REMOVED*** only removes inde***REMOVED***es from your schema object. Does **not** affect the inde***REMOVED***es
 * in MongoDB.
 *
 * #### E***REMOVED***ample:
 *
 ...
 *
 *     // Add a new inde***REMOVED*** on { name, color }
 *     ToySchema.inde***REMOVED***({ name: 1, color: 1 });
 *
 *     // Remove inde***REMOVED*** on { name, color }
 *     // Keep in mind that order matters! `removeInde***REMOVED***({ color: 1, name: 1 })` won't remove the inde***REMOVED***
 *     ToySchema.removeInde***REMOVED***({ name: 1, color: 1 });
 *
 *     // Add an inde***REMOVED*** with a custom name
 *     ToySchema.inde***REMOVED***({ color: 1 }, { name: 'my custom inde***REMOVED*** name' });
 *     // Remove inde***REMOVED*** by name
 *     ToySchema.removeInde***REMOVED***('my custom inde***REMOVED*** name');
 *
 * @param {Object|string} inde***REMOVED*** name or inde***REMOVED*** specification
 * @return {Schema} the Schema instance
 * @api public
 */

Schema.prototype.removeInde***REMOVED*** = function removeInde***REMOVED***(inde***REMOVED***) {
  if (arguments.length > 1) {
    throw new Error('removeInde***REMOVED***() takes only 1 argument');
  }

  if (typeof inde***REMOVED*** !== 'object' && typeof inde***REMOVED*** !== 'string') {
    throw new Error('removeInde***REMOVED***() may only take either an object or a string as an argument');
  }

  if (typeof inde***REMOVED*** === 'object') {
    for (let i = this._inde***REMOVED***es.length - 1; i >= 0; --i) {
      if (util.isDeepStrictEqual(this._inde***REMOVED***es[i][0], inde***REMOVED***)) {
        this._inde***REMOVED***es.splice(i, 1);
      }
    }
  } else {
    for (let i = this._inde***REMOVED***es.length - 1; i >= 0; --i) {
      if (this._inde***REMOVED***es[i][1] != null && this._inde***REMOVED***es[i][1].name === inde***REMOVED***) {
        this._inde***REMOVED***es.splice(i, 1);
      }
    }
  }
...
/**
 * Remove all inde***REMOVED***es from this schema.
 *
 * clearInde***REMOVED***es only removes inde***REMOVED***es from your schema object. Does **not** affect the inde***REMOVED***es
 * in MongoDB.
 *
 * #### E***REMOVED***ample:
 *
 *     const ToySchema = new Schema({ name: String, color: String, price: Number });
 *     ToySchema.inde***REMOVED***({ name: 1 });
 *     ToySchema.inde***REMOVED***({ color: 1 });
 *
 *     // Remove all inde***REMOVED***es on this schema
 *     ToySchema.clearInde***REMOVED***es();
 *
 *     ToySchema.inde***REMOVED***es(); // []
 *
 * @return {Schema} the Schema instance
 * @api public
 */

Schema.prototype.clearInde***REMOVED***es = function clearInde***REMOVED***es() {
  this._inde***REMOVED***es.length = 0;

  return this;
};

/**
 * Add an [Atlas search inde***REMOVED***](https://www.mongodb.com/docs/atlas/atlas-search/create-inde***REMOVED***/) that Mongoose will create using `Model.createSearchInde***REMOVED***()`.
 * This function only works when connected to MongoDB Atlas.
 *
 * #### E***REMOVED***ample:
 *
 *     const ToySchema = new Schema({ name: String, color: String, price: Number });
 *     ToySchema.searchInde***REMOVED***({ name: 'test', definition: { mappings: { dynamic: true } } });
 *
...
Schema.prototype.searchInde***REMOVED*** = function searchInde***REMOVED***(description) {
  this._searchInde***REMOVED***es.push(description);
 ...
 * - validate
 *
 * _NOTE:_ Use of these terms as method names is permitted, but play at your own risk, as they may be e***REMOVED***isting mongoose document methods you are stomping on.
 *
 *      const schema = new Schema(..);
...
Schema.prototype.path = function(path, obj) {
  if (obj === undefined) {
    ...
    const cleanPath = _pathToPositionalSynta***REMOVED***(path);
    ...
    // Look if a parent of this path is mi***REMOVED***ed
    schematype = this.hasMi***REMOVED***edParent(cleanPath);
    if (schematype != null) {
      return schematype;
    }

    // subpaths?
    return hasNumericSubpathRege***REMOVED***.test(path)
  ...
  if (schemaType.$isSchemaMap) {
    ...
    // can easily build a rege***REMOVED***p out of this path, and '*' to imply "any key."
    const mapPath = path + '.$*';
...
function _pathToPositionalSynta***REMOVED***(path) {
  if (!/\.\d+/.test(path)) {
    return path;
  }
...
function getMapPath(schema, path) {
  ...
  for (const val of schema.mapPaths) {
    ...
    const re = new RegE***REMOVED***p('^' + _path.replace(/\.\$\*/g, '\\.[^.]+') + '$');
    if (re.test(path)) {
...
Schema.prototype.interpretAsType = function(path, obj, options) {
  ...
  // and default to mi***REMOVED***ed if not specified.
  ...
  if (utils.isPOJO(type) || type === 'mi***REMOVED***ed') {
    return new MongooseTypes.Mi***REMOVED***ed(path, obj);
  }
  ...
  if (Array.isArray(type) || type === Array || type === 'array' || type === MongooseTypes.Array) {
    ...
    if (typeof cast === 'string') {
      ...
      if (Object.keys(castFromTypeKey).length) {
        ...
        // Special case: empty object becomes mi***REMOVED***ed
        return new MongooseTypes.Array(path, MongooseTypes.Mi***REMOVED***ed, obj);
      }
    }
    ...
    if (cast) {
      ...
      if (name === void 0) {
        throw new TypeError('Invalid schema configuration: ' +
          ...
          'See https://mongoosejs.com/docs/guide.html#definition for more info on supported schema synta***REMOVED***es.');
      }
    ...
    return new MongooseTypes.Array(path, cast || MongooseTypes.Mi***REMOVED***ed, obj, options);
  }
  ...
  if (name === void 0) {
    throw new TypeError(`Invalid schema configuration: \`${path}\` schematype definition is ` +
      'invalid. See ' +
      'https://mongoosejs.com/docs/guide.html#definition for more info on supported schema synta***REMOVED***es.');
  }
...
/**
 * Returns inde***REMOVED***es from fields and schema-level inde***REMOVED***es (cached).
 *
 * @api private
 * @return {Array}
 */

Schema.prototype.inde***REMOVED***edPaths = function inde***REMOVED***edPaths() {
  if (this._inde***REMOVED***edpaths) {
    return this._inde***REMOVED***edpaths;
  }
  this._inde***REMOVED***edpaths = this.inde***REMOVED***es();
  return this._inde***REMOVED***edpaths;
};
...
/**
 * Returns true iff this path is a child of a mi***REMOVED***ed schema.
 *
 * @param {String} path
 * @return {Boolean}
 * @api private
...
Schema.prototype.hasMi***REMOVED***edParent = function(path) {
  const subpaths = path.split(/\./g);
  path = '';
  for (let i = 0; i < subpaths.length; ++i) {
    path = i > 0 ? path + '.' + subpaths[i] : subpaths[i];
    if (this.paths.hasOwnProperty(path) &&
        this.paths[path] instanceof MongooseTypes.Mi***REMOVED***ed) {
      return this.paths[path];
    }
  }
 ...
 *
 *     toySchema.pre('save', function(ne***REMOVED***t) {
 *       if (!this.created) this.created = new Date;
 *       ne***REMOVED***t();
 *     });
 *
 *     toySchema.pre('validate', function(ne***REMOVED***t) {
 *       if (this.name !== 'Woody') this.name = 'Woody';
 *       ne***REMOVED***t();
 *     });
 *
 *     // Equivalent to calling `pre()` on `find`, `findOne`, `findOneAndUpdate`.
 *     toySchema.pre(/^find/, function(ne***REMOVED***t) {
 *       console.log(this.getFilter());
 *     });
 *
 *     // Equivalent to calling `pre()` on `updateOne`, `findOneAndUpdate`.
 *     toySchema.pre(['updateOne', 'findOneAndUpdate'], function(ne***REMOVED***t) {
 *       console.log(this.getFilter());
 *     });
 *
 ...
 *     });
 *
 * @param {String|RegE***REMOVED***p|String[]} methodName The method name or regular e***REMOVED***pression to match method name
 * @param {Object} [options]
 * @param {Boolean} [options.document] If `name` is a hook for both document and query middleware, set to `true` to run on document middleware. For e***REMOVED***ample, set `options.document` to `true` to apply this hook to `Document#deleteOne()` rather than `Query#deleteOne()`.
...
Schema.prototype.pre = function(name) {
  if (name instanceof RegE***REMOVED***p) {
 ...
 *
 *     const s = new Schema({ name: String });
 *     s.plugin((schema, opts) => console.log(opts.te***REMOVED***t, schema.path('name').path), { te***REMOVED***t: "Schema Path Name:" });
 ...
 *     await Drink.findByName('LaCroi***REMOVED***');
 *
 * If a hash of name/fn pairs is passed as the only argument, each name/fn pair will be added as methods.
 *
 *     schema.static({
 ...
 * #### E***REMOVED***ample:
 *
 *     schema.inde***REMOVED***({ first: 1, last: -1 })
 *
 * @param {Object} fields The Fields to inde***REMOVED***, with the order, available values: `1 | -1 | '2d' | '2dsphere' | 'geoHaystack' | 'hashed' | 'te***REMOVED***t'`
 * @param {Object} [options] Options to pass to [MongoDB driver's `createInde***REMOVED***()` function](https://mongodb.github.io/node-mongodb-native/4.9/classes/Collection.html#createInde***REMOVED***)
 * @param {String | number} [options.e***REMOVED***pires=null] Mongoose-specific syntactic sugar, uses [ms](https://www.npmjs.com/package/ms) to convert `e***REMOVED***pires` option into seconds for the `e***REMOVED***pireAfterSeconds` in the above link.
 * @param {String} [options.language_override=null] Tells mongodb to use the specified field instead of `language` for parsing te***REMOVED***t inde***REMOVED***es.
 * @api public
 */

Schema.prototype.inde***REMOVED*** = function(fields, options) {
  fields || (fields = {});
  options || (options = {});

  if (options.e***REMOVED***pires) {
  ...
  this._inde***REMOVED***es.push([fields, options]);
  return this;
};
...
const inde***REMOVED***Types = '2d 2dsphere hashed te***REMOVED***t'.split(' ');

/**
 * The allowed inde***REMOVED*** types
 *
 * @property {String[]} inde***REMOVED***Types
 * @memberOf Schema
 * @static
 * @api public
 */

Object.defineProperty(Schema, 'inde***REMOVED***Types', {
  get: function() {
    return inde***REMOVED***Types;
  },
  set: function() {
    throw new Error('Cannot overwrite Schema.inde***REMOVED***Types');
  }
});

/**
 * Returns a list of inde***REMOVED***es that this schema declares, via `schema.inde***REMOVED***()` or by `inde***REMOVED***: true` in a path's options.
 * Inde***REMOVED***es are e***REMOVED***pressed as an array `[spec, options]`.
 *
 * #### E***REMOVED***ample:
 *
 *     const userSchema = new Schema({
 *       email: { type: String, required: true, unique: true },
 *       registeredAt: { type: Date, inde***REMOVED***: true }
 *     });
 *
 *     // [ [ { email: 1 }, { unique: true, background: true } ],
 *     //   [ { registeredAt: 1 }, { background: true } ] ]
 *     userSchema.inde***REMOVED***es();
 *
 * [Plugins](https://mongoosejs.com/docs/plugins.html) can use the return value of this function to modify a schema's inde***REMOVED***es.
 * For e***REMOVED***ample, the below plugin makes every inde***REMOVED*** unique by default.
 *
 *     function myPlugin(schema) {
 *       for (const inde***REMOVED*** of schema.inde***REMOVED***es()) {
 *         if (inde***REMOVED***[1].unique === undefined) {
 *           inde***REMOVED***[1].unique = true;
 *         }
 *       }
 *     }
 *
 * @api public
 * @return {Array} list of inde***REMOVED***es defined in the schema
 */

Schema.prototype.inde***REMOVED***es = function() {
  return getInde***REMOVED***es(this);
};
 ...
 * @returns {Schema} the Schema instance, or a mongoose error if the virtual does not e***REMOVED***ist.
 * @api public
 */
  ...
  if (Array.isArray(path)) {
    for (const virtual of path) {
      if (this.virtuals[virtual] == null) {
        throw new MongooseError(`Attempting to remove virtual "${virtual}" that does not e***REMOVED***ist.`);
      }
    }
    ...
    while (p--) {
      ...
      if (foundschema) {
        ...
        if (foundschema.caster) {
          // array of Mi***REMOVED***ed?
          if (foundschema.caster instanceof MongooseTypes.Mi***REMOVED***ed) {
          ...
          if (foundschema.$__schemaType instanceof MongooseTypes.Mi***REMOVED***ed) {
            return foundschema.$__schemaType;
          }
    ...
    while (p--) {
      ...
      if (foundschema) {
        if (foundschema.caster) {
          // array of Mi***REMOVED***ed?
          if (foundschema.caster instanceof MongooseTypes.Mi***REMOVED***ed) {
            return { schema: foundschema, pathType: 'mi***REMOVED***ed' };
          }
...
module.e***REMOVED***ports = e***REMOVED***ports = Schema;
 ...
 * - [Mi***REMOVED***ed](https://mongoosejs.com/docs/schematypes.html#mi***REMOVED***ed)
 * - [UUID](https://mongoosejs.com/docs/schematypes.html#uuid)
 * - [BigInt](https://mongoosejs.com/docs/schematypes.html#bigint)
 *
 * Using this e***REMOVED***posed access to the `Mi***REMOVED***ed` SchemaType, we can use them in our schema.
 *
 *     const Mi***REMOVED***ed = mongoose.Schema.Types.Mi***REMOVED***ed;
 *     new mongoose.Schema({ _user: Mi***REMOVED***ed })
 *
 * @api public
 */

Schema.Types = MongooseTypes = require('./schema/inde***REMOVED***');

/*!
 * ignore
 */

e***REMOVED***ports.ObjectId = MongooseTypes.ObjectId;
