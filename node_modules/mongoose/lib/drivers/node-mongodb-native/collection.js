...
function iter(i) {
  NativeCollection.prototype[i] = function() {
    ...
    try {
      ...
      // Collection operation may throw because of ma***REMOVED*** bson size, catch it here
      // See gh-3906
      if (typeof lastArg === 'function') {
...
NativeCollection.prototype.$print = function(name, i, args, color, shell) {
  const moduleName = color ? '\***REMOVED***1B[0;36mMongoose:\***REMOVED***1B[0m ' : 'Mongoose: ';
...
function formatObjectId(***REMOVED***, key) {
  ***REMOVED***[key] = inspectable('ObjectId("' + ***REMOVED***[key].toHe***REMOVED***String() + '")');
}
function formatDate(***REMOVED***, key, shell) {
  if (shell) {
    ***REMOVED***[key] = inspectable('ISODate("' + ***REMOVED***[key].toUTCString() + '")');
  } else {
    ***REMOVED***[key] = inspectable('new Date("' + ***REMOVED***[key].toUTCString() + '")');
  }
}
function format(obj, sub, color, shell) {
  ...
  let ***REMOVED*** = clone(obj, internalToObjectOptions);
  const constructorName = getConstructorName(***REMOVED***);

  if (constructorName === 'Binary') {
    ***REMOVED*** = 'BinData(' + ***REMOVED***.sub_type + ', "' + ***REMOVED***.toString('base64') + '")';
  } else if (constructorName === 'ObjectId') {
    ***REMOVED*** = inspectable('ObjectId("' + ***REMOVED***.toHe***REMOVED***String() + '")');
  } else if (constructorName === 'Date') {
    ***REMOVED*** = inspectable('new Date("' + ***REMOVED***.toUTCString() + '")');
    ...
    for (let i = 0; i < numKeys; ++i) {
      ...
      if (***REMOVED***[key]) {
        let error;
        if (typeof ***REMOVED***[key].toBSON === 'function') {
          try {
            ...
            ***REMOVED***[key] = ***REMOVED***[key].toBSON();
          } catch (_error) {
            error = _error;
          }
        }
        const _constructorName = getConstructorName(***REMOVED***[key]);
        if (_constructorName === 'Binary') {
          ***REMOVED***[key] = 'BinData(' + ***REMOVED***[key].sub_type + ', "' +
            ***REMOVED***[key].buffer.toString('base64') + '")';
        } else if (_constructorName === 'Object') {
          ***REMOVED***[key] = format(***REMOVED***[key], true);
        } else if (_constructorName === 'ObjectId') {
          formatObjectId(***REMOVED***, key);
        } else if (_constructorName === 'Date') {
          formatDate(***REMOVED***, key, shell);
        } else if (_constructorName === 'ClientSession') {
          ***REMOVED***[key] = inspectable('ClientSession("' +
            (
              ***REMOVED***[key] &&
              ***REMOVED***[key].id &&
              ***REMOVED***[key].id.id &&
              ***REMOVED***[key].id.id.buffer || ''
            ).toString('he***REMOVED***') + '")');
        } else if (Array.isArray(***REMOVED***[key])) {
          ***REMOVED***[key] = ***REMOVED***[key].map(map);
        } else if (error != null) {
  ...
  return util.
    inspect(***REMOVED***, false, 10, color).
    replace(/\n/g, '').
    replace(/\s{2,}/g, ' ');
}

/**
 * Retrieves information about this collections inde***REMOVED***es.
 *
 * @param {Function} callback
 * @method getInde***REMOVED***es
 * @api public
 */

NativeCollection.prototype.getInde***REMOVED***es = NativeCollection.prototype.inde***REMOVED***Information;

/*!
 * Module e***REMOVED***ports.
 */

module.e***REMOVED***ports = NativeCollection;
