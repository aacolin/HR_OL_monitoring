...
const SchemaTypes = require('./schema/inde***REMOVED***');
...
const objectIdHe***REMOVED***Rege***REMOVED***p = /^[0-9A-Fa-f]{24}$/;
 ...
 *
 * The e***REMOVED***ports object of the `mongoose` module is an instance of this class.
...
function Mongoose(options) {
  this.connections = [];
  this.ne***REMOVED***tConnectionId = 0;
  this.models = {};
  ...
  this.options = Object.assign({
    pluralization: true,
    autoInde***REMOVED***: true,
    autoCreate: true,
    autoSearchInde***REMOVED***: false
  }, options);
...
/**
 * E***REMOVED***pose connection states for user-land
 *
 * @memberOf Mongoose
 * @property STATES
 * @api public
 ...
 *
 *     mongoose.set({ debug: true, autoInde***REMOVED***: false }); // set multiple options at once
 *
 ...
 * - `autoCreate`: Set to `true` to make Mongoose call [`Model.createCollection()`](https://mongoosejs.com/docs/api/model.html#Model.createCollection()) automatically when you create a model with `mongoose.model()` or `conn.model()`. This is useful for testing transactions, change streams, and other features that require the collection to e***REMOVED***ist.
 * - `autoInde***REMOVED***`: `true` by default. Set to false to disable automatic inde***REMOVED*** creation for all models associated with this Mongoose instance.
 * - `bufferCommands`: enable/disable mongoose's buffering mechanism for all connections and models
 * - `bufferTimeoutMS`: If bufferCommands is on, this option sets the ma***REMOVED***imum amount of time Mongoose buffering will wait before throwing an error. If not specified, Mongoose will use 10000 (10 seconds).
 * - `cloneSchemas`: `false` by default. Set to `true` to `clone()` all schemas before compiling into a model.
 * - `debug`: If `true`, prints the operations mongoose sends to MongoDB to the console. If a writable stream is passed, it will log to that stream, without colorization. If a callback function is passed, it will receive the collection name, the method name, then all arguments passed to the method. For e***REMOVED***ample, if you wanted to replicate the default logging, you could output from the callback `Mongoose: ${collectionName}.${methodName}(${methodArgs.join(', ')})`.
 ...
 * - `ma***REMOVED***TimeMS`: If set, attaches [ma***REMOVED***TimeMS](https://www.mongodb.com/docs/manual/reference/operator/meta/ma***REMOVED***TimeMS/) to every query
...
Mongoose.prototype.set = function(key, value) {
  ...
  if (arguments.length === 1 && typeof key !== 'object') {
    if (VALID_OPTIONS.inde***REMOVED***Of(key) === -1) {
  ...
  for (const [optionKey, optionValue] of Object.entries(options)) {
    if (VALID_OPTIONS.inde***REMOVED***Of(optionKey) === -1) {
      if (!error) {
 ...
 * @param {Object} [options] passed down to the [MongoDB driver's `connect()` function](https://mongodb.github.io/node-mongodb-native/4.9/interfaces/MongoClientOptions.html), e***REMOVED***cept for 4 mongoose-specific options e***REMOVED***plained below.
 * @param {Boolean} [options.bufferCommands=true] Mongoose specific option. Set to false to [disable buffering](https://mongoosejs.com/docs/faq.html#callback_never_e***REMOVED***ecutes) on all models associated with this connection.
 ...
 * @param {Boolean} [options.autoInde***REMOVED***=true] Mongoose-specific option. Set to false to disable automatic inde***REMOVED*** creation for all models associated with this connection.
 ...
 * @param {Number} [options.ma***REMOVED***PoolSize=5] The ma***REMOVED***imum number of sockets the MongoDB driver will keep open for this connection. Keep in mind that MongoDB only allows one operation per socket at a time, so you may want to increase this if you find you have a few slow queries that are blocking faster queries from proceeding. See [Slow Trains in MongoDB and Node.js](https://thecodebarbarian.com/slow-trains-in-mongodb-and-nodejs).
...
Mongoose.prototype.createConnection = function(uri, options) {
  ...
  _mongoose.ne***REMOVED***tConnectionId++;
 ...
 *     const uri = 'mongodb://none***REMOVED***istent.domain:27000';
 ...
 * @param {String} [options.user] username for authentication, equivalent to `options.auth.user`. Maintained for backwards compatibility.
 * @param {String} [options.pass] password for authentication, equivalent to `options.auth.password`. Maintained for backwards compatibility.
 * @param {Number} [options.ma***REMOVED***PoolSize=100] The ma***REMOVED***imum number of sockets the MongoDB driver will keep open for this connection. Keep in mind that MongoDB only allows one operation per socket at a time, so you may want to increase this if you find you have a few slow queries that are blocking faster queries from proceeding. See [Slow Trains in MongoDB and Node.js](https://thecodebarbarian.com/slow-trains-in-mongodb-and-nodejs).
 ...
 * @param {Boolean} [options.overwriteModels=false] If true, overwrite e***REMOVED***isting models with the same name to avoid `OverwriteModelError`
 * @return {Model} The model associated with `name`. Mongoose will create the model if it doesn't already e***REMOVED***ist.
 * @api public
 */
...
Mongoose.prototype.model = function(name, schema, collection, options) {
  ...
  // an e***REMOVED***isting model name. in this case don't read from cache.
...
Mongoose.prototype._model = function(name, schema, collection, options) {
  ...
  if (typeof name === 'function') {
    ...
    if (!(model.prototype instanceof Model)) {
      throw new _mongoose.Error('The provided class ' + name + ' must e***REMOVED***tend Model');
    }
  }
 ...
 *
 * @api public
 * @param {String|RegE***REMOVED***p} name if string, the name of the model to remove. If rege***REMOVED***p, removes all models whose name matches the rege***REMOVED***p.
...
/**
 * An integer containing the value of the ne***REMOVED***t connection id. Calling
 ...
 *
 *     const mongoose = require('mongoose');
 *     mongoose.createConnection(); // id `0`, `ne***REMOVED***tConnectionId` becomes `1`
 *     mongoose.createConnection(); // id `1`, `ne***REMOVED***tConnectionId` becomes `2`
 *     mongoose.connections[0].destroy() // Removes connection with id `0`
 *     mongoose.createConnection(); // id `2`, `ne***REMOVED***tConnectionId` becomes `3`
 *
 * @memberOf Mongoose
 * @instance
 * @property {Number} ne***REMOVED***tConnectionId
 * @api private
 */

Mongoose.prototype.ne***REMOVED***tConnectionId;
 ...
 * #### E***REMOVED***ample:
 *
 *     console.log(mongoose.version); // '5.***REMOVED***.***REMOVED***'
 *
 * @property version
 * @api public
 */
 ...
 *
 * The e***REMOVED***ports of the mongoose module is an instance of this class.
 *
 * #### E***REMOVED***ample:
 *
 *     const mongoose = require('mongoose');
 ...
 *
 * Using this e***REMOVED***posed access to the `ObjectId` type, we can construct ids on demand.
 *
 ...
 * given value is a 24 character he***REMOVED*** string, which is the most commonly used string representation
 * of an ObjectId.
 *
 ...
 * `isObjectIdOrHe***REMOVED***String()` returns true only for `ObjectId` instances or 24 character he***REMOVED***
 * strings, and will return false for numbers, documents, and strings of length 12.
 *
 * #### E***REMOVED***ample:
 *
 *     mongoose.isObjectIdOrHe***REMOVED***String(new mongoose.Types.ObjectId()); // true
 *     mongoose.isObjectIdOrHe***REMOVED***String('62261a65d66c6be0a63c051f'); // true
 *
 *     mongoose.isObjectIdOrHe***REMOVED***String('0123456789ab'); // false
 *     mongoose.isObjectIdOrHe***REMOVED***String(6); // false
 *     mongoose.isObjectIdOrHe***REMOVED***String(new User({ name: 'test' })); // false
 *     mongoose.isObjectIdOrHe***REMOVED***String({ test: 42 }); // false
 *
 * @method isObjectIdOrHe***REMOVED***String
 * @param {Any} v
 * @returns {boolean} true if `v` is an ObjectId instance _or_ a 24 char he***REMOVED*** string
 * @api public
 */

Mongoose.prototype.isObjectIdOrHe***REMOVED***String = function(v) {
  return isBsonType(v, 'ObjectId') || (typeof v === 'string' && objectIdHe***REMOVED***Rege***REMOVED***p.test(v));
};

/**
 *
 * Syncs all the inde***REMOVED***es for the models registered with this connection.
 *
 * @param {Object} options
 * @param {Boolean} options.continueOnError `false` by default. If set to `true`, mongoose will not throw an error if one model syncing failed, and will return an object where the keys are the names of the models, and the values are the results/errors for each model.
 * @return {Promise} Returns a Promise, when the Promise resolves the value is a list of the dropped inde***REMOVED***es.
 */
Mongoose.prototype.syncInde***REMOVED***es = function(options) {
  const _mongoose = this instanceof Mongoose ? this : mongoose;
  return _mongoose.connection.syncInde***REMOVED***es(options);
};
...
/**
 * The Mongoose Mi***REMOVED***ed [SchemaType](https://mongoosejs.com/docs/schematypes.html). Used for
 * declaring paths in your schema that Mongoose's change tracking, casting,
 * and validation should ignore.
 *
 * #### E***REMOVED***ample:
 *
 *     const schema = new Schema({ arbitrary: mongoose.Mi***REMOVED***ed });
 *
 * @property Mi***REMOVED***ed
 * @api public
 */

Mongoose.prototype.Mi***REMOVED***ed = SchemaTypes.Mi***REMOVED***ed;
...
Mongoose.prototype.Error = require('./error/inde***REMOVED***');
 ...
 *       // Will skip e***REMOVED***ecuting `save()`, but will e***REMOVED***ecute post hooks as if
 *       // `save()` had e***REMOVED***ecuted with the result `{ matchedCount: 0 }`
...
/**
 * The e***REMOVED***ports object is an instance of Mongoose.
 *
 * @api private
 */

const mongoose = module.e***REMOVED***ports = e***REMOVED***ports = new Mongoose({
