...
const SyncInde***REMOVED***esError = require('./error/syncInde***REMOVED***es');
 ...
 * @event `connecting`: Emitted when `connection.openUri()` is e***REMOVED***ecuted on this connection.
 * @event `connected`: Emitted when this connection successfully connects to the db. May be emitted _multiple_ times in `reconnected` scenarios.
 * @event `open`: Emitted after we `connected` and `onOpen` is e***REMOVED***ecuted on all of this connection's models.
 * @event `disconnecting`: Emitted when `connection.close()` was e***REMOVED***ecuted.
 * @event `disconnected`: Emitted after getting disconnected from the db.
 * @event `close`: Emitted after we `disconnected` and `onClose` e***REMOVED***ecuted on all of this connection's models.
...
function Connection(base) {
  ...
  if (typeof base === 'undefined' || !base.connections.length) {
    ...
    this.id = base.ne***REMOVED***tConnectionId;
  }

  // Internal queue of objects `{ fn, ct***REMOVED***, args }` that Mongoose calls when this connection is successfully
 ...
 *
 * - `ma***REMOVED***TimeMS`: Set [`ma***REMOVED***TimeMS`](https://mongoosejs.com/docs/api/query.html#Query.prototype.ma***REMOVED***TimeMS()) for all queries on this connection.
...
/**
 * Helper for `createCollection()`. Will e***REMOVED***plicitly create the given collection
 ...
 *
 * @method withSession
 * @param {Function} e***REMOVED***ecutor called with 1 argument: a `ClientSession` instance
 * @return {Promise} resolves to the return value of the e***REMOVED***ecutor function
 * @api public
 */

Connection.prototype.withSession = async function withSession(e***REMOVED***ecutor) {
  if (arguments.length === 0) {
    throw new Error('Please provide an e***REMOVED***ecutor function');
  }
  return await this.client.withSession(e***REMOVED***ecutor);
};
...
/**
 * _Requires MongoDB >= 3.6.0._ E***REMOVED***ecutes the wrapped async function
 * in a transaction. Mongoose will commit the transaction if the
 * async function e***REMOVED***ecutes successfully and attempt to retry if
 ...
 *
 * @method transaction
 * @param {Function} fn Function to e***REMOVED***ecute in a transaction
 ...
 * all documents and inde***REMOVED***es.
 *
 ...
 * collections, documents, and inde***REMOVED***es.
 *
 * #### E***REMOVED***ample:
 *
...
Connection.prototype.onOpen = function() {
  ...
  for (const d of this._queue) {
    d.fn.apply(d.ct***REMOVED***, d.args);
  }
  this._queue = [];
 ...
 * to avoid uncaught e***REMOVED***ceptions when using `on('error')`. See gh-14377.
 */
...
Connection.prototype._close = async function _close(force, destroy) {
  ...
  switch (this.readyState) {
    case STATES.disconnected:
      if (destroy && this.base.connections.inde***REMOVED***Of(conn) !== -1) {
        this.base.connections.splice(this.base.connections.inde***REMOVED***Of(conn), 1);
      }
      if (!closeCalled) {
    ...
    case STATES.connected:
      ...
      await this.doClose(force);
      if (destroy && _this.base.connections.inde***REMOVED***Of(conn) !== -1) {
        this.base.connections.splice(this.base.connections.inde***REMOVED***Of(conn), 1);
      }
      this.onClose(force);
 ...
 * This method returns a thin wrapper around a [MongoDB Node.js driver collection]([MongoDB Node.js driver collection](https://mongodb.github.io/node-mongodb-native/Ne***REMOVED***t/classes/Collection.html)).
...
Connection.prototype.collection = function(name, options) {
  const defaultOptions = {
    autoInde***REMOVED***: this.config.autoInde***REMOVED*** != null ? this.config.autoInde***REMOVED*** : this.base.options.autoInde***REMOVED***,
    autoCreate: this.config.autoCreate != null ? this.config.autoCreate : this.base.options.autoCreate,
    autoSearchInde***REMOVED***: this.config.autoSearchInde***REMOVED*** != null ? this.config.autoSearchInde***REMOVED*** : this.base.options.autoSearchInde***REMOVED***
  };
...
/**
 * Declares a plugin e***REMOVED***ecuted on all schemas you pass to `conn.model()`
 *
 ...
 *
 * @param {String|Function} name the model name or class e***REMOVED***tending Model
...
Connection.prototype.model = function(name, schema, collection, options) {
  ...
  if (this.models[name] && !collection && opts.overwriteModels !== true) {
    // model e***REMOVED***ists but we are not subclassing with custom collection
...
/**
 * Removes the model named `name` from this connection, if it e***REMOVED***ists. You can
...
Connection.prototype.deleteModel = function(name) {
  ...
  } else if (name instanceof RegE***REMOVED***p) {
    const pattern = name;
...
Connection.prototype.authMechanismDoesNotRequirePassword = function() {
  if (this.options && this.options.auth) {
    return noPasswordAuthMechanisms.inde***REMOVED***Of(this.options.auth.authMechanism) >= 0;
  }
  return true;
};
 ...
 * @return {Promise<Object>} Returns a Promise, when the Promise resolves the value is a list of the dropped inde***REMOVED***es.
 */
Connection.prototype.syncInde***REMOVED***es = async function syncInde***REMOVED***es(options = {}) {
  const result = {};
  const errorsMap = { };
  ...
  for (const model of Object.values(this.models)) {
    try {
      result[model.modelName] = await model.syncInde***REMOVED***es(options);
    } catch (err) {
      if (!continueOnError) {
        errorsMap[model.modelName] = err;
        break;
  ...
  if (!continueOnError && Object.keys(errorsMap).length) {
    const message = Object.entries(errorsMap).map(([modelName, err]) => `${modelName}: ${err.message}`).join(', ');
    const syncInde***REMOVED***esError = new SyncInde***REMOVED***esError(message, errorsMap);
    throw syncInde***REMOVED***esError;
  }
...
module.e***REMOVED***ports = Connection;
