...
/**
 * Declares a TTL inde***REMOVED*** (rounded to the nearest second) for _Date_ types only.
 *
 * This sets the `e***REMOVED***pireAfterSeconds` inde***REMOVED*** option available in MongoDB >= 2.1.2.
 * This inde***REMOVED*** type is only compatible with Date types.
 *
 * #### E***REMOVED***ample:
 *
 *     // e***REMOVED***pire in 24 hours
 *     new Schema({ createdAt: { type: Date, e***REMOVED***pires: 60*60*24 }});
 *
 ...
 *
 *     // e***REMOVED***pire in 24 hours
 *     new Schema({ createdAt: { type: Date, e***REMOVED***pires: '24h' }});
 *
 *     // e***REMOVED***pire in 1.5 hours
 *     new Schema({ createdAt: { type: Date, e***REMOVED***pires: '1.5h' }});
 *
 *     // e***REMOVED***pire in 7 days
 *     const schema = new Schema({ createdAt: Date });
 *     schema.path('createdAt').e***REMOVED***pires('7d');
 *
...
SchemaDate.prototype.e***REMOVED***pires = function(when) {
  if (getConstructorName(this._inde***REMOVED***) !== 'Object') {
    this._inde***REMOVED*** = {};
  }

  this._inde***REMOVED***.e***REMOVED***pires = when;
...
/**
 * Sets a ma***REMOVED***imum date validator.
 *
 * #### E***REMOVED***ample:
 *
 *     const s = new Schema({ d: { type: Date, ma***REMOVED***: Date('2014-01-01') })
 *     const M = db.model('M', s)
 ...
 *     const ma***REMOVED*** = [Date('2014-01-01'), 'The value of path `{PATH}` ({VALUE}) e***REMOVED***ceeds the limit ({MAX}).'];
 *     const schema = new Schema({ d: { type: Date, ma***REMOVED***: ma***REMOVED*** })
 *     const M = mongoose.model('M', schema);
 *     const s= new M({ d: Date('2014-12-08') });
 *     s.validate(function (err) {
 *       console.log(String(err)) // ValidationError: The value of path `d` (2014-12-08) e***REMOVED***ceeds the limit (2014-01-01).
 *     })
 *
 * @param {Date} ma***REMOVED***imum date
 * @param {String} [message] optional custom error message
 * @return {SchemaType} this
 * @see Customized Error Messages https://mongoosejs.com/docs/api/error.html#Error.messages
 * @api public
...
SchemaDate.prototype.ma***REMOVED*** = function(value, message) {
  if (this.ma***REMOVED***Validator) {
    this.validators = this.validators.filter(function(v) {
      return v.validator !== this.ma***REMOVED***Validator;
    }, this);
  }

  if (value) {
    let msg = message || MongooseError.messages.Date.ma***REMOVED***;
    if (typeof msg === 'string') {
      msg = msg.replace(/{MAX}/, (value === Date.now ? 'Date.now()' : value.toString()));
    }
    const _this = this;
    this.validators.push({
      validator: this.ma***REMOVED***Validator = function(val) {
        let _value = value;
        if (typeof _value === 'function' && _value !== Date.now) {
          _value = _value.call(this);
        }
        const ma***REMOVED*** = (_value === Date.now ? _value() : _this.cast(_value));
        return val === null || val.valueOf() <= ma***REMOVED***.valueOf();
      },
      message: msg,
      type: 'ma***REMOVED***',
      ma***REMOVED***: value
...
SchemaDate.prototype.castForQuery = function($conditional, val, conte***REMOVED***t) {
  if ($conditional == null) {
    try {
...
module.e***REMOVED***ports = SchemaDate;
