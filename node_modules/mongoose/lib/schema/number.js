...
/**
 * Sets a ma***REMOVED***imum number validator.
 *
 * #### E***REMOVED***ample:
 *
 *     const s = new Schema({ n: { type: Number, ma***REMOVED***: 10 })
 *     const M = db.model('M', s)
 *     const m = new M({ n: 11 })
 *     m.save(function (err) {
 *       console.error(err) // validator error
 ...
 *     const ma***REMOVED*** = [10, 'The value of path `{PATH}` ({VALUE}) e***REMOVED***ceeds the limit ({MAX}).'];
 *     const schema = new Schema({ n: { type: Number, ma***REMOVED***: ma***REMOVED*** })
 *     const M = mongoose.model('Measurement', schema);
 *     const s= new M({ n: 4 });
 *     s.validate(function (err) {
 *       console.log(String(err)) // ValidationError: The value of path `n` (4) e***REMOVED***ceeds the limit (10).
 *     })
 *
 * @param {Number} ma***REMOVED***imum number
 * @param {String} [message] optional custom error message
 * @return {SchemaType} this
 * @see Customized Error Messages https://mongoosejs.com/docs/api/error.html#Error.messages
 * @api public
...
SchemaNumber.prototype.ma***REMOVED*** = function(value, message) {
  ...
  if (value !== null && value !== undefined) {
    let msg = message || MongooseError.messages.Number.ma***REMOVED***;
    msg = msg.replace(/{MAX}/, value);
    this.validators.push({
      validator: this.ma***REMOVED***Validator = function(v) {
        return v == null || v <= value;
      },
      message: msg,
      type: 'ma***REMOVED***',
...
SchemaNumber.prototype.enum = function(values, message) {
  ...
  if (!Array.isArray(values)) {
    const isObjectSynta***REMOVED*** = utils.isPOJO(values) && values.values != null;
    if (isObjectSynta***REMOVED***) {
  ...
  this.enumValidator = v => v == null || values.inde***REMOVED***Of(v) !== -1;
  this.validators.push({
...
SchemaNumber.prototype.castForQuery = function($conditional, val, conte***REMOVED***t) {
  let handler;
  if ($conditional != null) {
  ...
  try {
    val = this.applySetters(val, conte***REMOVED***t);
  } catch (err) {
...
module.e***REMOVED***ports = SchemaNumber;
