...
function SchemaArray(key, cast, options, schemaOptions) {
  ...
  if (cast) {
    ...
    const Types = require('./inde***REMOVED***.js');
...
SchemaArray.prototype._applySetters = function(value, scope, init, priorVal) {
  if (this.casterConstructor.$isMongooseArray &&
    ...
    if (value != null && value.length !== 0) {
      ...
      if (valueDepth.min === valueDepth.ma***REMOVED*** && valueDepth.ma***REMOVED*** < depth && valueDepth.containsNonArrayItem) {
        for (let i = valueDepth.ma***REMOVED***; i < depth; ++i) {
          value = [value];
        }
      }
    }
...
SchemaArray.prototype.cast = function(value, doc, init, prev, options) {
  ...
  if (Array.isArray(value)) {
    ...
    if (!len && doc) {
      const inde***REMOVED***es = doc.schema.inde***REMOVED***edPaths();

      const arrayPath = this.path;
      for (i = 0, l = inde***REMOVED***es.length; i < l; ++i) {
        const pathInde***REMOVED*** = inde***REMOVED***es[i][0][arrayPath];
        if (pathInde***REMOVED*** === '2dsphere' || pathInde***REMOVED*** === '2d') {
          return;
        }
      }

      // Special case: if this inde***REMOVED*** is on the parent of what looks like
      // GeoJSON, skip setting the default to empty array re: #1668, #3233
      const arrayGeojsonPath = this.path.endsWith('.coordinates') ?
        this.path.substring(0, this.path.lastInde***REMOVED***Of('.')) : null;
      if (arrayGeojsonPath != null) {
        for (i = 0, l = inde***REMOVED***es.length; i < l; ++i) {
          const pathInde***REMOVED*** = inde***REMOVED***es[i][0][arrayGeojsonPath];
          if (pathInde***REMOVED*** === '2dsphere') {
            return;
          }
        }
      }
    ...
    if (options.arrayPathInde***REMOVED*** != null) {
      path += '.' + options.arrayPathInde***REMOVED***;
    }
    ...
    if (caster && this.casterConstructor !== Mi***REMOVED***ed) {
      try {
        const len = rawValue.length;
        for (i = 0; i < len; i++) {
          const opts = {};
          // Perf: creating `arrayPath` is e***REMOVED***pensive for large arrays.
          // We only need `arrayPath` if this is a nested array, so
          // skip if possible.
          if (isMongooseArray) {
            if (options.arrayPath != null) {
              opts.arrayPathInde***REMOVED*** = i;
...
SchemaArray.prototype._castForQuery = function(val, conte***REMOVED***t) {
  ...
  if (Array.isArray(val)) {
    ...
    val = val.map(function(v) {
      ...
      if (protoCastForQuery) {
        v = protoCastForQuery.call(caster, null, v, conte***REMOVED***t);
        return v;
      } else if (protoCast) {
        v = protoCast.call(caster, v);
        return v;
      } else if (constructorCastForQuery) {
        v = constructorCastForQuery.call(caster, null, v, conte***REMOVED***t);
        return v;
      }
      if (v != null) {
        v = new Constructor(v);
    ...
    val = protoCastForQuery.call(caster, null, val, conte***REMOVED***t);
  } else if (protoCast) {
    val = protoCast.call(caster, val);
  } else if (constructorCastForQuery) {
    val = constructorCastForQuery.call(caster, null, val, conte***REMOVED***t);
  } else if (val != null) {
...
SchemaArray.prototype.castForQuery = function($conditional, val, conte***REMOVED***t) {
  let handler;
  ...
  if ($conditional != null) {
    ...
    return this._castForQuery(val, conte***REMOVED***t);
  }
};
...
function cast$all(val, conte***REMOVED***t) {
  if (!Array.isArray(val)) {
    val = [val];
  }
  ...
  val = val.map((v) => {
    ...
    if (v.$elemMatch != null) {
      return { $elemMatch: cast(this.casterConstructor.schema, v.$elemMatch, null, this && this.$$conte***REMOVED***t) };
    }

    const o = {};
    o[this.path] = v;
    return cast(this.casterConstructor.schema, o, null, this && this.$$conte***REMOVED***t)[this.path];
  }, this);

  return this.castForQuery(null, val, conte***REMOVED***t);
}

function cast$elemMatch(val, conte***REMOVED***t) {
  ...
  for (let i = 0; i < numKeys; ++i) {
    ...
    if (isOperator(key) && value != null) {
      val[key] = this.castForQuery(key, value, conte***REMOVED***t);
    }
  }
...
function createLogicalQueryOperatorHandler(op) {
  return function logicalQueryOperatorHandler(val, conte***REMOVED***t) {
    if (!Array.isArray(val)) {
    ...
    for (const obj of val) {
      ret.push(cast(this.casterConstructor.schema ?? conte***REMOVED***t.schema, obj, null, this && this.$$conte***REMOVED***t));
    }
...
handle.$size =
handle.$minDistance =
handle.$ma***REMOVED***Distance = castToNumber;

handle.$e***REMOVED***ists = $e***REMOVED***ists;
handle.$type = $type;
...
module.e***REMOVED***ports = SchemaArray;
