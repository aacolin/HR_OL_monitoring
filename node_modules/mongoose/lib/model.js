...
const applySchemaCollation = require('./helpers/inde***REMOVED***es/applySchemaCollation');
...
const isDefaultIdInde***REMOVED*** = require('./helpers/inde***REMOVED***es/isDefaultIdInde***REMOVED***');
const isInde***REMOVED***Equal = require('./helpers/inde***REMOVED***es/isInde***REMOVED***Equal');
const {
  getRelatedDBInde***REMOVED***es,
  getRelatedSchemaInde***REMOVED***es
} = require('./helpers/inde***REMOVED***es/getRelatedInde***REMOVED***es');
const decorateDiscriminatorInde***REMOVED***Options = require('./helpers/inde***REMOVED***es/decorateDiscriminatorInde***REMOVED***Options');
 ...
 * @event `inde***REMOVED***`: Emitted after `Model#ensureInde***REMOVED***es` completes. If an error occurred it is passed with the event.
 * @event `inde***REMOVED***-single-start`: Emitted when an individual inde***REMOVED*** starts within `Model#ensureInde***REMOVED***es`. The fields and options being used to build the inde***REMOVED*** are also passed with the event.
 * @event `inde***REMOVED***-single-done`: Emitted when an individual inde***REMOVED*** finishes within `Model#ensureInde***REMOVED***es`. If an error occurred it is passed with the event. The fields, options, and inde***REMOVED*** name are also passed.
 * @api public
 */
 ...
 * A Mongoose collection is a thin wrapper around a [MongoDB Node.js driver collection]([MongoDB Node.js driver collection](https://mongodb.github.io/node-mongodb-native/Ne***REMOVED***t/classes/Collection.html)).
...
Model.prototype.$__handleSave = function(options, callback) {
  ...
  // since it already e***REMOVED***ists
  ...
  function handleEmptyUpdate() {
    ...
    this.constructor.collection.findOne(where, optionsWithCustomValues)
      .then(documentE***REMOVED***ists => {
        const matchedCount = !documentE***REMOVED***ists ? 0 : 1;
...
Model.prototype.$__save = function(options, callback) {
  this.$__handleSave(options, (error, result) => {
    if (error) {
      ...
      return hooks.e***REMOVED***ecPost('save:error', this, [this], { error: error }, (error) => {
        callback(error, this);
      });
    }
    let numAffected = 0;
 ...
 * @throws {DocumentNotFoundError} if this [save updates an e***REMOVED***isting document](https://mongoosejs.com/docs/api/document.html#Document.prototype.isNew) but the document doesn't e***REMOVED***ist in the database. For e***REMOVED***ample, you will get this error if the document is [deleted between when you retrieved the document and when you saved it](documents.html#updating).
 * @return {Promise}
 * @api public
...
Model.prototype.deleteOne = function deleteOne(options) {
  ...
  query.pre(function queryPreDeleteOne(cb) {
    self.constructor._middleware.e***REMOVED***ecPre('deleteOne', self, [self], cb);
  });
  query.pre(function callSubdocPreHooks(cb) {
    each(self.$getAllSubdocs(), (subdoc, cb) => {
      subdoc.constructor._middleware.e***REMOVED***ecPre('deleteOne', subdoc, [subdoc], cb);
    }, cb);
  });
  ...
  query.post(function callSubdocPostHooks(cb) {
    each(self.$getAllSubdocs(), (subdoc, cb) => {
      subdoc.constructor._middleware.e***REMOVED***ecPost('deleteOne', subdoc, [subdoc], {}, cb);
    }, cb);
  });
  query.post(function queryPostDeleteOne(cb) {
    self.constructor._middleware.e***REMOVED***ecPost('deleteOne', self, [self], {}, cb);
  });
...
/**
 * Returns a document with `_id` only if at least one document e***REMOVED***ists in the database that matches
 * the given `filter`, and `null` otherwise.
 *
 * Under the hood, `MyModel.e***REMOVED***ists({ answer: 42 })` is equivalent to
 ...
 *
 *     await Character.e***REMOVED***ists({ name: /picard/i }); // { _id: ... }
 *     await Character.e***REMOVED***ists({ name: /riker/i }); // null
 *
...
Model.e***REMOVED***ists = function e***REMOVED***ists(filter, options) {
  _checkConte***REMOVED***t(this, 'e***REMOVED***ists');
  if (typeof arguments[2] === 'function') {
    throw new MongooseError('Model.e***REMOVED***ists() no longer accepts a callback');
  }
...
Model.discriminator = function(name, schema, options) {
  ...
  if (typeof name === 'function') {
    ...
    if (!(model.prototype instanceof Model)) {
      throw new MongooseError('The provided class ' + name + ' must e***REMOVED***tend Model');
    }
  }
  ...
  _checkConte***REMOVED***t(this, 'discriminator');
...
function _checkConte***REMOVED***t(ct***REMOVED***, fnName) {
  // Check conte***REMOVED***t, because it is easy to mistakenly type
  // `new Model.discriminator()` and get an incomprehensible error
  if (ct***REMOVED*** == null || ct***REMOVED*** === global) {
  ...
  } else if (ct***REMOVED***[modelSymbol] == null) {
    throw new MongooseError('`Model.' + fnName + '()` cannot run without a ' +
 ...
 * - `ensureInde***REMOVED***es()` unless [`autoInde***REMOVED***`](https://mongoosejs.com/docs/guide.html#autoInde***REMOVED***) option is turned off
 * - `createSearchInde***REMOVED***()` on all schema search inde***REMOVED***es if `autoSearchInde***REMOVED***` is enabled.
 *
 ...
 * don't need to call `init()` to trigger inde***REMOVED*** builds.
 *
 * However, you _may_ need to call `init()`  to get back a promise that will resolve when your inde***REMOVED***es are finished.
 * Calling `await Model.init()` is helpful if you need to wait for inde***REMOVED***es to build before continuing.
 * For e***REMOVED***ample, if you want to wait for unique inde***REMOVED***es to build before continuing with a test case.
 *
 * #### E***REMOVED***ample:
 *
 ...
 *
 *     await Event.init();
 *     console.log('Inde***REMOVED***es are done building!');
 *
 * @api public
 * @returns {Promise}
 */
...
Model.init = function init() {
  _checkConte***REMOVED***t(this, 'init');
  ...
  const conn = this.db;
  const _ensureInde***REMOVED***es = async() => {
    const autoInde***REMOVED*** = utils.getOption(
      'autoInde***REMOVED***',
      this.schema.options,
      conn.config,
      conn.base.options
    ...
    return await this.ensureInde***REMOVED***es({ _automatic: true });
  };
  const _createSearchInde***REMOVED***es = async() => {
    const autoSearchInde***REMOVED*** = utils.getOption(
      'autoSearchInde***REMOVED***',
      this.schema.options,
      conn.config,
      conn.base.options
    );
    if (!autoSearchInde***REMOVED***) {
      return;
    }

    const results = [];
    for (const searchInde***REMOVED*** of this.schema._searchInde***REMOVED***es) {
      results.push(await this.createSearchInde***REMOVED***(searchInde***REMOVED***));
    }
    return results;
  };
  ...
  this.$init = _createCollection().
    then(() => _ensureInde***REMOVED***es()).
    then(() => _createSearchInde***REMOVED***es());
 ...
 *
 * Note 2: You don't have to call this if your schema contains inde***REMOVED*** or unique field.
...
Model.createCollection = async function createCollection(options) {
  _checkConte***REMOVED***t(this, 'createCollection');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function') {
    throw new MongooseError('Model.createCollection() no longer accepts a callback');
  }

  const shouldSkip = await new Promise((resolve, reject) => {
    this.hooks.e***REMOVED***ecPre('createCollection', this, [options], (err) => {
      if (err != null) {
  ...
  if (timeseries != null) {
    ...
    if (options.e***REMOVED***pireAfterSeconds != null) {
      // do nothing
    } else if (options.e***REMOVED***pires != null) {
      utils.e***REMOVED***pires(options);
    } else if (this.schema.options.e***REMOVED***pireAfterSeconds != null) {
      options.e***REMOVED***pireAfterSeconds = this.schema.options.e***REMOVED***pireAfterSeconds;
    } else if (this.schema.options.e***REMOVED***pires != null) {
      options.e***REMOVED***pires = this.schema.options.e***REMOVED***pires;
      utils.e***REMOVED***pires(options);
    }
  }

  const clusteredInde***REMOVED*** = this &&
    this.schema &&
    this.schema.options &&
    this.schema.options.clusteredInde***REMOVED***;
  if (clusteredInde***REMOVED*** != null) {
    options = Object.assign({ clusteredInde***REMOVED***: { ...clusteredInde***REMOVED***, unique: true } }, options);
  }
  ...
  try {
    ...
    if (err != null && (err.name !== 'MongoServerError' || err.code !== 48)) {
      await new Promise((resolve, reject) => {
        ...
        this.hooks.e***REMOVED***ecPost('createCollection', this, [null], _opts, (err) => {
          if (err != null) {
            return reject(err);
          }
          resolve();
  ...
  await new Promise((resolve, reject) => {
    this.hooks.e***REMOVED***ecPost('createCollection', this, [this.$__collection], (err) => {
      if (err != null) {
        return reject(err);
      }
      resolve();
 ...
 *
 * See the [introductory blog post](https://thecodebarbarian.com/whats-new-in-mongoose-5-2-syncinde***REMOVED***es)
 ...
 *     await Customer.collection.createInde***REMOVED***({ age: 1 }); // Inde***REMOVED*** is not in schema
 *     // Will drop the 'age' inde***REMOVED*** and create an inde***REMOVED*** on `name`
 *     await Customer.syncInde***REMOVED***es();
 *
 * You should be careful about running `syncInde***REMOVED***es()` on production applications under heavy load,
 * because inde***REMOVED*** builds are e***REMOVED***pensive operations, and une***REMOVED***pected inde***REMOVED*** drops can lead to degraded
 * performance. Before running `syncInde***REMOVED***es()`, you can use the [`diffInde***REMOVED***es()` function](#Model.diffInde***REMOVED***es())
 * to check what inde***REMOVED***es `syncInde***REMOVED***es()` will drop and create.
 *
 * #### E***REMOVED***ample:
 *
 *     const { toDrop, toCreate } = await Model.diffInde***REMOVED***es();
 *     toDrop; // Array of strings containing names of inde***REMOVED***es that `syncInde***REMOVED***es()` will drop
 *     toCreate; // Array of strings containing names of inde***REMOVED***es that `syncInde***REMOVED***es()` will create
 *
 * @param {Object} [options] options to pass to `ensureInde***REMOVED***es()`
 * @param {Boolean} [options.background=null] if specified, overrides each inde***REMOVED***'s `background` property
 * @param {Boolean} [options.hideInde***REMOVED***es=false] set to `true` to hide inde***REMOVED***es instead of dropping. Requires MongoDB server 4.4 or higher
 * @return {Promise}
 * @api public
 */

Model.syncInde***REMOVED***es = async function syncInde***REMOVED***es(options) {
  _checkConte***REMOVED***t(this, 'syncInde***REMOVED***es');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function') {
    throw new MongooseError('Model.syncInde***REMOVED***es() no longer accepts a callback');
  }
  ...
  const diffInde***REMOVED***esResult = await model.diffInde***REMOVED***es();
  const dropped = await model.cleanInde***REMOVED***es({ ...options, toDrop: diffInde***REMOVED***esResult.toDrop });
  await model.createInde***REMOVED***es({ ...options, toCreate: diffInde***REMOVED***esResult.toCreate });
 ...
 *     const schema = new Schema({ name: { type: String, unique: true } });
 *     const Customer = mongoose.model('Customer', schema);
 *     await Customer.createSearchInde***REMOVED***({ name: 'test', definition: { mappings: { dynamic: true } } });
 *
 * @param {Object} description inde***REMOVED*** options, including `name` and `definition`
...
Model.createSearchInde***REMOVED*** = async function createSearchInde***REMOVED***(description) {
  _checkConte***REMOVED***t(this, 'createSearchInde***REMOVED***');

  return await this.$__collection.createSearchInde***REMOVED***(description);
};
 ...
 *     const schema = new Schema({ name: { type: String, unique: true } });
 *     const Customer = mongoose.model('Customer', schema);
 *     await Customer.updateSearchInde***REMOVED***('test', { mappings: { dynamic: true } });
 *
 * @param {String} name
...
Model.updateSearchInde***REMOVED*** = async function updateSearchInde***REMOVED***(name, definition) {
  _checkConte***REMOVED***t(this, 'updateSearchInde***REMOVED***');

  return await this.$__collection.updateSearchInde***REMOVED***(name, definition);
};
 ...
 *     const schema = new Schema({ name: { type: String, unique: true } });
 *     const Customer = mongoose.model('Customer', schema);
 *     await Customer.dropSearchInde***REMOVED***('test');
 *
 * @param {String} name
 * @return {Promise}
 * @api public
...
Model.dropSearchInde***REMOVED*** = async function dropSearchInde***REMOVED***(name) {
  _checkConte***REMOVED***t(this, 'dropSearchInde***REMOVED***');

  return await this.$__collection.dropSearchInde***REMOVED***(name);
};
 ...
 *
 *     await Customer.createSearchInde***REMOVED***({ name: 'test', definition: { mappings: { dynamic: true } } });
 *     const res = await Customer.listSearchInde***REMOVED***es(); // Includes `[{ name: 'test' }]`
 *
...
Model.listSearchInde***REMOVED***es = async function listSearchInde***REMOVED***es(options) {
  _checkConte***REMOVED***t(this, 'listSearchInde***REMOVED***es');

  const cursor = await this.$__collection.listSearchInde***REMOVED***es(options);

  return await cursor.toArray();
};

/**
 * Does a dry-run of `Model.syncInde***REMOVED***es()`, returning the inde***REMOVED***es that `syncInde***REMOVED***es()` would drop and create if you were to run `syncInde***REMOVED***es()`.
 *
 * #### E***REMOVED***ample:
 *
 *     const { toDrop, toCreate } = await Model.diffInde***REMOVED***es();
 ...
 *
 * @param {Object} [options]
 * @return {Promise<Object>} contains the inde***REMOVED***es that would be dropped in MongoDB and inde***REMOVED***es that would be created in MongoDB as `{ toDrop: string[], toCreate: string[] }`.
 */

Model.diffInde***REMOVED***es = async function diffInde***REMOVED***es() {
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function') {
    throw new MongooseError('Model.syncInde***REMOVED***es() no longer accepts a callback');
  }
  ...
  let dbInde***REMOVED***es = await model.listInde***REMOVED***es().catch(err => {
    if (err.codeName == 'NamespaceNotFound') {
      return undefined;
    }
    throw err;
  });
  if (dbInde***REMOVED***es === undefined) {
    dbInde***REMOVED***es = [];
  }
  dbInde***REMOVED***es = getRelatedDBInde***REMOVED***es(model, dbInde***REMOVED***es);

  const schema = model.schema;
  const schemaInde***REMOVED***es = getRelatedSchemaInde***REMOVED***es(model, schema.inde***REMOVED***es());

  const toDrop = getInde***REMOVED***esToDrop(schema, schemaInde***REMOVED***es, dbInde***REMOVED***es);
  const toCreate = getInde***REMOVED***esToCreate(schema, schemaInde***REMOVED***es, dbInde***REMOVED***es, toDrop);

  return { toDrop, toCreate };
};

function getInde***REMOVED***esToCreate(schema, schemaInde***REMOVED***es, dbInde***REMOVED***es, toDrop) {
  const toCreate = [];

  for (const [schemaInde***REMOVED***KeysObject, schemaInde***REMOVED***Options] of schemaInde***REMOVED***es) {
    let found = false;

    const options = decorateDiscriminatorInde***REMOVED***Options(schema, clone(schemaInde***REMOVED***Options));

    for (const inde***REMOVED*** of dbInde***REMOVED***es) {
      if (isDefaultIdInde***REMOVED***(inde***REMOVED***)) {
        continue;
      }
      if (
        isInde***REMOVED***Equal(schemaInde***REMOVED***KeysObject, options, inde***REMOVED***) &&
        !toDrop.includes(inde***REMOVED***.name)
      ) {
        found = true;
        break;
      }
    ...
    if (!found) {
      toCreate.push(schemaInde***REMOVED***KeysObject);
    }
  }
...
function getInde***REMOVED***esToDrop(schema, schemaInde***REMOVED***es, dbInde***REMOVED***es) {
  const toDrop = [];

  for (const dbInde***REMOVED*** of dbInde***REMOVED***es) {
    let found = false;
    // Never try to drop `_id` inde***REMOVED***, MongoDB server doesn't allow it
    if (isDefaultIdInde***REMOVED***(dbInde***REMOVED***)) {
      continue;
    }

    for (const [schemaInde***REMOVED***KeysObject, schemaInde***REMOVED***Options] of schemaInde***REMOVED***es) {
      const options = decorateDiscriminatorInde***REMOVED***Options(schema, clone(schemaInde***REMOVED***Options));
      applySchemaCollation(schemaInde***REMOVED***KeysObject, options, schema.options);

      if (isInde***REMOVED***Equal(schemaInde***REMOVED***KeysObject, options, dbInde***REMOVED***)) {
        found = true;
        break;
      }
    }
    ...
    if (!found) {
      toDrop.push(dbInde***REMOVED***.name);
    }
  }
...
/**
 * Deletes all inde***REMOVED***es that aren't defined in this model's schema. Used by
 * `syncInde***REMOVED***es()`.
 *
 * The returned promise resolves to a list of the dropped inde***REMOVED***es' names as an array
 *
 * @param {Object} [options]
 * @param {Array<String>} [options.toDrop] if specified, contains a list of inde***REMOVED*** names to drop
 * @param {Boolean} [options.hideInde***REMOVED***es=false] set to `true` to hide inde***REMOVED***es instead of dropping. Requires MongoDB server 4.4 or higher
 * @return {Promise<String>} list of dropped or hidden inde***REMOVED*** names
 * @api public
 */

Model.cleanInde***REMOVED***es = async function cleanInde***REMOVED***es(options) {
  _checkConte***REMOVED***t(this, 'cleanInde***REMOVED***es');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function') {
    throw new MongooseError('Model.cleanInde***REMOVED***es() no longer accepts a callback');
  }
  const model = this;

  if (Array.isArray(options && options.toDrop)) {
    const res = await _dropInde***REMOVED***es(options.toDrop, model, options);
    return res;
  }

  const res = await model.diffInde***REMOVED***es();
  return await _dropInde***REMOVED***es(res.toDrop, model, options);
};

async function _dropInde***REMOVED***es(toDrop, model, options) {
  if (toDrop.length === 0) {
    return [];
  }

  const collection = model.$__collection;
  if (options && options.hideInde***REMOVED***es) {
    await Promise.all(toDrop.map(inde***REMOVED***Name => {
      return model.db.db.command({
        collMod: collection.collectionName,
        inde***REMOVED***: { name: inde***REMOVED***Name, hidden: true }
      });
    }));
  } else {
    await Promise.all(toDrop.map(inde***REMOVED***Name => collection.dropInde***REMOVED***(inde***REMOVED***Name)));
  }
 ...
 * use the [`autoInde***REMOVED***` option](https://mongoosejs.com/docs/guide.html#autoInde***REMOVED***) and if you
...
Model.listInde***REMOVED***es = async function listInde***REMOVED***es() {
  _checkConte***REMOVED***t(this, 'listInde***REMOVED***es');
  if (typeof arguments[0] === 'function') {
    throw new MongooseError('Model.listInde***REMOVED***es() no longer accepts a callback');
  }
  ...
  return this.$__collection.listInde***REMOVED***es().toArray();
};
 ...
 * #### E***REMOVED***ample:
 *
 *     await Event.ensureInde***REMOVED***es();
 *
 * After completion, an `inde***REMOVED***` event is emitted on this `Model` passing an error if one occurred.
 *
 * #### E***REMOVED***ample:
 *
 *     const eventSchema = new Schema({ thing: { type: 'string', unique: true } })
 *     const Event = mongoose.model('Event', eventSchema);
 *
 *     Event.on('inde***REMOVED***', function (err) {
 *       if (err) console.error(err); // error occurred during inde***REMOVED*** creation
 *     });
 *
 * _NOTE: It is not recommended that you run this in production. Inde***REMOVED*** creation may impact database performance depending on your load. Use with caution._
 *
...
Model.ensureInde***REMOVED***es = async function ensureInde***REMOVED***es(options) {
  _checkConte***REMOVED***t(this, 'ensureInde***REMOVED***es');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function') {
    throw new MongooseError('Model.ensureInde***REMOVED***es() no longer accepts a callback');
  }

  await new Promise((resolve, reject) => {
    _ensureInde***REMOVED***es(this, options, (err) => {
      if (err != null) {
        return reject(err);
      }
      resolve();
...
/**
 * Similar to `ensureInde***REMOVED***es()`, e***REMOVED***cept for it uses the [`createInde***REMOVED***`](https://mongodb.github.io/node-mongodb-native/4.9/classes/Db.html#createInde***REMOVED***)
 * function.
 *
 * @param {Object} [options] internal options
 * @return {Promise}
...
Model.createInde***REMOVED***es = async function createInde***REMOVED***es(options) {
  _checkConte***REMOVED***t(this, 'createInde***REMOVED***es');

  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function') {
    throw new MongooseError('Model.createInde***REMOVED***es() no longer accepts a callback');
  }

  return this.ensureInde***REMOVED***es(options);
};
...
function _ensureInde***REMOVED***es(model, options, callback) {
  const inde***REMOVED***es = model.schema.inde***REMOVED***es();
  let inde***REMOVED***Error;
  ...
  const done = function(err) {
    ...
    model.emit('inde***REMOVED***', err || inde***REMOVED***Error);
    callback && callback(err || inde***REMOVED***Error);
  };

  for (const inde***REMOVED*** of inde***REMOVED***es) {
    if (isDefaultIdInde***REMOVED***(inde***REMOVED***)) {
      utils.warn('mongoose: Cannot specify a custom inde***REMOVED*** on `_id` for ' +
        'model name "' + model.modelName + '", ' +
        'MongoDB does not allow overwriting the default `_id` inde***REMOVED***. See ' +
        'https://bit.ly/mongodb-id-inde***REMOVED***');
    }
  }

  if (!inde***REMOVED***es.length) {
    immediate(function() {
      done();
    });
    return;
  }
  // Inde***REMOVED***es are created one-by-one to support how MongoDB < 2.4 deals
  // with background inde***REMOVED***es.

  const inde***REMOVED***SingleDone = function(err, fields, options, name) {
    model.emit('inde***REMOVED***-single-done', err, fields, options, name);
  };
  const inde***REMOVED***SingleStart = function(fields, options) {
    model.emit('inde***REMOVED***-single-start', fields, options);
  };

  const baseSchema = model.schema._baseSchema;
  const baseSchemaInde***REMOVED***es = baseSchema ? baseSchema.inde***REMOVED***es() : [];
  ...
  function create() {
    if (options._automatic) {
      if (model.schema.options.autoInde***REMOVED*** === false ||
          (model.schema.options.autoInde***REMOVED*** == null && model.db.config.autoInde***REMOVED*** === false)) {
        return done();
      }
    }

    const inde***REMOVED*** = inde***REMOVED***es.shift();
    if (!inde***REMOVED***) {
      return done();
    }
    if (options._automatic && inde***REMOVED***[1]._autoInde***REMOVED*** === false) {
      return create();
    }

    if (baseSchemaInde***REMOVED***es.find(i => utils.deepEqual(i, inde***REMOVED***))) {
      return create();
    }

    const inde***REMOVED***Fields = clone(inde***REMOVED***[0]);
    const inde***REMOVED***Options = clone(inde***REMOVED***[1]);

    delete inde***REMOVED***Options._autoInde***REMOVED***;
    decorateDiscriminatorInde***REMOVED***Options(model.schema, inde***REMOVED***Options);
    applyWriteConcern(model.schema, inde***REMOVED***Options);
    applySchemaCollation(inde***REMOVED***Fields, inde***REMOVED***Options, model.schema.options);

    inde***REMOVED***SingleStart(inde***REMOVED***Fields, options);

    if ('background' in options) {
      inde***REMOVED***Options.background = options.background;
    }
    ...
    // Just in case `createInde***REMOVED***()` throws a sync error
    let promise = null;
    try {
      promise = model.collection.createInde***REMOVED***(inde***REMOVED***Fields, inde***REMOVED***Options);
    } catch (err) {
      if (!inde***REMOVED***Error) {
        inde***REMOVED***Error = err;
      }
      if (!model.$caught) {
        model.emit('error', err);
      }

      inde***REMOVED***SingleDone(err, inde***REMOVED***Fields, inde***REMOVED***Options);
      create();
      return;
    }
    ...
    promise.then(
      name => {
        inde***REMOVED***SingleDone(null, inde***REMOVED***Fields, inde***REMOVED***Options, name);
        create();
      },
      err => {
        if (!inde***REMOVED***Error) {
...
Model.translateAliases = function translateAliases(fields, errorOnDuplicates) {
  _checkConte***REMOVED***t(this, 'translateAliases');
...
Model.deleteOne = function deleteOne(conditions, options) {
  _checkConte***REMOVED***t(this, 'deleteOne');
 ...
 * #### E***REMOVED***ample:
 *
 *     await Character.deleteMany({ name: /Stark/, age: { $gte: 18 } }); // returns {deletedCount: ***REMOVED***} where ***REMOVED*** is the number of documents deleted.
 *
 * #### Note:
 *
...
Model.deleteMany = function deleteMany(conditions, options) {
  _checkConte***REMOVED***t(this, 'deleteMany');
 ...
 *     await MyModel.find({ name: 'john', age: { $gte: 18 } }).e***REMOVED***ec();
 *
 *     // e***REMOVED***ecutes, name LIKE john and only selecting the "name" and "friends" fields
 *     await MyModel.find({ name: /john/i }, 'name friends').e***REMOVED***ec();
 *
 *     // passing options
 *     await MyModel.find({ name: /john/i }, null, { skip: 10 }).e***REMOVED***ec();
 *
...
Model.find = function find(conditions, projection, options) {
  _checkConte***REMOVED***t(this, 'find');
 ...
 * - `findOne()`
 *
 * \* E***REMOVED***cept for how it treats `undefined`. If you use `findOne()`, you'll see
 ...
 *     await Adventure.findById(id).e***REMOVED***ec();
 *
 *     // select only the adventures name and length
 *     await Adventure.findById(id, 'name length').e***REMOVED***ec();
 *
...
Model.findById = function findById(id, projection, options) {
  _checkConte***REMOVED***t(this, 'findById');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function' || typeof arguments[2] === 'function') {
 ...
 *     await Adventure.findOne({ country: 'Croatia' }).e***REMOVED***ec();
 *
 *     // Model.findOne() no longer accepts a callback
 *
 *     // Select only the adventures name and length
 *     await Adventure.findOne({ country: 'Croatia' }, 'name length').e***REMOVED***ec();
 *
 * @param {Object} [conditions]
 * @param {Object|String|String[]} [projection] optional fields to return, see [`Query.prototype.select()`](https://mongoosejs.com/docs/api/query.html#Query.prototype.select())
 * @param {Object} [options] optional see [`Query.prototype.setOptions()`](https://mongoosejs.com/docs/api/query.html#Query.prototype.setOptions())
...
Model.findOne = function findOne(conditions, projection, options) {
  _checkConte***REMOVED***t(this, 'findOne');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function' || typeof arguments[2] === 'function') {
...
Model.estimatedDocumentCount = function estimatedDocumentCount(options) {
  _checkConte***REMOVED***t(this, 'estimatedDocumentCount');
 ...
 * instead. If you call `countDocuments({})`, MongoDB will always e***REMOVED***ecute
 * a full collection scan and **not** use any inde***REMOVED***es.
 *
 ...
 *
 * - `$where`: [`$e***REMOVED***pr`](https://www.mongodb.com/docs/manual/reference/operator/query/e***REMOVED***pr/)
...
Model.countDocuments = function countDocuments(conditions, options) {
  _checkConte***REMOVED***t(this, 'countDocuments');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function' || typeof arguments[2] === 'function') {
...
Model.distinct = function distinct(field, conditions, options) {
  _checkConte***REMOVED***t(this, 'distinct');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function' || typeof arguments[2] === 'function') {
 ...
 *
 * For e***REMOVED***ample, instead of writing:
 *
 *     User.find({ age: { $gte: 21, $lte: 65 } });
 *
 * we can instead write:
 *
 *     User.where('age').gte(21).lte(65).e***REMOVED***ec();
 *
...
Model.where = function where(path, val) {
  _checkConte***REMOVED***t(this, 'where');
 ...
 *
 * Sometimes you need to query for things in mongodb using a JavaScript e***REMOVED***pression. You can do so via `find({ $where: javascript })`, or you can use the mongoose shortcut method $where via a Query chain or from your mongoose Model.
 *
 *     Blog.$where('this.username.inde***REMOVED***Of("val") !== -1').e***REMOVED***ec(function (err, docs) {});
 *
 ...
 *
 * Finds a matching document, updates it according to the `update` arg, passing any `options`, and returns the found document (if any) to the callback. The query e***REMOVED***ecutes if `callback` is passed else a Query object is returned.
 *
 * #### E***REMOVED***ample:
 *
 ...
 * @param {Number} [options.ma***REMOVED***TimeMS] puts a time limit on the query - requires mongodb >= 2.6.0
...
Model.findOneAndUpdate = function(conditions, update, options) {
  _checkConte***REMOVED***t(this, 'findOneAndUpdate');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function' || typeof arguments[2] === 'function' || typeof arguments[3] === 'function') {
...
Model.findByIdAndUpdate = function(id, update, options) {
  _checkConte***REMOVED***t(this, 'findByIdAndUpdate');
  if (typeof arguments[0] === 'function' || typeof arguments[1] === 'function' || typeof arguments[2] === 'function' || typeof arguments[3] === 'function') {
...
Model.findOneAndDelete = function(conditions, options) {
  _checkConte***REMOVED***t(this, 'findOneAndDelete');
...
Model.findByIdAndDelete = function(id, options) {
  _checkConte***REMOVED***t(this, 'findByIdAndDelete');
...
Model.findOneAndReplace = function(filter, replacement, options) {
  _checkConte***REMOVED***t(this, 'findOneAndReplace');
...
Model.create = async function create(doc, options) {
  ...
  _checkConte***REMOVED***t(this, 'create');
  ...
  if (Array.isArray(doc)) {
    ...
    if (hasCallback) {
      ...
      args = [...arguments];
      // For backwards compatibility with 6.***REMOVED***, because of gh-5061 Mongoose 6.***REMOVED*** and
 ...
 *
 * - 'change': A change occurred, see below e***REMOVED***ample
...
Model.watch = function(pipeline, options) {
  _checkConte***REMOVED***t(this, 'watch');
 ...
 *     // secondary that is e***REMOVED***periencing replication lag.
...
Model.startSession = function() {
  _checkConte***REMOVED***t(this, 'startSession');
 ...
 * @param {Boolean} [options.lean=false] if `true`, skips hydrating the documents. This means Mongoose will **not** cast, validate, or apply defaults to any of the documents passed to `insertMany()`. This option is useful if you need the e***REMOVED***tra performance, but comes with data integrity risk. Consider using with [`castObject()`](https://mongoosejs.com/docs/api/model.html#Model.castObject()) and [`applyDefaults()`](https://mongoosejs.com/docs/api/model.html#Model.applyDefaults()).
...
Model.insertMany = async function insertMany(arr, options) {
  _checkConte***REMOVED***t(this, 'insertMany');
  if (typeof options === 'function' ||
    typeof arguments[2] === 'function') {
...
Model.$__insertMany = function(arr, options, callback) {
  ...
  const toE***REMOVED***ecute = arr.map((doc, inde***REMOVED***) =>
    callback => {
      // If option `lean` is set to true bypass validation and hydration
      if (lean) {
        // we have to e***REMOVED***ecute callback at the ne***REMOVED***tTick to be compatible
        // with parallelLimit, as `results` variable has TDZ issue if we
        // e***REMOVED***ecute the callback synchronously
        return immediate(() => callback(null, doc));
      }
      let createdNewDoc = false;
      if (!(doc instanceof _this)) {
        if (doc != null && typeof doc !== 'object') {
          return callback(new ObjectParameterError(doc, 'arr.' + inde***REMOVED***, 'insertMany'));
        }
        try {
          doc = new _this(doc);
          createdNewDoc = true;
      ...
      doc.$validate(createdNewDoc ? { _skipParallelValidateCheck: true } : null).then(
        ...
        error => {
          if (ordered === false) {
            ...
            validationErrorsToOriginalOrder.set(error, inde***REMOVED***);
            results[inde***REMOVED***] = error;
  ...
  parallelLimit(toE***REMOVED***ecute, limit, function(error, docs) {
    if (error) {
      callback(error, null);
      return;
    }

    const originalDocInde***REMOVED*** = new Map();
    const validDocInde***REMOVED***ToOriginalInde***REMOVED*** = new Map();
    for (let i = 0; i < docs.length; ++i) {
      originalDocInde***REMOVED***.set(docs[i], i);
    }
    ...
    for (let i = 0; i < docAttributes.length; ++i) {
      validDocInde***REMOVED***ToOriginalInde***REMOVED***.set(i, originalDocInde***REMOVED***.get(docAttributes[i]));
    }
    ...
    _this.$__collection.insertMany(docObjects, options).then(
      ...
      error => {
        ...
        const erroredInde***REMOVED***es = new Set((error && error.writeErrors || []).map(err => err.inde***REMOVED***));

        if (error.writeErrors != null) {
          for (let i = 0; i < error.writeErrors.length; ++i) {
            const originalInde***REMOVED*** = validDocInde***REMOVED***ToOriginalInde***REMOVED***.get(error.writeErrors[i].inde***REMOVED***);
            error.writeErrors[i] = {
              ...error.writeErrors[i],
              inde***REMOVED***: originalInde***REMOVED***
            };
            if (!ordered) {
              results[originalInde***REMOVED***] = error.writeErrors[i];
            }
          }
        }
        ...
        let firstErroredInde***REMOVED*** = -1;
        error.insertedDocs = docAttributes.
          filter((doc, i) => {
            const isErrored = !hasWriteErrors || erroredInde***REMOVED***es.has(i);

            if (ordered) {
              if (firstErroredInde***REMOVED*** > -1) {
                return i < firstErroredInde***REMOVED***;
              }

              if (isErrored) {
                firstErroredInde***REMOVED*** = i;
              }
            }
 ...
 * The only e***REMOVED***ception is [setting the `update` operator for `updateOne` or `updateMany` to a pipeline](https://www.mongodb.com/docs/manual/reference/method/db.collection.bulkWrite/#updateone-and-updatemany): Mongoose does **not** cast update pipelines.
 *
 ...
 * @param {Object} [options]
 * @param {Boolean} [options.ordered=true] If true, e***REMOVED***ecute writes in order and stop at the first error. If false, e***REMOVED***ecute writes in parallel and continue until all writes have either succeeded or errored.
...
Model.bulkWrite = async function bulkWrite(ops, options) {
  _checkConte***REMOVED***t(this, 'bulkWrite');
  ...
  const shouldSkip = await new Promise((resolve, reject) => {
    this.hooks.e***REMOVED***ecPre('bulkWrite', this, [ops, options], (err) => {
      if (err != null) {
        if (err instanceof Kareem.skipWrappedFunction) {
          return resolve(err);
        }
  ...
  if (ordered) {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        this.hooks.e***REMOVED***ecPost('bulkWrite', this, [null], _opts, (err) => {
          if (err != null) {
            return reject(err);
          }
          resolve();
      ...
      for (let i = 0; i < validations.length; ++i) {
        validations[i]((err) => {
          if (err == null) {
            ...
            validationErrors.push({ inde***REMOVED***: i, error: err });
            results[i] = err;
          }
          if (--remaining <= 0) {
            resolve();
    ...
    validationErrors = validationErrors.
      sort((v1, v2) => v1.inde***REMOVED*** - v2.inde***REMOVED***).
      map(v => v.error);

    const validOpInde***REMOVED***es = validOps;
    validOps = validOps.sort().map(inde***REMOVED*** => ops[inde***REMOVED***]);
    ...
    for (let i = 0; i < validOpInde***REMOVED***es.length; ++i) {
      results[validOpInde***REMOVED***es[i]] = null;
    }
    if (validationErrors.length > 0) {
  ...
  await new Promise((resolve, reject) => {
    this.hooks.e***REMOVED***ecPost('bulkWrite', this, [res], (err) => {
      if (err != null) {
        return reject(err);
      }
      resolve();
 ...
 * - `bulkWrite()` fails (for e***REMOVED***ample, due to being unable to connect to MongoDB or due to duplicate key error)
...
function buildPreSavePromise(document) {
  return new Promise((resolve, reject) => {
    document.schema.s.hooks.e***REMOVED***ecPre('save', document, (err) => {
      if (err) {
        reject(err);
        return;
      }
...
function handleSuccessfulWrite(document) {
  return new Promise((resolve, reject) => {
    ...
    document.$__reset();
    document.schema.s.hooks.e***REMOVED***ecPost('save', document, [document], {}, (err) => {
      if (err) {
        reject(err);
        return;
      }
...
Model.applyDefaults = function applyDefaults(doc) {
  ...
  if (doc.$__ != null) {
    applyDefaultsHelper(doc, doc.$__.fields, doc.$__.e***REMOVED***clude);

    for (const subdoc of doc.$getAllSubdocs()) {
      applyDefaults(subdoc, subdoc.$__.fields, subdoc.$__.e***REMOVED***clude);
    }
...
/**
 * Apply this model's virtuals to a given POJO. Virtuals e***REMOVED***ecute with the POJO as the conte***REMOVED***t `this`.
 *
 * #### E***REMOVED***ample:
 *
 *     const userSchema = new Schema({ name: String });
...
Model.castObject = function castObject(obj, options) {
  ...
  for (const path of paths) {
    ...
    const pieces = path.inde***REMOVED***Of('.') === -1 ? [path] : path.split('.');
    let cur = ret;
 ...
 * @param {Boolean} options.timestamps defaults to `null`, when set to false, mongoose will not add/update timestamps to the documents.
 * @return {Array<Promise>} Returns a array of all Promises the function e***REMOVED***ecutes to be awaited.
 * @api private
 */

Model.buildBulkWriteOperations = function buildBulkWriteOperations(documents, options) {
  if (!Array.isArray(documents)) {
    throw new Error(`bulkSave e***REMOVED***pects an array of documents to be passed, received \`${documents}\` instead`);
  }
...
Model.hydrate = function(obj, projection, options) {
  _checkConte***REMOVED***t(this, 'hydrate');
...
/**
 * Same as `updateOne()`, e***REMOVED***cept MongoDB will update _all_ documents that match
...
Model.updateMany = function updateMany(conditions, doc, options) {
  _checkConte***REMOVED***t(this, 'updateMany');
...
Model.updateOne = function updateOne(conditions, doc, options) {
  _checkConte***REMOVED***t(this, 'updateOne');

  return _update(this, 'updateOne', conditions, doc, options);
};

/**
 * Replace the e***REMOVED***isting document with the given document (no atomic operators like `$set`).
 *
 * #### E***REMOVED***ample:
 *
...
Model.replaceOne = function replaceOne(conditions, doc, options) {
  _checkConte***REMOVED***t(this, 'replaceOne');
 ...
 *
 * If a `callback` is passed, the `aggregate` is e***REMOVED***ecuted and a `Promise` is returned. If a callback is not passed, the `aggregate` itself is returned.
 *
 * This function triggers the following middleware.
 *
 * - `aggregate()`
 ...
 * #### E***REMOVED***ample:
 *
 *     // Find the ma***REMOVED*** balance of all accounts
 *     const res = await Users.aggregate([
 *       { $group: { _id: null, ma***REMOVED***Balance: { $ma***REMOVED***: '$balance' }}},
 *       { $project: { _id: 0, ma***REMOVED***Balance: 1 }}
 *     ]);
 *
 *     console.log(res); // [ { ma***REMOVED***Balance: 98000 } ]
 *
 *     // Or use the aggregation pipeline builder.
 *     const res = await Users.aggregate().
 *       group({ _id: null, ma***REMOVED***Balance: { $ma***REMOVED***: '$balance' } }).
 *       project('-id ma***REMOVED***Balance').
 *       e***REMOVED***ec();
 *     console.log(res); // [ { ma***REMOVED***Balance: 98 } ]
 *
 * #### Note:
 *
...
Model.aggregate = function aggregate(pipeline, options) {
  _checkConte***REMOVED***t(this, 'aggregate');
 ...
 * given `conte***REMOVED***t` to custom validators.
 *
 * #### E***REMOVED***ample:
 *
...
Model.validate = async function validate(obj, pathsOrOptions, conte***REMOVED***t) {
  if ((arguments.length < 3) || (arguments.length === 3 && typeof arguments[2] === 'function')) {
    // For convenience, if we're validating a document or an object, make `conte***REMOVED***t` default to
    // the model so users don't have to always pass `conte***REMOVED***t`, re: gh-10132, gh-10346
    conte***REMOVED***t = obj;
  }
  if (typeof conte***REMOVED***t === 'function' || typeof arguments[3] === 'function') {
    ...
    paths = paths.filter(p => {
      if (pathsOrOptions.pathsToSkip) {
        if (Array.isArray(pathsOrOptions.pathsToSkip)) {
          if (pathsOrOptions.pathsToSkip.find(***REMOVED*** => ***REMOVED*** == p)) {
            return false;
          }
 ...
 * @param {boolean} [options.getters=false] If true, Mongoose will call any getters defined on the `localField`. By default, Mongoose gets the raw value of `localField`. For e***REMOVED***ample, you would need to set this option to `true` if you wanted to [add a `lowercase` getter to your `localField`](https://mongoosejs.com/docs/schematypes.html#schematype-options).
 * @param {boolean} [options.clone=false] When you do `BlogPost.find().populate('author')`, blog posts with the same author will share 1 copy of an `author` doc. Enable this option to make Mongoose clone populated docs before assigning them.
 * @param {Object|Function} [options.match=null] Add an additional filter to the populate query. Can be a filter object containing [MongoDB query synta***REMOVED***](https://www.mongodb.com/docs/manual/tutorial/query-documents/), or a function that returns a filter object.
 ...
 * @param {Number} [options.perDocumentLimit=null] For legacy reasons, `limit` with `populate()` may give incorrect results because it only e***REMOVED***ecutes a single query for every document being populated. If you set `perDocumentLimit`, Mongoose will ensure correct `limit` per document by e***REMOVED***ecuting a separate query for each document to `populate()`. For e***REMOVED***ample, `.find().populate({ path: 'test', perDocumentLimit: 2 })` will e***REMOVED***ecute 2 additional queries if `.find()` returns 2 documents.
 ...
 * @param {Function} [callback(err,doc)] Optional callback, e***REMOVED***ecuted upon completion. Receives `err` and the `doc(s)`.
 * @return {Promise}
 * @api public
 */

Model.populate = async function populate(docs, paths) {
  _checkConte***REMOVED***t(this, 'populate');
 ...
 * @param {Function} [callback] Optional callback, e***REMOVED***ecuted upon completion. Receives `err` and the `doc(s)`.
 * @return {Function}
 * @api private
 */
...
function _populate(model, docs, paths, cache, callback) {
  ...
  // each path has its own query options and must be e***REMOVED***ecuted separately
  for (const path of paths) {
    populate(model, docs, path, ne***REMOVED***t);
  }

  function ne***REMOVED***t(err) {
    if (err) {
 ...
 * Populates `docs`
 */
const e***REMOVED***cludeIdReg = /\s?-_id\s?/;
const e***REMOVED***cludeIdRegGlobal = /\s?-_id\s?/g;
...
function populate(model, docs, options, callback) {
  ...
  for (let i = 0; i < len; ++i) {
    ...
    assignmentOpts.e***REMOVED***cludeId = e***REMOVED***cludeIdReg.test(select) || (select && select._id === 0);
    ...
    if (ids.length === 0 || ids.every(utils.isNullOrUndefined)) {
      ...
      // if we don't actually e***REMOVED***ecute a query to make sure there's a value
    ...
    if (assignmentOpts.e***REMOVED***cludeId) {
      // override the e***REMOVED***clusion from the query so we can use the _id
      // for document matching during assignment. we'll delete the
      // _id back off before returning the result.
      if (typeof select === 'string') {
        select = select.replace(e***REMOVED***cludeIdRegGlobal, ' ');
    ...
    params.push([mod, match, select, assignmentOpts, _ne***REMOVED***t]);
  }
  if (!hasOne) {
  ...
  for (const arr of params) {
    _e***REMOVED***ecPopulateQuery.apply(null, arr);
  }
  function _ne***REMOVED***t(err, valsFromDb) {
    if (err != null) {
      return callback(err, null);
    }
...
function _e***REMOVED***ecPopulateQuery(mod, match, select, assignmentOpts, callback) {
  ...
  // If projection is e***REMOVED***clusive and client e***REMOVED***plicitly unselected the foreign
  ...
  if (subPopulate) {
    // If subpopulating on a discriminator, skip check for non-e***REMOVED***istent
 ...
 * Compiler utility.
 *
 * @param {String|Function} name model name or class e***REMOVED***tending Model
 * @param {Schema} schema
...
Model.compile = function compile(name, schema, collectionName, connection, base) {
  ...
  if (typeof name === 'function' && name.prototype instanceof Model) {
    ...
    model = function model(doc, fields, skipId) {
      model.hooks.e***REMOVED***ecPreSync('createModel', doc);
 ...
 * Not for e***REMOVED***ternal use, only used by `setDriver()` to ensure that you can still
...
Model._applyQueryMiddleware = function _applyQueryMiddleware() {
  ...
  const queryMiddleware = this.schema.s.hooks.filter(hook => {
    const conte***REMOVED***ts = _getConte***REMOVED***ts(hook);
    if (hook.name === 'validate') {
      return !!conte***REMOVED***ts.query;
    }
    if (hook.name === 'deleteOne' || hook.name === 'updateOne') {
      return !!conte***REMOVED***ts.query || Object.keys(conte***REMOVED***ts).length === 0;
    }
...
function _getConte***REMOVED***ts(hook) {
  const ret = {};
...
module.e***REMOVED***ports = e***REMOVED***ports = Model;
