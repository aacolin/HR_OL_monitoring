...
SuperAgent is light-weight progressive aja***REMOVED*** API crafted for fle***REMOVED***ibility, readability, and a low learning curve after being frustrated with many of the e***REMOVED***isting request APIs. It also works with Node.js!
...
A request can be initiated by invoking the appropriate method on the `request` object, then calling `.then()` (or `.end()` [or `await`](#promise-and-generator-support)) to send the request. For e***REMOVED***ample a simple __GET__ request:
...
Absolute URLs can be used. In web browsers absolute URLs work only if the server implements [CORS](#cors).
     ...
     request
       .get('https://e***REMOVED***ample.com/search')
       .then(res => {

       });

The __Node__ client supports making requests to [Uni***REMOVED*** Domain Sockets](https://en.wikipedia.org/wiki/Uni***REMOVED***_domain_socket):

    // pattern: https?+uni***REMOVED***://SOCKET_PATH/REQUEST_PATH
    //          Use `%2F` as `/` in SOCKET_PATH
    try {
      const res = await request
        .get('http+uni***REMOVED***://%2Fabsolute%2Fpath%2Fto%2Funi***REMOVED***.sock/search');
      // res.body, res.headers, res.status
    } catch(err) {
      // err.message, err.response
    }
...
Since JSON is undoubtedly the most common, it's the _default_! The following e***REMOVED***ample is equivalent to the previous.
...
By default sending strings will set the `Content-Type` to `application/***REMOVED***-www-form-urlencoded`,
...
SuperAgent formats are e***REMOVED***tensible, however by default "json" and "form" are supported. To send the data as `application/***REMOVED***-www-form-urlencoded` simply invoke `.type()` with "form", where the default is "json". This request will __POST__ the body "name=tj&pet=tobi".
...
Sending a [`FormData`](https://developer.mozilla.org/en-US/docs/Web/API/FormData/FormData) object is also supported. The following e***REMOVED***ample will __POST__ the content of the HTML form identified by id="myForm":
...
simply the e***REMOVED***tension name such as "***REMOVED***ml", "json", "png", etc:
...
```js
request.serialize['application/***REMOVED***ml'] = function (obj) {
    return 'string generated from obj';
};

// going forward, all requests with a Content-type of
// 'application/***REMOVED***ml' will be automatically serialized
```
...
This method has two optional arguments: number of retries (default 1) and a callback. It calls `callback(err, res)` before each retry. The callback may return `true`/`false` to control whether the request should be retried (but the ma***REMOVED***imum number of retries is always applied).
...
In a similar fashion to the `.type()` method it is also possible to set the `Accept` header via the short hand method `.accept()`. Which references `request.types` as well allowing you to specify either the full canonicalized MIME type name as `type/subtype`, or the e***REMOVED***tension suffi***REMOVED*** form as "***REMOVED***ml", "json", "png", etc. for convenience:
...
- `.pf***REMOVED***()`: Set the client PFX or PKCS12 encoded private key and certificate chain
- `.disableTLSCerts()`: Does not reject e***REMOVED***pired or invalid TLS certs. Sets internally `rejectUnauthorized=true`. *Be warned, this method allows MITM attacks.*
...
currently supporting `application/***REMOVED***-www-form-urlencoded`,
`application/json`, and `multipart/form-data`. You can setup
automatic parsing for other response-body data as well:

```js
//browser
request.parse['application/***REMOVED***ml'] = function (str) {
    return {'object': 'parsed from str'};
};

//node
request.parse['application/***REMOVED***ml'] = function (res, cb) {
    //parse response te***REMOVED***t and set res.body here

    cb(null, res);
};

//going forward, responses of type 'application/***REMOVED***ml'
...
The property `res.body` is the parsed object, for e***REMOVED***ample if a request responded with the JSON string '{"user":{"name":"tobi"}}', `res.body.user.name` would be "tobi". Likewise the ***REMOVED***-www-form-urlencoded value of "user[name]=tobi" would yield the same result. Only one level of nesting is supported. If you need more comple***REMOVED*** data, send JSON instead.
...
The Node client supports _multipart/form-data_ via the [Formidable](https://github.com/feli***REMOVED***ge/node-formidable) module. When parsing multipart responses, the object `res.files` is also available to you. Suppose for e***REMOVED***ample a request responds with the following multipart body:
    ...
    Content-Type: te***REMOVED***t/plain
...
For more information, see the Mozilla Developer Network [***REMOVED***hr.responseType docs](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/responseType).

## Response properties

Many helpful flags and properties are set on the `Response` object, ranging from the response te***REMOVED***t, parsed response body, header fields, status flags and more.

### Response te***REMOVED***t

The `res.te***REMOVED***t` property contains the unparsed response body string. This property is always present for the client API, and only when the mime type matches "te***REMOVED***t/*", "*/json", or "***REMOVED***-www-form-urlencoded" by default for node. The reasoning is to conserve memory, as buffering te***REMOVED***t of large bodies such as multipart files or images is e***REMOVED***tremely inefficient. To force buffering see the "Buffering responses" section.

### Response body

Much like SuperAgent can auto-serialize request data, it can also automatically parse it. When a parser is defined for the Content-Type, it is parsed, which by default includes "application/json" and "application/***REMOVED***-www-form-urlencoded". The parsed object is then available via `res.body`.

### Response header fields

The `res.header` contains an object of parsed header fields, lowercasing field names much like node does. For e***REMOVED***ample `res.header['content-length']`.

### Response Content-Type

The Content-Type response header is special-cased, providing `res.type`, which is void of the charset (if any). For e***REMOVED***ample the Content-Type of "te***REMOVED***t/html; charset=utf8" will provide "te***REMOVED***t/html" as `res.type`, and the `res.charset` property would then contain "utf8".
...
Sometimes networks and servers get "stuck" and never respond after accepting a request. Set timeouts to avoid requests waiting forever.
  ...
  * `req.timeout({response:ms})` sets ma***REMOVED***imum time to wait for the first byte to arrive from the server, but it does not limit how long the entire download can take. Response timeout should be at least few seconds longer than just the time it takes the server to respond, because it also includes time to make DNS lookup, TCP/IP and TLS connections, and time to upload request data.
...
Redirects e***REMOVED***ceeding the limit are treated as errors. Use `.ok(res => res.status < 400)` to read them as successful responses.
...
The complete list of methods that the agent can use to set defaults is: `use`, `on`, `once`, `set`, `query`, `type`, `accept`, `auth`, `withCredentials`, `sortQuery`, `retry`, `ok`, `redirects`, `timeout`, `buffer`, `serialize`, `parse`, `ca`, `key`, `pf***REMOVED***`, `cert`.
...
For e***REMOVED***ample piping a file's contents as the request:
...
Or piping the response to a file:
 ...
 It's not possible to mi***REMOVED*** pipes and callbacks or promises. Note that you should **NOT** attempt to pipe the result of `.end()` or the `Response` object:

    // Don't do either of these:
    const stream = getAWritableStream();
    const req = request
      .get('/some.json')
      // BAD: this pipes garbage to the stream and fails in une***REMOVED***pected ways
...
<br>
    ...
    request
      .post('/upload')
      .attach('image1', 'path/to/feli***REMOVED***.jpeg')
...
To force buffering of response bodies as `res.te***REMOVED***t` you may invoke `req.buffer()`. To undo the default of buffering for te***REMOVED***t responses such as "te***REMOVED***t/plain", "te***REMOVED***t/html" etc you may invoke `req.buffer(false)`.
...
For security reasons, browsers will block cross-origin requests unless the server opts-in using CORS headers. Browsers will also make e***REMOVED***tra __OPTIONS__ requests to check what HTTP headers and methods are allowed by the server. [Read more about CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/Access_control_CORS).
...
The `.withCredentials()` method enables the ability to send cookies from the origin, however only when `Access-Control-Allow-Origin` is _not_ a wildcard ("*"), and `Access-Control-Allow-Credentials` is "true".
    ...
    request
      .get('https://api.e***REMOVED***ample.com:4001/')
      .withCredentials()
      .then(res => {
        assert.equal(200, res.status);
        assert.equal('tobi', res.te***REMOVED***t);
      })
...
In Node.js it's possible to ignore DNS resolution and direct all requests to a specific IP address using `.connect()` method. For e***REMOVED***ample, this request will go to localhost instead of `e***REMOVED***ample.com`:

    const res = await request.get("http://e***REMOVED***ample.com").connect("127.0.0.1");
...
Because the request may be redirected, it's possible to specify multiple hostnames and multiple IPs, as well as a special `*` as the fallback (note: other wildcards are not supported). The requests will keep their `Host` header with the original value. `.connect(undefined)` turns off the feature.
    ...
    const res = await request.get("http://redir.e***REMOVED***ample.com:555")
      .connect({
        "redir.e***REMOVED***ample.com": "127.0.0.1", // redir.e***REMOVED***ample.com:555 will use 127.0.0.1:555
        "www.e***REMOVED***ample.com": false, // don't override this one; use DNS as normal
        "mapped.e***REMOVED***ample.com": { host: "127.0.0.1", port: 8080}, // mapped.e***REMOVED***ample.com:* will use 127.0.0.1:8080
        "*": "pro***REMOVED***y.e***REMOVED***ample.com", // all other requests will go to this host
      });
...
It's generally safe to ignore broken HTTPS on `localhost`, because the loopback interface is not e***REMOVED***posed to untrusted networks. Trusting `localhost` may become the default in the future. Use `.trustLocalhost(false)` to force check of `127.0.0.1`'s authenticity.

We intentionally don't support disabling of HTTPS security when making requests to any other IP, because such options end up abused as a quick "fi***REMOVED***" for HTTPS problems. You can get free HTTPS certificates from [Let's Encrypt](https://certbot.eff.org) or set your own CA (`.ca(ca_public_pem)`) to make your self-signed certificates trusted.

## Promise and Generator support

SuperAgent's request is a "thenable" object that's compatible with JavaScript promises and the `async`/`await` synta***REMOVED***.
...
Note that SuperAgent e***REMOVED***pects the global `Promise` object to be present. You'll need a polyfill to use promises in Internet E***REMOVED***plorer or Node.js 0.10.
