...
test('bar', function (t) {
    ...
    resolve('foo', { basedir: dir + '/bar' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar/node_modules/foo/inde***REMOVED***.js'));
...
test('baz', function (t) {
    ...
    resolve('./baz', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'baz/quu***REMOVED***.js'));
        t.equal(pkg.main, 'quu***REMOVED***.js');
    });
...
test('biz', function (t) {
    ...
    resolve('./gru***REMOVED***', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'gru***REMOVED***/inde***REMOVED***.js'));
        t.equal(pkg, undefined);
    });

    resolve('./gru***REMOVED***', { basedir: dir, 'package': { main: 'biz' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'gru***REMOVED***/inde***REMOVED***.js'));
    ...
    resolve('./garply', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'garply/lib/inde***REMOVED***.js'));
    ...
    resolve('tiv', { basedir: dir + '/gru***REMOVED***' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'tiv/inde***REMOVED***.js'));
        t.equal(pkg, undefined);
    });

    resolve('tiv', { basedir: dir + '/gru***REMOVED***', 'package': { main: 'gru***REMOVED***' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'tiv/inde***REMOVED***.js'));
        t.equal(pkg.main, 'gru***REMOVED***');
    });
    ...
    resolve('gru***REMOVED***', { basedir: dir + '/tiv' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'gru***REMOVED***/inde***REMOVED***.js'));
        t.equal(pkg, undefined);
    });

    resolve('gru***REMOVED***', { basedir: dir + '/tiv', 'package': { main: 'tiv' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'gru***REMOVED***/inde***REMOVED***.js'));
...
test('quu***REMOVED***', function (t) {
    t.plan(2);
    var dir = path.join(__dirname, 'resolver/quu***REMOVED***');

    resolve('./foo', { basedir: dir, 'package': { main: 'quu***REMOVED***' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'foo/inde***REMOVED***.js'));
        t.equal(pkg.main, 'quu***REMOVED***');
    });
});

test('normalize', function (t) {
    t.plan(2);
    var dir = path.join(__dirname, 'resolver/biz/node_modules/gru***REMOVED***');

    resolve('../gru***REMOVED***', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'inde***REMOVED***.js'));
        t.equal(pkg, undefined);
    });
});
...
test('cup', function (t) {
    ...
    resolve('./cup', { basedir: dir, e***REMOVED***tensions: ['.js', '.coffee'] }, function (err, res) {
        if (err) t.fail(err);
    ...
    resolve('./cup', { basedir: dir, e***REMOVED***tensions: ['.js'] }, function (err, res) {
        t.equal(err.message, "Cannot find module './cup' from '" + path.resolve(dir) + "'");
        t.equal(err.code, 'MODULE_NOT_FOUND');
    });

    // Test that filename is reported as the "from" value when passed.
    resolve('./cup', { basedir: dir, e***REMOVED***tensions: ['.js'], filename: path.join(dir, 'cupboard.js') }, function (err, res) {
...
test('mug', function (t) {
    ...
    resolve('./mug', { basedir: dir, e***REMOVED***tensions: ['.coffee', '.js'] }, function (err, res) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, '/mug.coffee'));
    });

    resolve('./mug', { basedir: dir, e***REMOVED***tensions: ['.js', '.coffee'] }, function (err, res) {
        t.equal(res, path.join(dir, '/mug.js'));
    });
});
...
test('path iterator', function (t) {
    ...
    var e***REMOVED***actIterator = function (***REMOVED***, start, getPackageCandidates, opts) {
        return [path.join(resolverDir, ***REMOVED***)];
    };

    resolve('baz', { packageIterator: e***REMOVED***actIterator }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(resolverDir, 'baz/quu***REMOVED***.js'));
...
test('missing inde***REMOVED***', function (t) {
    t.plan(2);

    var resolverDir = path.join(__dirname, 'resolver');
    resolve('./missing_inde***REMOVED***', { basedir: resolverDir }, function (err, res, pkg) {
...
test('main: false', function (t) {
    ...
    resolve('./false_main', { basedir: basedir }, function (err, res, pkg) {
        ...
        t.equal(
            res,
            path.join(dir, 'inde***REMOVED***.js'),
            '`"main": false`: resolves to `inde***REMOVED***.js`'
        );
        t.deepEqual(pkg, {
            name: 'false_main',
            main: false
...
test('#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name', function (t) {
    ...
    resolve('./foo/', { basedir: path.join(dir, 'same_names') }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'same_names/foo/inde***REMOVED***.js'));
    });
});
...
test('async: #121 - treating an e***REMOVED***isting file as a dir when no basedir', function (t) {
    ...
    t.test('with a fake directory', function (st) {
        ...
        resolve('./' + testFile + '/blah', function (err, res, pkg) {
            ...
            st.equal(
                ...
                'can not find none***REMOVED***istent module'
            );
            st.end();
        });
    });
...
test('async dot main', function (t) {
    ...
    resolve('./resolver/dot_main', function (err, ret) {
        t.notOk(err);
        t.equal(ret, path.join(__dirname, 'resolver/dot_main/inde***REMOVED***.js'));
...
test('async dot slash main', function (t) {
    ...
    resolve('./resolver/dot_slash_main', function (err, ret) {
        t.notOk(err);
        t.equal(ret, path.join(__dirname, 'resolver/dot_slash_main/inde***REMOVED***.js'));
        t.ok(new Date() - start < 50, 'resolve.sync timedout');
        t.end();
    });
});
...
test('absolute paths', function (t) {
    ...
    var e***REMOVED***tensionless = __filename.slice(0, -path.e***REMOVED***tname(__filename).length);
    ...
    resolve(e***REMOVED***tensionless, function (err, res) {
        t.equal(
            res,
            __filename,
            'e***REMOVED***tensionless absolute path to this file resolves'
        );
    });
    ...
    resolve(e***REMOVED***tensionless, { basedir: process.cwd() }, function (err, res) {
        t.equal(
            res,
            __filename,
            'e***REMOVED***tensionless absolute path to this file with a basedir resolves'
        );
    });
});

var malformedDir = path.join(__dirname, 'resolver/malformed_package_json');
test('malformed package.json', { skip: !fs.e***REMOVED***istsSync(malformedDir) }, function (t) {
    ...
    var e***REMOVED***pected = path.join(basedir, 'inde***REMOVED***.js');

    resolve('./inde***REMOVED***.js', { basedir: basedir }, function (err, res, pkg) {
        t.error(err, 'no error');
        t.equal(res, e***REMOVED***pected, 'malformed package.json is silently ignored');
    ...
    resolve(
        ...
        function (err, res, pkg) {
            ...
            t.equal(res, e***REMOVED***pected, 'with packageFilter: malformed package.json is silently ignored');
    ...
    resolve(
            ...
            readPackage: function (readFile, pkgfile, cb) {
                ...
                readFile(pkgfile, function (err, result) {
                    try {
                        ...
                        t.ok(e instanceof Synta***REMOVED***Error, 'readPackage: malformed package.json parses as a synta***REMOVED*** error');
                        cb(null);
                    }
                });
            }
        ...
        function (err, res, pkg) {
            ...
            t.equal(res, e***REMOVED***pected, 'with readPackage: malformed package.json is silently ignored');
