...
Encode a URL to a percent-encoded form, e***REMOVED***cluding already-encoded sequences.
...
It is e***REMOVED***pected that any output from `new URL(url)` will not change when used with this package, as the output has already been encoded. Additionally, if we were to encode before `new URL(url)`, we do not e***REMOVED***pect the before and after encoded formats to be parsed any differently.
...
- [RFC 3986: Uniform Resource Identifier (URI): Generic Synta***REMOVED***][rfc-3986]
