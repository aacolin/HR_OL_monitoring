...
nodemon is a utility for node, and replaces the use of the e***REMOVED***ecutable
...
`nodemon` - if no package, looks for inde***REMOVED***.js
 ...
 * nodemon options, the user script and the e***REMOVED***ecutable script.
 *
...
function parse(argv) {
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (!script) {
      if (args[i] === '.' || e***REMOVED***istsSync(args[i])) {
        ...
        // arguments to the e***REMOVED***ec process for now).
    ...
    if (lookForArgs) {
      ...
      if (args[i] === '--') {
        ...
        // move to the ne***REMOVED***t iteration
        continue;
      }
 ...
 * @param {Function} the callback to eat up the ne***REMOVED***t argument in argv
 * @return {Boolean} false if argument was not a nodemon arg
 */
function nodemonOption(options, arg, eatNe***REMOVED***t) {
  // line separation on purpose to help legibility
  if (arg === '--help' || arg === '-h' || arg === '-?') {
    var help = eatNe***REMOVED***t();
  ...
  if (arg === '--polling-interval' || arg === '-P') {
    options.pollingInterval = parseInt(eatNe***REMOVED***t(), 10);
  } else
  ...
  if (arg === '--config') {
    options.configFile = eatNe***REMOVED***t();
  } else

  if (arg === '--watch' || arg === '-w') {
    if (!options.watch) { options.watch = []; }
    options.watch.push(eatNe***REMOVED***t());
  } else

  if (arg === '--ignore' || arg === '-i') {
    if (!options.ignore) { options.ignore = []; }
    options.ignore.push(eatNe***REMOVED***t());
  } else

  if (arg === '--e***REMOVED***itcrash') {
    options.e***REMOVED***itcrash = true;
  } else

  if (arg === '--delay' || arg === '-d') {
    options.delay = parseDelay(eatNe***REMOVED***t());
  } else

  if (arg === '--e***REMOVED***ec' || arg === '-***REMOVED***') {
    options.e***REMOVED***ec = eatNe***REMOVED***t();
  } else
  ...
  if (arg === '--e***REMOVED***t' || arg === '-e') {
    options.e***REMOVED***t = eatNe***REMOVED***t();
  } else
  ...
  if (arg === '--signal' || arg === '-s') {
    options.signal = eatNe***REMOVED***t();
  } else

  if (arg === '--cwd') {
    options.cwd = eatNe***REMOVED***t();
...
function parseDelay(value) {
  ...
  if (value.match(/^\d*ms$/)) {
    // E***REMOVED***plicitly parse for milliseconds when using ms time specifier
