...
function nodemon(settings) {
  ...
  if (typeof settings === 'string') {
    ...
    if (settings.inde***REMOVED***Of('node') !== 0) {
      if (settings.inde***REMOVED***Of('nodemon') !== 0) {
  ...
  config.load(settings, function (config) {
    if (!config.options.dump && !config.options.e***REMOVED***ecOptions.script &&
      config.options.e***REMOVED***ecOptions.e***REMOVED***ec === 'node') {
      if (!config.required) {
        console.log(help('usage'));
        process.e***REMOVED***it();
      }
      return;
    }
    ...
    if (config.options.stdin && config.options.restartable) {
      ...
      // special keys, like ctrl+c ***REMOVED*** 2 or '.e***REMOVED***it' or ctrl+d or ctrl+l
      var ctrlC = false;
      var buffer = '';
      ...
      process.stdin.on('data', function (data) {
        ...
        if (chr === 3) {
          ...
          ctrlC = true;
          return;
        } else if (buffer === '.e***REMOVED***it' || chr === 4) { // ctrl+d
          process.e***REMOVED***it();
    ...
    const ignoring = config.options.monitor.map(function (rule) {
      ...
      if (defaults.ignoreRoot.inde***REMOVED***Of(rule) !== -1) {
        return false;
    ...
    utils.log.info('watching e***REMOVED***tensions: ' + (config.options.e***REMOVED***ecOptions.e***REMOVED***t || '(all)'));
...
nodemon.once = function (event, handler) {
  ...
  bus.once(event, function () {
    ...
    eventHandlers[event].splice(eventHandlers[event].inde***REMOVED***Of(handler), 1);
...
// e***REMOVED***pose the full config
nodemon.config = config;

module.e***REMOVED***ports = nodemon;
