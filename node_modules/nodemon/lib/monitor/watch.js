module.e***REMOVED***ports.watch = watch;
module.e***REMOVED***ports.resetWatchers = resetWatchers;
...
function watch() {
  if (watchers.length) {
    debug('early e***REMOVED***it on watch, still watching (%s)', watchers.length);
    return;
  }
  ...
  const promise = new Promise(function (resolve) {
    ...
    // don't ignore dotfiles if e***REMOVED***plicitly watched.
    ...
    var watchOptions = {
      ...
      // to the props and in some cases it fi***REMOVED***es bugs but typically it causes
...
function filterAndRestart(files) {
  ...
  if (files.length) {
    ...
    if (utils.isWindows) {
      ...
      files = files.map(f => {
        if (f.inde***REMOVED***Of(':') === -1) { return f; }
    ...
    var matched = match(
      ...
      undefsafe(config, 'options.e***REMOVED***ecOptions.e***REMOVED***t')
    );
    ...
    if (config.options.e***REMOVED***ecOptions && config.options.e***REMOVED***ecOptions.script) {
      const script = path.resolve(config.options.e***REMOVED***ecOptions.script);
...
function debounce(fn, delay) {
  var timer = null;
  return function () {
    const conte***REMOVED***t = this;
    const args = arguments;
    clearTimeout(timer);
    timer = setTimeout(() =>fn.apply(conte***REMOVED***t, args), delay);
  };
}
