...
const declaresArguments = (node: t.Node) =>
  t.isFunction(node) && !t.isArrowFunctionE***REMOVED***pression(node);
...
const secondPass = walk<{
  globals: (t.Identifier | t.ThisE***REMOVED***pression)[];
}>({
  ...
  ThisE***REMOVED***pression(node, state, parents) {
    for (const parent of parents) {
...
e***REMOVED***port default function findGlobals(ast: t.Node) {
  const globals: (t.Identifier | t.ThisE***REMOVED***pression)[] = [];
  ...
  const groupedGlobals = new Map<string, (t.Identifier | t.ThisE***REMOVED***pression)[]>();
  for (const node of globals) {
    const name: string = node.type === 'ThisE***REMOVED***pression' ? 'this' : node.name;
    const e***REMOVED***isting = groupedGlobals.get(name);
    if (e***REMOVED***isting) {
      e***REMOVED***isting.push(node);
    } else {
