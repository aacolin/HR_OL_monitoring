...
module.e***REMOVED***ports = favicon
...
function favicon (path, options) {
  ...
  var ma***REMOVED***Age = calcMa***REMOVED***Age(opts.ma***REMOVED***Age)
  ...
  if (Buffer.isBuffer(path)) {
    icon = createIcon(Buffer.from(path), ma***REMOVED***Age)
  ...
  return function favicon (req, res, ne***REMOVED***t) {
    ...
    fs.readFile(path, function (err, buf) {
      ...
      icon = createIcon(buf, ma***REMOVED***Age)
      send(req, res, icon)
    })
  }
}
...
/**
 * Calculate the ma***REMOVED***-age from a configured value.
 *
 * @private
...
function calcMa***REMOVED***Age (val) {
  var num = typeof val === 'string'
    ? ms(val)
    : val

  return num != null
    ? Math.min(Math.ma***REMOVED***(0, num), ONE_YEAR_MS)
    : ONE_YEAR_MS
}

/**
 * Create icon data from Buffer and ma***REMOVED***-age.
 *
 * @private
 * @param {Buffer} buf
...
function createIcon (buf, ma***REMOVED***Age) {
  return {
    body: buf,
    headers: {
      'Cache-Control': 'public, ma***REMOVED***-age=' + Math.floor(ma***REMOVED***Age / 1000),
      'ETag': etag(buf)
    }
  }
}
...
function send (req, res, icon) {
  ...
  res.setHeader('Content-Type', 'image/***REMOVED***-icon')
  res.end(icon.body)
}
