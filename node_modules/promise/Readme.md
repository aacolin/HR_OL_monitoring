...
This is a simple implementation of Promises.  It is a super set of ES6 Promises designed to have readable, performant code and to provide just the e***REMOVED***tensions that are absolutely necessary for using promises today.

For detailed tutorials on its use, see www.promisejs.org

**N.B.** This promise e***REMOVED***poses internals via underscore (`_`) prefi***REMOVED***ed properties.  If you use these, your code will break with each new release.
...
```html
<script src="https://cdnjs.cloudflare.com/aja***REMOVED***/libs/es5-shim/3.4.0/es5-shim.min.js"></script>
```

## Usage

The e***REMOVED***ample below shows how you can load the promise library (in a way that works on both client and server using node or browserify).  It then demonstrates creating a promise from scratch.  You simply call `new Promise(fn)`.  There is a complete specification for what is returned by this method in [Promises/A+](http://promises-aplus.github.com/promises-spec/).
...
```js
var Promise = require('promise/lib/es6-e***REMOVED***tensions');
// or require('promise/domains/es6-e***REMOVED***tensions');
// or require('promise/setimmediate/es6-e***REMOVED***tensions');
```
...
You can enable logging of all unhandled rejections if you need to debug an e***REMOVED***ception you think is being swallowed by promises:
...
`rejection-tracking.enable(options)` takes the following options:
 ...
 - allRejections (`boolean`) - track all e***REMOVED***ceptions, not just reference errors and type errors.  Note that this has a high probability of resulting in false positives if your code loads data optimisticly
...
Before all e***REMOVED***amples, you will need:
...
The twin to `denodeify` is useful when you want to e***REMOVED***port an API that can be used by people who haven't learnt about the brilliance of promises yet.

```javascript
module.e***REMOVED***ports = Promise.nodeify(awesomeAPI)
function awesomeAPI(a, b) {
  return download(a, b)
}
```

If the last argument passed to `module.e***REMOVED***ports` is a function, then it will be treated like a node.js callback and not parsed on to the child function, otherwise the API will just return a promise.
...
This method follows the [Promises/A+ spec](http://promises-aplus.github.io/promises-spec/).  It e***REMOVED***plains things very clearly so I recommend you read it.

Either `onFulfilled` or `onRejected` will be called and they will not be called more than once.  They will be passed a single argument and will always be called asynchronously (in the ne***REMOVED***t turn of the event loop).

If the promise is fulfilled then `onFulfilled` is called.  If the promise is rejected then `onRejected` is called.

The call to `.then` also returns a promise.  If the handler that is called returns a promise, the promise returned by `.then` takes on the state of that returned promise.  If the handler that is called returns a value that is not a promise, the promise returned by `.then` will be fulfilled with that value. If the handler that is called throws an e***REMOVED***ception then the promise returned by `.then` is rejected with that e***REMOVED***ception.
...
The same semantics as `.then` e***REMOVED***cept that it does not return a promise and any e***REMOVED***ceptions are re-thrown so that they can be logged (crashing the application in non-browser environments)
...
People who use typical node.js style callbacks will be able to just pass a callback and get the e***REMOVED***pected behavior.  The enlightened people can not pass a callback and will get awesome promises.
