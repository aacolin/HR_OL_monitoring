...
- separated collection implementations for ma***REMOVED***imum fle***REMOVED***ibility
...
`mquery` requires a collection object to work with. In the e***REMOVED***ample above we just pass the collection object created using the official [MongoDB driver](https://github.com/mongodb/node-mongodb-native).
...
- [mquery](#mquery)
  ...
  - [Helpers](#helpers)
    ...
    - [distinct()](#distinct)
    - [e***REMOVED***ec()](#e***REMOVED***ec)
    - [stream()](#stream)
    - [all()](#all)
    - [and()](#and)
    - [bo***REMOVED***()](#bo***REMOVED***)
    - [circle()](#circle)
    - [elemMatch()](#elemmatch)
    - [equals()](#equals)
    ...
    - [lt()](#lt)
    - [lte()](#lte)
    - [ma***REMOVED***Distance()](#ma***REMOVED***distance)
    - [mod()](#mod)
    - [ne()](#ne)
    - [nin()](#nin)
    - [nor()](#nor)
    ...
    - [select()](#select)
        - [String synta***REMOVED***](#string-synta***REMOVED***)
    - [selected()](#selected)
    - [selectedInclusively()](#selectedinclusively)
    - [selectedE***REMOVED***clusively()](#selectede***REMOVED***clusively)
    - [size()](#size)
    - [slice()](#slice)
    - [within()](#within)
    - [where()](#where)
    ...
    - [j()](#j)
    - [limit()](#limit)
    - [ma***REMOVED***Time()](#ma***REMOVED***time)
    - [skip()](#skip)
    - [sort()](#sort)
    - [read()](#read)
...
When e***REMOVED***ecuted, the first matching document (if found) is modified according to the update document and passed back.
...
- `upsert`: boolean - creates the object if it doesn't e***REMOVED***ist. defaults to false
...
// the following all e***REMOVED***ecute the command
...
When e***REMOVED***ecuted, the first matching document (if found) is modified according to the update document, removed from the collection and passed as a result.
...
```js
const docs = await mquery().findOne().where('route').intersects(polygon).e***REMOVED***ec()
```

### stream()

E***REMOVED***ecutes the query and returns a stream.
...
mquery().where('location').within().bo***REMOVED***(lowerLeft, upperRight)
```

[MongoDB Documentation](http://docs.mongodb.org/manual/reference/operator/bo***REMOVED***/)
...
mquery().where('name').e***REMOVED***ists()
mquery().where('name').e***REMOVED***ists(true)
mquery().e***REMOVED***ists('name')

// { name: { $e***REMOVED***ists: false }}
mquery().where('name').e***REMOVED***ists(false);
mquery().e***REMOVED***ists('name', false);
```

[MongoDB Documentation](http://docs.mongodb.org/manual/reference/operator/e***REMOVED***ists/)
...
```js
mquery().where('location').near({ center: [139, 74.3] }).ma***REMOVED***Distance(5)
```

[MongoDB Documentation](http://docs.mongodb.org/manual/reference/operator/ma***REMOVED***Distance/)
...
query.where('loc').near({ center: { type: 'Point', coordinates: [10, 10] }, ma***REMOVED***Distance: 5, spherical: true });
query.where('loc').near().geometry({ type: 'Point', coordinates: [10, 10] });

// For a $nearSphere condition, pass the `spherical` option.
query.near({ center: [10, 10], ma***REMOVED***Distance: 5, spherical: true });
```

[MongoDB Documentation](http://www.mongodb.org/display/DOCS/Geospatial+Inde***REMOVED***ing)
...
```js
mquery().where('name').rege***REMOVED***(/^si***REMOVED***stepsrecords/)
```

[MongoDB Documentation](http://docs.mongodb.org/manual/reference/operator/rege***REMOVED***/)

### select()

Specifies which document fields to include or e***REMOVED***clude

```js
// 1 means include, 0 means e***REMOVED***clude
...
#### String synta***REMOVED***

When passing a string, prefi***REMOVED***ing a path with `-` will flag that path as e***REMOVED***cluded. When a path does not have the `-` prefi***REMOVED***, it is included.

```js
// include a and b, e***REMOVED***clude c
query.select('a b -c');
...
### selectedE***REMOVED***clusively()

Determines if the query has selected any fields e***REMOVED***clusively.
...
Use `$where` when you need to select documents using a JavaScript e***REMOVED***pression.

```js
await query.$where('this.comments.length > 10 || this.name.length > 5').e***REMOVED***ec()
...
```js
mquery().hint({ inde***REMOVED***A: 1, inde***REMOVED***B: -1 })
```
...
Specifies the ma***REMOVED***TimeMS option.

```js
query.ma***REMOVED***Time(100)
query.ma***REMOVED***TimeMS(100)
```

[MongoDB documentation](http://docs.mongodb.org/manual/reference/method/cursor.ma***REMOVED***TimeMS/)
...
If a string is passed, it must be a space delimited list of path names. The sort order of each path is ascending unless the path name is prefi***REMOVED***ed with `-` which will be treated as descending.
...
- `linearizable` - The query returns data that reflects all successful majority-acknowledged writes that completed prior to the start of the read operation. The query may wait for concurrently e***REMOVED***ecuting writes to propagate to a majority of replica set members before returning results. (MongoDB 3.4+)
...
Can be break down to use the following synta***REMOVED***:
...
Specifies a time limit, in milliseconds, for the write concern. If `w > 1`, it is ma***REMOVED***imum amount of time to
...
E***REMOVED***ecutes the query and returns a promise which will be resolved with the query results or rejected if the query responds with an error.
...
`mquery` clones query arguments before passing them to a `collection` method for e***REMOVED***ecution.
...
To clone `ObjectIds`, we call its optional `clone` method. If a `clone` method does not e***REMOVED***ist, we fall
...
If you need to specify tags, pass an instance of your drivers ReadPreference constructor or roll your own. `mquery` will store whatever you provide and pass later to your collection during e***REMOVED***ecution.
