...
[![Sauce Test Status](https://app.saucelabs.com/browser-matri***REMOVED***/stream-http.svg)](https://app.saucelabs.com/u/stream-http)
...
* Firefo***REMOVED*** >= 9 (using `moz-chunked-arraybuffer` responseType with ***REMOVED***hr)
...
practical, although limitations in browsers make an e***REMOVED***act clone of the Node API impossible.
...
### E***REMOVED***tra features compared to Node
...
in Chrome 37 and newer, Firefo***REMOVED*** 32 and newer, and Safari 9 and newer.
...
at the e***REMOVED***pense of causing the 'content-type' response header to be incorrectly reported
(as 'te***REMOVED***t/plain; charset=***REMOVED***-user-defined') in some browsers, notably Safari and Chrome 42
...
is only safe for te***REMOVED***t data. May also cause the 'content-type' response header to be
incorrectly reported (as 'te***REMOVED***t/plain; charset=***REMOVED***-user-defined').
...
You will also need to run a [Sauce Connect Pro***REMOVED***y](https://wiki.saucelabs.com/display/DOCS/Sauce+Connect+Pro***REMOVED***y)
