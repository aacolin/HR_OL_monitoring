...
{{## def.willOptimize:
  it.util.varOccurences($code, $ne***REMOVED***tData) < 2
#}}
...
{{## def._optimizeValidate:
  it.util.varReplace($code, $ne***REMOVED***tData, $passData)
#}}
...
{{## def.iterateProperties:
  {{? $ownProperties }}
    ...
    for (var {{=$id***REMOVED***}}=0; {{=$id***REMOVED***}}<{{=$dataProperties}}.length; {{=$id***REMOVED***}}++) {
      var {{=$key}} = {{=$dataProperties}}[{{=$id***REMOVED***}}];
  {{??}}
    for (var {{=$key}} in {{=$data}}) {
  {{?}}
#}}
