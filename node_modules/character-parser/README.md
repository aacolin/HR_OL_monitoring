...
### Custom Delimited E***REMOVED***pressions
...
assert(section.end === 17); // e***REMOVED***clusive end of string
assert(section.src = 'foo.bar("%>").baz');

var section = parser.parseUntil('<%foo.bar("%>").baz%> bing bong', '%>', {start: 2});
assert(section.start === 2);
assert(section.end === 19); // e***REMOVED***clusive end of string
assert(section.src = 'foo.bar("%>").baz');

// Jade-style
var section = parser.parseUntil('#[p= [1, 2][i]]', ']', {start: 2})
assert(section.start === 2);
assert(section.end === 14); // e***REMOVED***clusive end of string
...
assert(section.start === 2);
assert(section.end === 10); // e***REMOVED***clusive end of string
...
All methods may throw an e***REMOVED***ception in the case of synta***REMOVED*** errors. The e***REMOVED***ception contains an additional `code` property that always starts with `CHARACTER_PARSER:` that is unique for the error.

### parse(str, state = defaultState(), options = {start: 0, end: src.length})

Parse a string starting at the inde***REMOVED*** start, and return the state after parsing that string.

If you want to parse one string in multiple sections you should keep passing the resulting state to the ne***REMOVED***t parse operation.
...
If `ignoreNesting` is `true`, it will stop at the first bracket, not taking into account if the bracket part of nesting or not. See e***REMOVED***ample above.

It returns an object with the structure:

```js
{
  start: 0,//inde***REMOVED*** of first character of string
  end: 13,//inde***REMOVED*** of first character after the end of string
  src: 'source string'
}
```
...
```js
{
  ...
  rege***REMOVED***pStart: false, // true if a slash is just encountered and a REGEXP state has just been added to the stack
...
All errors thrown by character-parser has a `code` property attached to it that allows one to identify what sort of error is thrown. For errors thrown from `parse` and `parseUntil`, an additional `inde***REMOVED***` property is available.
...
state.rege***REMOVED***p       → state.current() === parser.TOKEN_TYPES.REGEXP
```
