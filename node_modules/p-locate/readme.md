...
(async () => {
	const foundPath = await pLocate(files, file => pathE***REMOVED***ists(file));
...
*The above is just an e***REMOVED***ample. Use [`locate-path`](https://github.com/sindresorhus/locate-path) if you need this.*
...
This function will receive resolved values from `input` and is e***REMOVED***pected to return a `Promise<boolean>` or `boolean`.
