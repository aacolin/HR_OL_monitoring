/**
 * dashdash - A light, featureful and e***REMOVED***plicit option parsing library for
 * node.js.
 */
...
/**
 * Return an array of lines wrapping the given te***REMOVED***t to the given width.
 * This splits on whitespace. Single tokens longer than `width` are not
 * broken up.
 */
function te***REMOVED***twrap(s, width) {
...
function Parser(config) {
    ...
    for (var i = 0; i < this.options.length; i++) {
        ...
        assert.ok((o.name || o.names) && !(o.name && o.names),
            format('e***REMOVED***actly one of "name" or "names" required: %j', o));
 ...
 *        `process.argv`.
 *      - slice {Number} The inde***REMOVED*** into argv at which options/args begin.
...
Parser.prototype.parse = function parse(inputs) {
    ...
    outer: while (i < args.length) {
        ...
        // Long option
            ...
            var val = null;
            var id***REMOVED*** = name.inde***REMOVED***Of('=');
            if (id***REMOVED*** !== -1) {
                val = name.slice(id***REMOVED*** + 1);
                name = name.slice(0, id***REMOVED***);
            }
 ...
 *      [{names: ['help', 'h'], type: 'bool', help: 'Show help and e***REMOVED***it.'}]
 * then this would return:
 *      '  -h, --help     Show help and e***REMOVED***it.\n'
 *
 * @param config {Object} Config for controlling the option help output.
 *      - indent {Number|String} Default 4. An indent/prefi***REMOVED*** to use for
 *        each option line.
 *      - nameSort {String} Default is 'length'. By default the names are
 *        sorted to put the short opts first (i.e. '-h, --help' preferred
 *        to '--help, -h'). Set to 'none' to not do this sorting.
 *      - ma***REMOVED***Col {Number} Default 80. Note that long tokens in a help string
 *        can go past this.
 ...
 *        output.
 *      - helpWrap {Boolean} Default true. Wrap help te***REMOVED***t in helpCol..ma***REMOVED***Col
 *        bounds.
 * @returns {String}
 */
Parser.prototype.help = function help(config) {
    ...
    assert.ok(~['length', 'none'].inde***REMOVED***Of(nameSort),
        'invalid "config.nameSort"');
    assert.optionalNumber(config.ma***REMOVED***Col, 'config.ma***REMOVED***Col');
    assert.optionalNumber(config.ma***REMOVED***HelpCol, 'config.ma***REMOVED***HelpCol');
    ...
    var ma***REMOVED***Col = config.ma***REMOVED***Col || 80;
    var minHelpCol = config.minHelpCol || 20;
    var ma***REMOVED***HelpCol = config.ma***REMOVED***HelpCol || 40;

    var lines = [];
    var ma***REMOVED***Width = 0;
    this.options.forEach(function (o) {
        if (o.hidden) {
            return;
        }
        if (o.group !== undefined && o.group !== null) {
            // We deal with groups in the ne***REMOVED***t pass
            lines.push(null);
            return;
        }
        ...
        ma***REMOVED***Width = Math.ma***REMOVED***(ma***REMOVED***Width, line.length);
        lines.push(line);
    });
    ...
    if (!helpCol) {
        helpCol = ma***REMOVED***Width + indent.length + 2;
        helpCol = Math.min(Math.ma***REMOVED***(helpCol, minHelpCol), ma***REMOVED***HelpCol);
    }
    var i = -1;
    this.options.forEach(function (o) {
        if (o.hidden) {
        ...
        if (o.helpWrap !== false && config.helpWrap !== false) {
            ...
            if (help.length && !~'.!?"\''.inde***REMOVED***Of(help.slice(-1))) {
                help += '.';
            }
            if (help.length) {
                help += ' ';
            ...
            line += te***REMOVED***twrap(help, ma***REMOVED***Col - helpCol).join(
                '\n' + space(helpCol));
        } else {
 ...
 * @param args.specE***REMOVED***tra {String} Optional. E***REMOVED***tra Bash code content to add
 *      to the end of the "spec". Typically this is used to append Bash
 *      "complete_TYPE" functions for custom option types. See
 *      "e***REMOVED***amples/ddcompletion.js" for an e***REMOVED***ample.
 ...
 *      See `specE***REMOVED***tra` for providing Bash `complete_TYPE` functions, e.g.
 *      `complete_fruit` and `complete_veggie` in this e***REMOVED***ample.
 */
Parser.prototype.bashCompletion = function bashCompletion(args) {
    assert.object(args, 'args');
    assert.string(args.name, 'args.name');
    assert.optionalString(args.specE***REMOVED***tra, 'args.specE***REMOVED***tra');
    assert.optionalArrayOfString(args.argtypes, 'args.argtypes');

    return bashCompletionFromOptions({
        name: args.name,
        specE***REMOVED***tra: args.specE***REMOVED***tra,
 ...
 * @param args.conte***REMOVED***t {String} Optional. A conte***REMOVED***t string for the "local cmd*"
 ...
 *      hidden options and subcmds are "e***REMOVED***cluded". Here e***REMOVED***cluded means they
 ...
 *      `hidden: true` attribute to e***REMOVED***clude them from default help output.
 * @param args.argtypes {Array} Optional. Array of completion types for
 *      positional args (i.e. non-options). E.g.
 *          argtypes = ['fruit', 'veggie', 'file']
 *      will result in completion of fruits for the first arg, veggies for the
...
function bashCompletionSpecFromOptions(args) {
    ...
    assert.optionalString(args.conte***REMOVED***t, 'args.conte***REMOVED***t');
    assert.optionalBool(args.includeHidden, 'args.includeHidden');
    assert.optionalArrayOfString(args.argtypes, 'args.argtypes');

    var conte***REMOVED***t = args.conte***REMOVED***t || '';
    ...
    spec.push(format('local cmd%s_shortopts="%s"',
        conte***REMOVED***t, shortopts.sort().join(' ')));
    spec.push(format('local cmd%s_longopts="%s"',
        conte***REMOVED***t, longopts.sort().join(' ')));
    spec.push(format('local cmd%s_optargs="%s"',
        conte***REMOVED***t, optargs.sort().join(' ')));
    if (args.argtypes) {
        spec.push(format('local cmd%s_argtypes="%s"',
            conte***REMOVED***t, args.argtypes.join(' ')));
    }
    return spec.join('\n');
}
...
function bashCompletionFromOptions(args) {
    ...
    if (args.specE***REMOVED***tra) {
        data.spec += '\n\n' + args.specE***REMOVED***tra;
    }
...
function synopsisFromOpt(o) {
    ...
    // `type` here could be undefined if, for e***REMOVED***ample, the command has a
...
module.e***REMOVED***ports = {
    ...
    // Bash completion-related e***REMOVED***ports
    ...
    // E***REMOVED***port the parseFoo parsers because they might be useful as primitives
