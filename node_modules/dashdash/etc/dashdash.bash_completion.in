...
#       $ tool --file=<TAB>
#       $ tool --file=./d<TAB>
#    We maintain the "--file=" prefi***REMOVED***. Limitation: With the attached prefi***REMOVED***
#    the 'complete -o filenames' doesn't know to do dirname '/' suffi***REMOVED***ing. Meh.
#
# 4. envvars:
#       $ tool $<TAB>
#       $ tool $P<TAB>
#    Limitation: Currently only getting e***REMOVED***ported vars, so we miss "PS1" and
#    others.
#
...
#
# Dev Notes:
# - compgen notes, from http://uni***REMOVED***.stacke***REMOVED***change.com/questions/151118/understand-compgen-builtin-command
...
function _{{name}}_completer {
    ...
    function _dashdash_complete {
        local id***REMOVED*** conte***REMOVED***t
        id***REMOVED***=$1
        conte***REMOVED***t=$2

        local shortopts longopts optargs subcmds allsubcmds argtypes
        shortopts="$(eval "echo \${cmd${conte***REMOVED***t}_shortopts}")"
        longopts="$(eval "echo \${cmd${conte***REMOVED***t}_longopts}")"
        optargs="$(eval "echo \${cmd${conte***REMOVED***t}_optargs}")"
        subcmds="$(eval "echo \${cmd${conte***REMOVED***t}_subcmds}")"
        allsubcmds="$(eval "echo \${cmd${conte***REMOVED***t}_allsubcmds}")"
        IFS=', ' read -r -a argtypes <<< "$(eval "echo \${cmd${conte***REMOVED***t}_argtypes}")"

        trace ""
        trace "_dashdash_complete(id***REMOVED***=$id***REMOVED***, conte***REMOVED***t=$conte***REMOVED***t)"
        ...
        while [[ $i -lt $len && $i -le $COMP_CWORD ]]; do
            ...
            if [[ "$arg" == "--" && $i -lt $COMP_CWORD ]]; then
                ...
                while [[ $j -lt ${#arg} ]]; do
                    optname=${arg:$j:1}
                    trace "    consider inde***REMOVED*** $j: optname '$optname'"
                ...
                _dashdash_complete $(( $i + 1 )) "${conte***REMOVED***t}__${arg/-/_}"
                return
            else
            ...
            trace "    state=$state prefi***REMOVED***='$prefi***REMOVED***' word='$word'"
            i=$(( $i + 1 ))
        done

        trace "  parsed: state=$state optname='$optname' argtype='$argtype' prefi***REMOVED***='$prefi***REMOVED***' word='$word' dashdashseen=$dashdashseen"
        local compgen_opts=
        if [[ -n "$prefi***REMOVED***" ]]; then
            compgen_opts="$compgen_opts -P $prefi***REMOVED***"
        fi
        ...
        arg)
            ...
            # e***REMOVED***pect that we are running with:
            #       complete -o default ...
            ...
            if [[ "${word:0:1}" == '$' ]]; then
                ...
                # gets *close* to the same completions: '-A e***REMOVED***port' misses envvars
                # like "PS1".
                trace "  completing envvars"
                compgen $compgen_opts -P '$' -A e***REMOVED***port -- "${word:1}"
            elif [[ -z "$argtype" ]]; then
...
##     cp FILE /etc/bash_completion.d/{{name}}             # Linu***REMOVED***
##
## or:
##
