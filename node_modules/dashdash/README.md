A light, featureful and e***REMOVED***plicit option parsing library for node.js.

[Why another one? See below](#why). tl;dr: The others I've tried are one of
too loosey goosey (not e***REMOVED***plicit), too big/too many deps, or ill specified.
YMMV.
...
var options = [
    {
        ...
        type: 'bool',
        help: 'Print this help and e***REMOVED***it.'
    }
];
...
# Longer E***REMOVED***ample

A more realistic [starter script "foo.js"](./e***REMOVED***amples/foo.js) is as follows.
...
var options = [
    {
        name: 'version',
        type: 'bool',
        help: 'Print tool version and e***REMOVED***it.'
    },
    {
        names: ['help', 'h'],
        type: 'bool',
...
Some e***REMOVED***ample output from this script (foo.js):
...
options:
    --version             Print tool version and e***REMOVED***it.
    -h, --help            Print this help and e***REMOVED***it.
...
$ node foo.js -f bar.t***REMOVED***t
# opts: { file: 'bar.t***REMOVED***t',
  _order: [ { name: 'file', value: 'bar.t***REMOVED***t', from: 'argv' } ],
  _args: [] }
# args: []
...
See the ["e***REMOVED***amples"](e***REMOVED***amples/) dir for a number of starter e***REMOVED***amples using
...
dashdash makes this easy. We can change the 'verbose' option in the e***REMOVED***ample
...
```shell
$ FOO_VERBOSE= node e***REMOVED***amples/foo.js                 # not set
# opts: { _order: [], _args: [] }
# args: []

$ FOO_VERBOSE=0 node e***REMOVED***amples/foo.js                # '0' is false
# opts: { verbose: [ false ],
  _order: [ { key: 'verbose', value: false, from: 'env' } ],
  _args: [] }
# args: []

$ FOO_VERBOSE=1 node e***REMOVED***amples/foo.js                # true
# opts: { verbose: [ true ],
  _order: [ { key: 'verbose', value: true, from: 'env' } ],
  _args: [] }
# args: []

$ FOO_VERBOSE=boogabooga node e***REMOVED***amples/foo.js       # true
# opts: { verbose: [ true ],
  _order: [ { key: 'verbose', value: true, from: 'env' } ],
  _args: [] }
# args: []
...
```shell
$ FOO_FILE=data.t***REMOVED***t node e***REMOVED***amples/foo.js
# opts: { file: 'data.t***REMOVED***t',
  _order: [ { key: 'file', value: 'data.t***REMOVED***t', from: 'env' } ],
  _args: [] }
# args: []
```
...
```shell
$ FOO_TIMEOUT=5000 node e***REMOVED***amples/foo.js
# opts: { timeout: 5000,
  _order: [ { key: 'timeout', value: 5000, from: 'env' } ],
  _args: [] }
# args: []

$ FOO_TIMEOUT=blarg node e***REMOVED***amples/foo.js
...
See [e***REMOVED***amples/ddcompletion.js](e***REMOVED***amples/ddcompletion.js) for a complete
e***REMOVED***ample, including how one can define bash functions for completion of custom
...
- TODO: document specE***REMOVED***tra
...
- `allowUnknown` (Boolean).  Optional.  Default is false.  If false, this causes
  ...
  **Caveat**: When a shortopt group, such as `-***REMOVED***az` contains a mi***REMOVED*** of
  ...
  following command line:
        ...
        node ./tool.js -***REMOVED***az

  where `-***REMOVED***` and `-z` are unknown.  There are multiple ways to
  interpret this:

    1. `-***REMOVED***` takes a value: `{***REMOVED***: 'az'}`
    2. `-***REMOVED***` and `-z` are both booleans: `{***REMOVED***:true,a:true,z:true}`

  Since dashdash does not know what `-***REMOVED***` and `-z` are, it can't know
  if you'd prefer to receive `{a:true,_args:['-***REMOVED***','-z']}` or
  `{***REMOVED***:'az'}`, or `{_args:['-***REMOVED***az']}`. Leaving the positional arg unprocessed
  is the easiest mistake for the user to recover from.


# Option specs

E***REMOVED***ample using all fields (required fields are noted):
...
- `completionType` (String). Optional. This is used for [Bash
  ...
  following values have meaning:
    ...
    - *Any string FOO for which a `function complete_FOO` Bash function is
      ...
      these custom functions are provided in the `specE***REMOVED***tra` argument to
      `dashdash.bashCompletionFromOptions()`. See
      ["e***REMOVED***amples/ddcompletion.js"](e***REMOVED***amples/ddcompletion.js) for an e***REMOVED***ample.

- `env` (String or Array of String). Optional. An environment variable name
  (or names) that can be used as a fallback for this option. For e***REMOVED***ample,
...
- `helpWrap` (Boolean). Optional, default true. Set this to `false` to have
  that option's `help` *not* be te***REMOVED***t wrapped in `<parser>.help()` output.
...
the heading for the subsequent options in the array.  For e***REMOVED***ample:
...
- `ma***REMOVED***Col` (Number). Default 80. Note that reflow is just done on whitespace
  so a long token in the option help can overflow ma***REMOVED***Col.
- `helpCol` (Number). If not set a reasonable value will be determined
  between `minHelpCol` and `ma***REMOVED***HelpCol`.
- `minHelpCol` (Number). Default 20.
- `ma***REMOVED***HelpCol` (Number). Default 40.
- `helpWrap` (Boolean). Default true. Set to `false` to have option `help`
  strings *not* be te***REMOVED***twrapped to the helpCol..ma***REMOVED***Col range.
...
For e***REMOVED***ample, a simple option type that accepts 'yes', 'y', 'no' or 'n' as
a boolean argument would look like:
    ...
    function parseYesNo(option, optstr, arg) {
        ...
        if (~['yes', 'y'].inde***REMOVED***Of(argLower)) {
            return true;
        } else if (~['no', 'n'].inde***REMOVED***Of(argLower)) {
            return false;
        } else {
...
See "e***REMOVED***amples/custom-option-\*.js" for other e***REMOVED***amples.
...
- `optimist` has surprise interpretation of options (at least to me).
  ...
  `process.e***REMOVED***it` calls makes it hard to use as a libary.
...
- `posi***REMOVED***-getopt` No type validation. Though that isn't a killer. AFAIK can't
