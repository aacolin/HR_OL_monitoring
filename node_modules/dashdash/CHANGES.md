...
- [issue #30] Change the output used by dashdash's Bash completion support to
  ...
  sorting rules on different Bash/platforms. For e***REMOVED***ample:
...
- New `synopsisFromOpt(<option spec>)` function. This will be used by
  ...
  of options for a command. Some e***REMOVED***amples:
...
- Update assert-plus dep to 1.***REMOVED*** to get recent fi***REMOVED***es (particularly for
  `assert.optional*`).

- Drop testing (and official support in packages.json#engines) for node 0.8.***REMOVED***.
  Add testing against node 5.***REMOVED*** and 4.***REMOVED*** with `make testall`.

- [pull #16] Change the `positiveInteger` type to NOT accept zero (0).
  For those who might need the old behaviour, see
  "e***REMOVED***amples/custom-option-intGteZero.js".  (By Dave Pacheco.)
...
- Bash completion: Add `argtypes` to specify the types of positional args.
  ...
  handle completing those types via the `specE***REMOVED***tra` arg. See
...
- Bash completion: Tweak so that options or only offered as completions when
  ...
  fallback to Bash's "default" completion. For e***REMOVED***ample `ls <TAB>` wouldn't
  result in filename completion. Now it will.

- Bash completion: A workaround for not being able to e***REMOVED***plicitly have *no*
  ...
  completion). Before this change, an attempt to e***REMOVED***plicitly say "there are
  ...
  This is a bit of a hack, but IMO a better e***REMOVED***perience than the surprise
...
- Support for generating Bash completion files. See the "Bash completion"
  section of the README.md and "e***REMOVED***amples/ddcompletion.js" for an e***REMOVED***ample.
...
- Add the `arrayFlatten` boolean option to `dashdash.addOptionType` used for
  ...
  where each usage of the option can return multiple results. For e***REMOVED***ample:

        node mytool.js --foo a,b --foo c

  We could define an option type for `--foo` such that
  `opts.foo = ['a', 'b', 'c']`. See
  "[e***REMOVED***amples/custom-option-arrayOfCommaSepString.js](e***REMOVED***amples/custom-option-arrayOfCommaSepString.js)"
  for an e***REMOVED***ample.
...
- [issue #11] Fi***REMOVED*** option group breakage introduced in v1.9.0.


## 1.9.0

- [issue #10] Custom option types added with `addOptionType` can specify a
  "default" value. See "e***REMOVED***amples/custom-option-fruit.js".


## 1.8.0

- Support `hidden: true` in an option spec to have help output e***REMOVED***clude this
  option.


## 1.7.3

- [issue #8] Fi***REMOVED*** parsing of a short option group when one of the
  option takes an argument. For e***REMOVED***ample, consider `tail` with
...
- E***REMOVED***clude 'tools/' dir in packages published to npm.
...
- Add support for adding custom option types. "e***REMOVED***amples/custom-option-duration.js"
  shows an e***REMOVED***ample adding a "duration" option type.
...
- [issue #4] Add `date` and `arrayOfDate` option types. They accept these date
  ...
  "2014-03-28T18:35:01.489Z"). See "e***REMOVED***amples/date.js" for an e***REMOVED***ample usage.

        $ node e***REMOVED***amples/date.js -s 2014-01-01 -e $(date +%s)
...
- [pull #2, pull #3] Add a `allowUnknown: true` option on `createParser` to
  ...
  throwing an e***REMOVED***ception (by https://github.com/isaacs).
...
- Fi***REMOVED*** a subtlety where a *bool* option using both `env` and `default` didn't
  work e***REMOVED***actly correctly. If `default: false` then all was fine (by luck).
  ...
  wanted `FOO_VERBOSE=0` to make the option false, then you need the fi***REMOVED***
  in this version of dashdash.


## 1.3.1

- [issue #1] Fi***REMOVED*** an envvar not winning over an option 'default'. Previously
...
- [Backward incompatible change for boolean envvars] Change the
  ...
  true -- which was quite misleading. E***REMOVED***ample:

        $ FOO_VERBOSE=0 node e***REMOVED***amples/foo.js
        # opts: { verbose: [ false ],
...
- Fi***REMOVED*** for `parse.help({includeEnv: true, ...})` handling to ensure that an
...
- Add "integer" and "arrayOfInteger" option types that accepts only integers.
  Note that, for better or worse, these do NOT accept: "0***REMOVED***42" (he***REMOVED***), "1e2"
  (with e***REMOVED***ponent) or "1.", "3.0" (floats).


## 1.0.1

- Fi***REMOVED*** not modifying the given option spec objects (which breaks creating
