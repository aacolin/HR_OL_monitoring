...
Modular standard library for JavaScript. Includes polyfills for [ECMAScript 5](#ecmascript-5), [ECMAScript 6](#ecmascript-6): [promises](#ecmascript-6-promise), [symbols](#ecmascript-6-symbol), [collections](#ecmascript-6-collections), iterators, [typed arrays](#ecmascript-6-typed-arrays), [ECMAScript 7+ proposals](#ecmascript-7-proposals), [setImmediate](#setimmediate), etc. Some additional features such as [dictionaries](#dict) or [e***REMOVED***tended partial application](#partial-application). You can require only needed features or use it without global namespace pollution.

[*E***REMOVED***ample*](http://goo.gl/a2***REMOVED***e***REMOVED***l):
```js
Array.from(new Set([1, 2, 3, 2, 1]));          // => [1, 2, 3]
'*'.repeat(10);                                // => '**********'
Promise.resolve(32).then(***REMOVED*** => console.log(***REMOVED***)); // => 32
setImmediate(***REMOVED*** => console.log(***REMOVED***), 42);         // => 42
```
...
core.Promise.resolve(32).then(***REMOVED*** => console.log(***REMOVED***)); // => 32
core.setImmediate(***REMOVED*** => console.log(***REMOVED***), 42);         // => 42
```
...
- [Features](#features)
  ...
  - [ECMAScript 6](#ecmascript-6)
    ...
    - [ECMAScript 6: RegE***REMOVED***p](#ecmascript-6-rege***REMOVED***p)
...
* [as a library](https://raw.githack.com/zloirock/core-js/v2.6.12/client/library.min.js): Like "default", but does not pollute the global namespace (see [2nd e***REMOVED***ample at the top](#core-js)).
* [shim only](https://raw.githack.com/zloirock/core-js/v2.6.12/client/shim.min.js): Only includes the standard methods.

Warning: if you use `core-js` with the e***REMOVED***tension of native objects, require all needed `core-js` modules at the beginning of entry point of your application, otherwise, conflicts may occur.
...
require('core-js/fn/array/find-inde***REMOVED***');
Array.from(new Set([1, 2, 3, 2, 1])); // => [1, 2, 3]
[1, 2, NaN, 3, 4].findInde***REMOVED***(isNaN);   // => 2

// or, w/o global namespace pollution:

var Set       = require('core-js/library/fn/set');
var from      = require('core-js/library/fn/array/from');
var findInde***REMOVED*** = require('core-js/library/fn/array/find-inde***REMOVED***');
from(new Set([1, 2, 3, 2, 1]));      // => [1, 2, 3]
findInde***REMOVED***([1, 2, NaN, 3, 4], isNaN); // => 2
```
Available entry points for methods / constructors, as above e***REMOVED***amples, and namespaces: for e***REMOVED***ample, `core-js/es6/array` (`core-js/library/es6/array`) contains all [ES6 `Array` features](#ecmascript-6-array), `core-js/es6` (`core-js/library/es6`) contains all ES6 features.
...
* `core-js` is e***REMOVED***tremely modular and uses a lot of very tiny modules, because of that for usage in browsers bundle up `core-js` instead of usage loader for each file, otherwise, you will have hundreds of requests.

#### CommonJS and prototype methods without global namespace pollution
In the `library` version, we can't pollute prototypes of native constructors. Because of that, prototype methods transformed to static methods like in e***REMOVED***amples above. `babel` `runtime` transformer also can't transform them. But with transpilers we can use one more trick - [bind operator and virtual methods](https://github.com/zenparsing/es-function-bind). Special for that, available `/virtual/` entry points. E***REMOVED***ample:
```js
import fill from 'core-js/library/fn/array/virtual/fill';
import findInde***REMOVED*** from 'core-js/library/fn/array/virtual/find-inde***REMOVED***';

Array(10)::fill(0).map((a, b) => b * b)::findInde***REMOVED***(it => it && !(it % 8)); // => 4

// or

import {fill, findInde***REMOVED***} from 'core-js/library/fn/array/virtual';
...
```
Where `core.dict` and `es6` are modules (namespaces) names, which will be added to the build, `es6.promise` and `es6.math` are modules (namespaces) names, which will be e***REMOVED***cluded from the build, `--library=on` is flag for build without global namespace pollution and `custom` is target file name.

Available namespaces: for e***REMOVED***ample, `es6.array` contains [ES6 `Array` features](#ecmascript-6-array), `es6` contains all modules whose names start with `es6`.

### Custom build (from e***REMOVED***ternal scripts)

[`core-js-builder`](https://www.npmjs.com/package/core-js-builder) package e***REMOVED***ports a function that takes the same parameters as the `build` target from the previous section. This will conditionally include or e***REMOVED***clude certain parts of `core-js`:
...
require('core-js-builder')({
  ...
  umd: true                      // use UMD wrapper for e***REMOVED***port `core` object, by default - true
}).then(code => {
  // ...
}).catch(error => {
  // ...
...
- Safari 5+
- Opera 12+
- Internet E***REMOVED***plorer 6+ (sure, IE8- with ES3 limitations)
- Edge
- Android Browser 2.3+
- iOS Safari 5.1+
- PhantomJS 1.9 / 2.1
...
Object
  ...
  .preventE***REMOVED***tensions(object)             -> object, cap for ie8-
  .isSealed(object)                      -> bool, cap for ie8-
  .isFrozen(object)                      -> bool, cap for ie8-
  .isE***REMOVED***tensible(object)                  -> bool, cap for ie8-
Array
  .isArray(var)                                -> bool
  #slice(start?, end?)                         -> array, fi***REMOVED*** for ie7-
  #join(string = ',')                          -> string, fi***REMOVED*** for ie7-
  #inde***REMOVED***Of(var, from?)                         -> int
  #lastInde***REMOVED***Of(var, from?)                     -> int
  #every(fn(val, inde***REMOVED***, @), that)              -> bool
  #some(fn(val, inde***REMOVED***, @), that)               -> bool
  #forEach(fn(val, inde***REMOVED***, @), that)            -> void
  #map(fn(val, inde***REMOVED***, @), that)                -> array
  #filter(fn(val, inde***REMOVED***, @), that)             -> array
  #reduce(fn(memo, val, inde***REMOVED***, @), memo?)      -> var
  #reduceRight(fn(memo, val, inde***REMOVED***, @), memo?) -> var
  #sort(fn?)                                   -> @, fi***REMOVED***es for some engines
Function
...
RegE***REMOVED***p
  #toString() -> str
Number
  #toFi***REMOVED***ed(digits)        -> string
  #toPrecision(precision) -> string
parseInt(str, radi***REMOVED***) -> int
parseFloat(str)      -> num
Date
  .now()         -> int
  #toISOString() -> string
...
Object
  ...
  .freeze(var)                           -> var
  .seal(var)                             -> var
  .preventE***REMOVED***tensions(var)                -> var
  .isFrozen(var)                         -> bool
  .isSealed(var)                         -> bool
  .isE***REMOVED***tensible(var)                     -> bool
  #toString()                            -> string, ES6 fi***REMOVED***: @@toStringTag support
```
[*CommonJS entry points:*](#commonjs)
```
...
core-js(/library)/fn/object/prevent-e***REMOVED***tensions
core-js(/library)/fn/object/is-frozen
core-js(/library)/fn/object/is-sealed
core-js(/library)/fn/object/is-e***REMOVED***tensible
core-js/fn/object/to-string
```
[*E***REMOVED***amples*](http://goo.gl/ywdwPz):
```js
var foo = {q: 1, w: 2}
  , bar = {e: 3, r: 4}
  , baz = {t: 5, y: 6};
...
```
[*E***REMOVED***ample*](http://goo.gl/zqu3Wp):
```js
...
```
#### ECMAScript 6: Array
Modules [`es6.array.from`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.from.js), [`es6.array.of`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.of.js), [`es6.array.copy-within`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.copy-within.js), [`es6.array.fill`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.fill.js), [`es6.array.find`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.find.js), [`es6.array.find-inde***REMOVED***`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.find-inde***REMOVED***.js), [`es6.array.iterator`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.iterator.js). ES5 features with fi***REMOVED***es: [`es6.array.is-array`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.is-array.js), [`es6.array.slice`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.slice.js), [`es6.array.join`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.join.js), [`es6.array.inde***REMOVED***-of`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.inde***REMOVED***-of.js), [`es6.array.last-inde***REMOVED***-of`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.last-inde***REMOVED***-of.js), [`es6.array.every`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.every.js), [`es6.array.some`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.some.js), [`es6.array.for-each`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.for-each.js), [`es6.array.map`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.map.js), [`es6.array.filter`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.filter.js), [`es6.array.reduce`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.reduce.js), [`es6.array.reduce-right`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.reduce-right.js), [`es6.array.sort`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.array.sort.js).
```js
Array
  .from(iterable | array-like, mapFn(val, inde***REMOVED***)?, that) -> array
  .of(...args)                                           -> array
  .isArray(var)                                          -> bool
  #copyWithin(target = 0, start = 0, end = @length)      -> @
  #fill(val, start = 0, end = @length)                   -> @
  #find(fn(val, inde***REMOVED***, @), that)                         -> val
  #findInde***REMOVED***(fn(val, inde***REMOVED***, @), that)                    -> inde***REMOVED*** | -1
  #values()                                              -> iterator
  #keys()                                                -> iterator
  #entries()                                             -> iterator
  #join(string = ',')                                    -> string, fi***REMOVED*** for ie7-
  #slice(start?, end?)                                   -> array, fi***REMOVED*** for ie7-
  #inde***REMOVED***Of(var, from?)                                   -> inde***REMOVED*** | -1
  #lastInde***REMOVED***Of(var, from?)                               -> inde***REMOVED*** | -1
  #every(fn(val, inde***REMOVED***, @), that)                        -> bool
  #some(fn(val, inde***REMOVED***, @), that)                         -> bool
  #forEach(fn(val, inde***REMOVED***, @), that)                      -> void
  #map(fn(val, inde***REMOVED***, @), that)                          -> array
  ...
  #reduce(fn(memo, val, inde***REMOVED***, @), memo?)                -> var
  #reduceRight(fn(memo, val, inde***REMOVED***, @), memo?)           -> var
  #sort(fn?)                                             -> @, invalid arguments fi***REMOVED***
...
core-js(/library)/fn/array/find-inde***REMOVED***
...
core-js(/library)/fn/array/inde***REMOVED***-of
core-js(/library)/fn/array/last-inde***REMOVED***-of
...
core-js(/library)/fn/array/virtual/find-inde***REMOVED***
...
core-js(/library)/fn/array/virtual/inde***REMOVED***-of
core-js(/library)/fn/array/virtual/last-inde***REMOVED***-of
...
```
[*E***REMOVED***amples*](http://goo.gl/oaUFUf):
```js
Array.from(new Set([1, 2, 3, 2, 1]));      // => [1, 2, 3]
...
[4, 8, 15, 16, 23, 42].findInde***REMOVED***(isOdd); // => 2
[4, 8, 15, 16, 23, 42].find(isNaN);      // => undefined
[4, 8, 15, 16, 23, 42].findInde***REMOVED***(isNaN); // => -1
...
core-js(/library)/fn/string/fi***REMOVED***ed
...
core-js(/library)/fn/string/virtual/fi***REMOVED***ed
...
```
[*E***REMOVED***amples*](http://goo.gl/3UaQ93):
```js
for(var val of 'a𠮷b'){
...
var name = 'Bob';
String.raw`Hi\n${name}!`;           // => 'Hi\\nBob!' (ES6 template string synta***REMOVED***)
String.raw({raw: 'test'}, 0, 1, 2); // => 't0e1s2t'

'foo'.bold();                     // => '<b>foo</b>'
'bar'.anchor('a"b');              // => '<a name="a&quot;b">bar</a>'
'baz'.link('http://e***REMOVED***ample.com'); // => '<a href="http://e***REMOVED***ample.com">baz</a>'
```
#### ECMAScript 6: RegE***REMOVED***p
Modules [`es6.rege***REMOVED***p.constructor`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.rege***REMOVED***p.constructor.js) and [`es6.rege***REMOVED***p.flags`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.rege***REMOVED***p.flags.js).

Support well-known [symbols](#ecmascript-6-symbol) `@@match`, `@@replace`, `@@search` and `@@split`, modules [`es6.rege***REMOVED***p.match`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.rege***REMOVED***p.match.js), [`es6.rege***REMOVED***p.replace`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.rege***REMOVED***p.replace.js), [`es6.rege***REMOVED***p.search`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.rege***REMOVED***p.search.js) and [`es6.rege***REMOVED***p.split`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/es6.rege***REMOVED***p.split.js).
```
[new] RegE***REMOVED***p(pattern, flags?) -> rege***REMOVED***p, ES6 fi***REMOVED***: can alter flags (IE9+)
  #flags -> str (IE9+)
  #toString() -> str, ES6 fi***REMOVED***es
  #@@match(str)             -> array | null
  #@@replace(str, replacer) -> string
  #@@search(str)            -> inde***REMOVED***
  #@@split(str, limit)      -> array
String
  #match(tpl)             -> var, ES6 fi***REMOVED*** for support @@match
  #replace(tpl, replacer) -> var, ES6 fi***REMOVED*** for support @@replace
  #search(tpl)            -> var, ES6 fi***REMOVED*** for support @@search
  #split(tpl, limit)      -> var, ES6 fi***REMOVED*** for support @@split, some fi***REMOVED***es for old engines
```
[*CommonJS entry points:*](#commonjs)
```
core-js/es6/rege***REMOVED***p
core-js/fn/rege***REMOVED***p/constructor
core-js(/library)/fn/rege***REMOVED***p/flags
core-js/fn/rege***REMOVED***p/to-string
core-js/fn/rege***REMOVED***p/match
core-js/fn/rege***REMOVED***p/replace
core-js/fn/rege***REMOVED***p/search
core-js/fn/rege***REMOVED***p/split
```
[*E***REMOVED***amples*](http://goo.gl/PiJ***REMOVED***BD):
```js
RegE***REMOVED***p(/./g, 'm'); // => /./m
...
[new] Number(var)         -> number | number object
  ...
  #toFi***REMOVED***ed(digits)        -> string, fi***REMOVED***es
  #toPrecision(precision) -> string, fi***REMOVED***es
parseFloat(str)           -> num, fi***REMOVED***es
parseInt(str)             -> int, fi***REMOVED***es
```
[*CommonJS entry points:*](#commonjs)
```
...
core-js(/library)/fn/math/e***REMOVED***pm1
...
```
[*E***REMOVED***ample*](http://goo.gl/haeHLR):
```js
...
```js
new Promise(e***REMOVED***ecutor(resolve(var), reject(var))) -> promise
...
```
Basic [*e***REMOVED***ample*](http://goo.gl/vGrtUC):
```js
...
}).catch(***REMOVED*** => console.log(***REMOVED***));         // => => Error: Irror!
```
`Promise.resolve` and `Promise.reject` [*e***REMOVED***ample*](http://goo.gl/vr8TN3):
```js
Promise.resolve(42).then(***REMOVED*** => console.log(***REMOVED***)); // => 42
Promise.reject(42).catch(***REMOVED*** => console.log(***REMOVED***)); // => 42

Promise.resolve($.getJSON('/data.json')); // => ES6 promise
```
`Promise.all` [*e***REMOVED***ample*](http://goo.gl/RdoDBZ):
```js
Promise.all([
  'foo',
  sleepRandom(5),
...
]).then(***REMOVED*** => console.log(***REMOVED***)); // => ['foo', 956, 85, 382]
```
`Promise.race` [*e***REMOVED***ample*](http://goo.gl/L8ovkJ):
```js
...
timeLimit(sleepRandom(5), 10).then(***REMOVED*** => console.log(***REMOVED***));   // => 853, after 5 sec.
timeLimit(sleepRandom(15), 10).catch(***REMOVED*** => console.log(***REMOVED***)); // Error: Await > 10 sec
```
ECMAScript 7 [async functions](https://tc39.github.io/ecmascript-asyncawait) [e***REMOVED***ample](http://goo.gl/wnQS4j):
```js
...
```
In a browser on rejection, by default, you will see notify in the console, or you can add a custom handler and a handler on handling unhandled, [*e***REMOVED***ample*](http://goo.gl/Wozskl):
```js
...
```
[*Basic e***REMOVED***ample*](http://goo.gl/BbvWFc):
```js
var Person = (function(){
...
```
`Symbol.for` & `Symbol.keyFor` [*e***REMOVED***ample*](http://goo.gl/0pdJjX):
```js
var symbol = Symbol.for('key');
symbol === Symbol.for('key'); // true
Symbol.keyFor(symbol);        // 'key'
```
[*E***REMOVED***ample*](http://goo.gl/mKVOQJ) with methods for getting own object keys:
```js
var O = {a: 1};
...
You can disable defining setters in `Object.prototype`. [E***REMOVED***ample](http://goo.gl/N5UD7J):
```js
Symbol.useSimple();
var s1 = Symbol('s1')
  , o1 = {};
...
* Some problems possible with environment e***REMOVED***otic objects (for e***REMOVED***ample, IE `localStorage`).

#### ECMAScript 6: Collections
`core-js` uses native collections in most case, just fi***REMOVED***es methods / constructor, if it's required, and in old environment uses fast polyfill (O(1) lookup).
#### Map
...
```
[*E***REMOVED***amples*](http://goo.gl/GWR7NI):
```js
var a = [1];
...
```
[*E***REMOVED***amples*](http://goo.gl/bmhLwg):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/SILXyw):
```js
var a = [1]
  , b = [2]
  , c = [3];
...
```
[*E***REMOVED***amples*](http://goo.gl/TdFbE***REMOVED***):
```js
var a = [1]
  , b = [2]
  , c = [3];
...
Implementations and fi***REMOVED***es `ArrayBuffer`, `DataView`, typed arrays constructors, static and prototype methods. Typed Arrays work only in environments with support descriptors (IE9+), `ArrayBuffer` and `DataView` should work anywhere.
...

{
  ...
  .from(arrayLike | iterable, mapFn(val, inde***REMOVED***)?, that) -> typed
  .of(...args) -> typed
  #BYTES_PER_ELEMENT -> uint
  #copyWithin(target = 0, start = 0, end = @length) -> @
  #every(fn(val, inde***REMOVED***, @), that) -> bool
  #fill(val, start = 0, end = @length) -> @
  #filter(fn(val, inde***REMOVED***, @), that) -> typed
  #find(fn(val, inde***REMOVED***, @), that) -> val
  #findInde***REMOVED***(fn(val, inde***REMOVED***, @), that) -> inde***REMOVED***
  #forEach(fn(val, inde***REMOVED***, @), that) -> void
  #inde***REMOVED***Of(var, from?) -> int
  #join(string = ',') -> string
  #lastInde***REMOVED***Of(var, from?) -> int
  #map(fn(val, inde***REMOVED***, @), that) -> typed
  #reduce(fn(memo, val, inde***REMOVED***, @), memo?) -> var
  #reduceRight(fn(memo, val, inde***REMOVED***, @), memo?) -> var
  #reverse() -> @
...
```
[*E***REMOVED***amples*](http://goo.gl/yla75z):
```js
...
Reflect
  ...
  .isE***REMOVED***tensible(target) -> bool
  .ownKeys(target) -> array
  .preventE***REMOVED***tensions(target) -> bool
...
core-js(/library)/fn/reflect/enumerate (deprecated and will be removed from the ne***REMOVED***t major release)
...
core-js(/library)/fn/reflect/is-e***REMOVED***tensible
core-js(/library)/fn/reflect/own-keys
core-js(/library)/fn/reflect/prevent-e***REMOVED***tensions
core-js(/library)/fn/reflect/set
core-js(/library)/fn/reflect/set-prototype-of
```
[*E***REMOVED***amples*](http://goo.gl/gVT0cH):
```js
var O = {a: 1};
Object.defineProperty(O, 'b', {value: 2});
O[Symbol('c')] = 3;
...
```
[*E***REMOVED***amples*](http://goo.gl/2Gq4ma):
```js
[1, 2, 3].includes(2);        // => true
[1, 2, 3].includes(4);        // => false
[1, 2, 3].includes(2, 2);     // => false

[NaN].inde***REMOVED***Of(NaN);           // => -1
[NaN].includes(NaN);          // => true
Array(1).inde***REMOVED***Of(undefined);  // => -1
...
```
[*E***REMOVED***amples*](http://goo.gl/6kuGOn):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/hK5ccv):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/gEqMl7):
```js
...
```
[*E***REMOVED***amples*](https://goo.gl/AhyBbJ):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/Er5lMJ):
```js
...
```
[*E***REMOVED***amples*](https://goo.gl/k5GGRo):
```js
...
```
[*E***REMOVED***amples*](https://goo.gl/jTXsZi):
```js
...
Set
  .of(...args) -> set
  .from(iterable, mapFn(val, inde***REMOVED***)?, that?) -> set
Map
  .of(...args) -> map
  .from(iterable, mapFn(val, inde***REMOVED***)?, that?) -> map
WeakSet
  .of(...args) -> weakset
  .from(iterable, mapFn(val, inde***REMOVED***)?, that?) -> weakset
WeakMap
  .of(...args) -> weakmap
  .from(iterable, mapFn(val, inde***REMOVED***)?, that?) -> weakmap
```
[*CommonJS entry points:*](#commonjs)
```js
...
```
[*E***REMOVED***amples*](https://goo.gl/mSC7eU):
```js
...
```js
String
  #matchAll(rege***REMOVED***p) -> iterator
```
[*CommonJS entry points:*](#commonjs)
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/6kp9EB):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/1LDywi):
```js
new Observable(observer => {
  observer.ne***REMOVED***t('hello');
  observer.ne***REMOVED***t('world');
  observer.complete();
}).forEach(it => console.log(it))
  .then(_ => console.log('!'));
```
* `Math.{clamp, DEG_PER_RAD, degrees, fscale, rad-per-deg, radians, scale}` 
  [proposal](https://github.com/rwaldron/proposal-math-e***REMOVED***tensions) - modules 
...
Math
  .DEG_PER_RAD -> number
  .RAD_PER_DEG -> number
  .clamp(***REMOVED***, lower, upper) -> number
  .degrees(radians) -> number
  .fscale(***REMOVED***, inLow, inHigh, outLow, outHigh) -> number
  .radians(degrees) -> number
  .scale(***REMOVED***, inLow, inHigh, outLow, outHigh) -> number
```
[*CommonJS entry points:*](#commonjs)
```js
...
```js
Math
  .signbit(***REMOVED***) -> bool
```
[*CommonJS entry points:*](#commonjs)
```js
core-js(/library)/fn/math/signbit
```
[*E***REMOVED***amples*](http://es6.zloirock.ru/):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/XluXI8):
```js
'a𠮷b'.at(1);        // => '𠮷'
'a𠮷b'.at(1).length; // => 2
```
...
```
[*E***REMOVED***amples*](http://goo.gl/t***REMOVED***3SRK):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/KCo3PS):
```js
var O = {};
...
Module [`web.timers`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/web.timers.js). Additional arguments fi***REMOVED*** for IE9-.
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/6nXGr***REMOVED***):
```js
...
```js
{
  ...
  SourceBufferList,
  StyleSheetList,
  Te***REMOVED***tTrackCueList,
  Te***REMOVED***tTrackList,
  TouchList
}
  #@@iterator() -> iterator (values)
...
```
[*E***REMOVED***amples*](http://goo.gl/lfXVFl):
```js
for(var {id} of document.querySelectorAll('*')){
  if(id)console.log(id);
}

for(var [inde***REMOVED***, {id}] of document.querySelectorAll('*').entries()){
  if(id)console.log(inde***REMOVED***, id);
}
```
### Non-standard
[*CommonJS entry points:*](#commonjs)
...
Object
  .isObject(var) -> bool
  .classof(var) -> string
  .define(target, mi***REMOVED***in) -> target
  .make(proto | null, mi***REMOVED***in?) -> object
```
...
```
Object classify [*e***REMOVED***amples*](http://goo.gl/YZQmGo):
```js
...
function E***REMOVED***ample(){}
E***REMOVED***ample.prototype[Symbol.toStringTag] = 'E***REMOVED***ample';

classof(new E***REMOVED***ample);          // => 'E***REMOVED***ample'
```
`Object.define` and `Object.make` [*e***REMOVED***amples*](http://goo.gl/rtpD5Z):
```js
// Before:
...
// Simple inheritance:
function Vector2D(***REMOVED***, y){
  this.***REMOVED*** = ***REMOVED***;
  this.y = y;
}
Object.define(Vector2D.prototype, {
  get ***REMOVED***y(){
    return Math.hypot(this.***REMOVED***, this.y);
  }
});
function Vector3D(***REMOVED***, y, z){
...
Vector3D.prototype = Object.make(Vector2D.prototype, {
  ...
  get ***REMOVED***yz(){
    return Math.hypot(this.***REMOVED***, this.y, this.z);
  }
});

var vector = new Vector3D(9, 12, 20);
console.log(vector.***REMOVED***y);  // => 15
console.log(vector.***REMOVED***yz); // => 25
vector.y++;
console.log(vector.***REMOVED***y);  // => 15.811388300841896
console.log(vector.***REMOVED***yz); // => 25.495097567963924
```
#### Dict
...
[*E***REMOVED***amples*](http://goo.gl/pnp8Vr):
```js
var map = new Map([['a', 1], ['b', 2], ['c', 3]]);
...
[*E***REMOVED***amples*](http://goo.gl/***REMOVED***AvECH):
```js
...
```
Basic dict operations for objects with prototype [*e***REMOVED***amples*](http://goo.gl/B28UnG):
```js
'q' in {q: 1};            // => true
'toString' in {};         // => true
...
[*E***REMOVED***amples*](http://goo.gl/***REMOVED***Fi1RH):
```js
var dict = {a: 1, b: 2, c: 3};
...
[*E***REMOVED***amples*](http://goo.gl/p9ZJ8K):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/o45pCN):
```js
...
```
#### Escaping strings
Modules [`core.rege***REMOVED***p.escape`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/core.rege***REMOVED***p.escape.js), [`core.string.escape-html`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/core.string.escape-html.js) and [`core.string.unescape-html`](https://github.com/zloirock/core-js/blob/v2.6.12/modules/core.string.unescape-html.js).
```js
RegE***REMOVED***p
  .escape(str) -> str
String
...
[*CommonJS entry points:*](#commonjs)
```js
core-js(/library)/core/rege***REMOVED***p
core-js(/library)/core/string
core-js(/library)/fn/rege***REMOVED***p/escape
...
```
[*E***REMOVED***amples*](http://goo.gl/6bOvsQ):
```js
RegE***REMOVED***p.escape('Hello, []{}()*+?.\\^$|!'); // => 'Hello, \[\]\{\}\(\)\*\+\?\.\\\^\$\|!'
...
```
[*E***REMOVED***amples*](http://goo.gl/lbucba):
```js
...
```
[*E***REMOVED***amples*](http://goo.gl/SXsM6D):
```js
var list = (function(){
  return arguments;
})(1, 2, 3);
...
console.log(iter.ne***REMOVED***t().value); // 1
console.log(iter.ne***REMOVED***t().value); // 2
console.log(iter.ne***REMOVED***t().value); // 3
console.log(iter.ne***REMOVED***t().value); // undefined
...
## Missing polyfills
- ES5 `JSON` is missing now only in IE7- and never will it be added to `core-js`, if you need it in these old browsers, many implementations are available, for e***REMOVED***ample, [json3](https://github.com/bestiejs/json3).
...
- ES6 `Pro***REMOVED***y` can't be polyfilled, but for Node.js / Chromium with additional flags you can try [harmony-reflect](https://github.com/tvcutsem/harmony-reflect) for adapt old style `Pro***REMOVED***y` API to final ES6 version.
...
- `window.fetch` is not a cross-platform feature, in some environments it makes no sense. For this reason, I don't think it should be in `core-js`. Looking at a large number of requests it *may be*  added in the future. Now you can use, for e***REMOVED***ample, [this polyfill](https://github.com/github/fetch).
