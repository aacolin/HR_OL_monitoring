...
try {
  wasm = new WebAssembly.Instance(
    ...
    {}
  ).e***REMOVED***ports as unknown as LongWASMHelpers;
} catch {
  // no wasm support
}
...
/** @public */
e***REMOVED***port interface LongE***REMOVED***tended {
  $numberLong: string;
}
 ...
 */
e***REMOVED***port class Long e***REMOVED***tends BSONValue {
  get _bsontype(): 'Long' {
    return 'Long';
  }
  ...
  static MAX_UNSIGNED_VALUE = Long.fromBits(0***REMOVED***ffffffff | 0, 0***REMOVED***ffffffff | 0, true);
  /** Signed zero */
  ...
  static MAX_VALUE = Long.fromBits(0***REMOVED***ffffffff | 0, 0***REMOVED***7fffffff | 0, false);
  /** Minimum signed value. */
  static MIN_VALUE = Long.fromBits(0, 0***REMOVED***80000000 | 0, false);
  ...
  static fromBigInt(value: bigint, unsigned?: boolean): Long {
    // eslint-disable-ne***REMOVED***t-line no-restricted-globals
   ...
   * @returns The corresponding Long value
   */
  private static _fromString(str: string, unsigned: boolean, radi***REMOVED***: number): Long {
    if (str.length === 0) throw new BSONError('empty string');
    if (radi***REMOVED*** < 2 || 36 < radi***REMOVED***) throw new BSONError('radi***REMOVED***');

    let p;
    if ((p = str.inde***REMOVED***Of('-')) > 0) throw new BSONError('interior hyphen');
    else if (p === 0) {
      return Long._fromString(str.substring(1), unsigned, radi***REMOVED***).neg();
    }

    // Do several (8) digits each time through the loop, so as to
    // minimize the calls to the very e***REMOVED***pensive emulated div.
    const radi***REMOVED***ToPower = Long.fromNumber(Math.pow(radi***REMOVED***, 8));

    let result = Long.ZERO;
    for (let i = 0; i < str.length; i += 8) {
      const size = Math.min(8, str.length - i),
        value = parseInt(str.substring(i, i + size), radi***REMOVED***);
      if (size < 8) {
  ...
  static fromStringStrict(str: string, unsignedOrRadi***REMOVED***?: boolean | number, radi***REMOVED***?: number): Long {
    let unsigned = false;
    ...
    if (!StringUtils.validateStringCharacters(str, radi***REMOVED***)) {
    ...
    const cleanedStr = StringUtils.removeLeadingZerosAndE***REMOVED***plicitPlus(str);
    ...
    if (result.toString(radi***REMOVED***).toLowerCase() !== cleanedStr.toLowerCase()) {
      throw new BSONError(
        `Input: ${str} is not representable as ${result.unsigned ? 'an unsigned' : 'a signed'} 64-bit Long ${radi***REMOVED*** != null ? `with radi***REMOVED***: ${radi***REMOVED***}` : ''}`
      );
    }
    return result;
  }
  ...
  static fromString(str: string, unsignedOrRadi***REMOVED***?: boolean | number, radi***REMOVED***?: number): Long {
    let unsigned = false;
    if (typeof unsignedOrRadi***REMOVED*** === 'number') {
      // For goog.math.long compatibility
      (radi***REMOVED*** = unsignedOrRadi***REMOVED***), (unsignedOrRadi***REMOVED*** = false);
    ...
    if (str === 'NaN' && radi***REMOVED*** < 24) {
      // radi***REMOVED*** does not support n, so coerce to zero
      return Long.ZERO;
    } else if ((str === 'Infinity' || str === '+Infinity' || str === '-Infinity') && radi***REMOVED*** < 35) {
      // radi***REMOVED*** does not support y, so coerce to zero
      return Long.ZERO;
    }
  ...
  divide(divisor: string | number | Long | Timestamp): Long {
    ...
    // floating-point that appro***REMOVED***imates remainder / other *from below*, add this
    // into the result, and subtract it from the remainder.  It is critical that
    // the appro***REMOVED***imate value is less than or equal to the real value so that the
    // remainder never becomes negative.
    // eslint-disable-ne***REMOVED***t-line @typescript-eslint/no-this-alias
    rem = this;
    while (rem.gte(divisor)) {
      // Appro***REMOVED***imate the result of division. This may be a little greater or
      // smaller than the actual value.
      appro***REMOVED*** = Math.ma***REMOVED***(1, Math.floor(rem.toNumber() / divisor.toNumber()));

      // We will tweak the appro***REMOVED***imate result by changing it in the 48-th digit or
      // the smallest non-fractional digit, whichever is larger.
      const log2 = Math.ceil(Math.log(appro***REMOVED***) / Math.LN2);
      const delta = log2 <= 48 ? 1 : Math.pow(2, log2 - 48);
      // Decrease the appro***REMOVED***imation until it is smaller than the remainder.  Note
      ...
      if (appro***REMOVED***Res.isZero()) appro***REMOVED***Res = Long.ONE;
  ...
  multiply(multiplier: string | number | Long | Timestamp): Long {
    ...
    // Divide each long into 4 chunks of 16 bits, and then add up 4***REMOVED***4 products.
  ...
  toBigInt(): bigint {
    // eslint-disable-ne***REMOVED***t-line no-restricted-globals -- This is allowed here as it is e***REMOVED***plicitly requesting a bigint
  ...
  toString(radi***REMOVED***?: number): string {
    ...
    if (this.isNegative()) {
      ...
      if (this.eq(Long.MIN_VALUE)) {
        ...
        const radi***REMOVED***Long = Long.fromNumber(radi***REMOVED***),
          div = this.div(radi***REMOVED***Long),
          rem1 = div.mul(radi***REMOVED***Long).sub(this);
        return div.toString(radi***REMOVED***) + rem1.toInt().toString(radi***REMOVED***);
      } else return '-' + this.neg().toString(radi***REMOVED***);
    }
  ...
  ***REMOVED***or(other: Long | number | string): Long {
  ...
  toE***REMOVED***tendedJSON(options?: EJSONOptions): number | LongE***REMOVED***tended {
    if (options && options.rela***REMOVED***ed) return this.toNumber();
