 ...
 */
e***REMOVED***port type BSONElement = [
  type: number,
  nameOffset: number,
  nameLength: number,
  offset: number,
 ...
 * **Does not** bounds check since this should **ONLY** be used within parseToElements which has asserted that `bytes` ends with a `0***REMOVED***00`.
 ...
 * @e***REMOVED***perimental
 */
e***REMOVED***port function parseToElements(
  bytes: Uint8Array,
  ...
  while (offset <= documentSize + startOffset) {
    ...
    if (
      ...
      type === BSONElementType.ma***REMOVED***Key ||
      type === BSONElementType.minKey
    ) {
      length = 0;
    }
    // Needs a size calculation
    else if (type === BSONElementType.rege***REMOVED***) {
      ...
      throw new BSONOffsetError(
        `Invalid 0***REMOVED***${type.toString(16).padStart(2, '0')} type byte`,
        offset
      );
    }
