...
e***REMOVED***ports.ClientBulkWriteCursorResponse = e***REMOVED***ports.E***REMOVED***plainedCursorResponse = e***REMOVED***ports.CursorResponse = e***REMOVED***ports.MongoDBResponse = void 0;
e***REMOVED***ports.isErrorResponse = isErrorResponse;
 ...
 * that e***REMOVED***pect the result to be a success and demand certain properties to e***REMOVED***ist.
 *
 * For e***REMOVED***ample, a cursor response always e***REMOVED***pects a cursor embedded document.
...
function isErrorResponse(bson, elements) {
    for (let eId***REMOVED*** = 0; eId***REMOVED*** < elements.length; eId***REMOVED***++) {
        const element = elements[eId***REMOVED***];
...
/** @internal */
class MongoDBResponse e***REMOVED***tends document_1.OnDemandDocument {
    get(name, as, required) {
        try {
            return super.get(name, as, required);
        }
        catch (cause) {
            throw new error_1.MongoUne***REMOVED***pectedServerResponseError(cause.message, { cause });
        }
    }
    static is(value) {
    ...
    get $clusterTime() {
        if (!('clusterTime' in this)) {
            ...
            // @ts-e***REMOVED***pect-error: `signature` is incorrectly typed. It is public API.
            this.clusterTime = { clusterTime, signature };
        }
        return this.clusterTime ?? null;
    }
    toObject(options) {
        const e***REMOVED***actBSONOptions = {
            ...(0, bson_1.pluckBSONSerializeOptions)(options ?? {}),
            validation: (0, bson_1.parseUtf8ValidationOption)(options)
        };
        return super.toObject(e***REMOVED***actBSONOptions);
    }
}
e***REMOVED***ports.MongoDBResponse = MongoDBResponse;
// {ok:1}
MongoDBResponse.empty = new MongoDBResponse(new Uint8Array([13, 0, 0, 0, 16, 111, 107, 0, 1, 0, 0, 0, 0]));
/** @internal */
class CursorResponse e***REMOVED***tends MongoDBResponse {
    constructor() {
        super(...arguments);
        this._batch = null;
        this.iterated = 0;
    ...
    get length() {
        return Math.ma***REMOVED***(this.batchSize - this.iterated, 0);
    }
    get encryptedBatch() {
        ...
        else if (cursor?.has('ne***REMOVED***tBatch'))
            this._encryptedBatch = cursor.get('ne***REMOVED***tBatch', bson_1.BSONType.array, true);
        else
            throw new error_1.MongoUne***REMOVED***pectedServerResponseError('Cursor document did not contain a batch');
        return this._encryptedBatch;
    }
    get batch() {
        if (this._batch != null)
        ...
        if (cursor.has('firstBatch'))
            ...
            this._batch = cursor.get('ne***REMOVED***tBatch', bson_1.BSONType.array, true);
        else
            throw new error_1.MongoUne***REMOVED***pectedServerResponseError('Cursor document did not contain a batch');
        return this._batch;
    }
...
e***REMOVED***ports.CursorResponse = CursorResponse;
/**
 * This supports a feature of the FindCursor.
 * It is an optimization to avoid an e***REMOVED***tra getMore when the limit has been reached
 */
...
/**
 * E***REMOVED***plain responses have nothing to do with cursor responses
 * This class serves to temporarily avoid refactoring how cursors handle
 * e***REMOVED***plain responses which is to detect that the response is not cursor-like and return the e***REMOVED***plain
 * result as the "first and only" document in the "batch" and end the "cursor"
 */
class E***REMOVED***plainedCursorResponse e***REMOVED***tends CursorResponse {
    constructor() {
        super(...arguments);
        this.isE***REMOVED***plain = true;
        this._length = 1;
    }
    get id() {
...
e***REMOVED***ports.E***REMOVED***plainedCursorResponse = E***REMOVED***plainedCursorResponse;
/**
 * Client bulk writes have some e***REMOVED***tra metadata at the top level that needs to be
 * included in the result returned to the user.
 */
class ClientBulkWriteCursorResponse e***REMOVED***tends CursorResponse {
    get insertedCount() {
...
e***REMOVED***ports.ClientBulkWriteCursorResponse = ClientBulkWriteCursorResponse;
