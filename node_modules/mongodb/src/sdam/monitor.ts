...
/** @public */
e***REMOVED***port const ServerMonitoringMode = Object.freeze({
  auto: 'auto',
  poll: 'poll',
  stream: 'stream'
} as const);
...
/** @public */
e***REMOVED***port type ServerMonitoringMode = (typeof ServerMonitoringMode)[keyof typeof ServerMonitoringMode];

/** @internal */
e***REMOVED***port interface MonitorPrivate {
  state: string;
}

/** @public */
e***REMOVED***port interface MonitorOptions
  e***REMOVED***tends Omit<ConnectionOptions, 'id' | 'generation' | 'hostAddress'> {
...
/** @public */
e***REMOVED***port type MonitorEvents = {
...
/** @internal */
e***REMOVED***port class Monitor e***REMOVED***tends TypedEventEmitter<MonitorEvents> {
  /** @internal */
  s: MonitorPrivate;
  address: string;
  options: Readonly<
...
function checkServer(monitor: Monitor, callback: Callback<Document | null>) {
  ...
  function onHeartbeatSucceeded(hello: Document) {
    ...
    if (isAwaitable) {
      ...
      // We have not actually sent an outgoing handshake, but when we get the ne***REMOVED***t response we
  ...
  if (connection && !connection.closed) {
    ...
    const ma***REMOVED***AwaitTimeMS = monitor.options.heartbeatFrequencyMS;

    const cmd = {
      [serverApi?.version || helloOk ? 'hello' : LEGACY_HELLO_COMMAND]: 1,
      ...(isAwaitable && topologyVersion
        ? { ma***REMOVED***AwaitTimeMS, topologyVersion: makeTopologyVersion(topologyVersion) }
        : {})
    };

    const options = isAwaitable
      ? {
          socketTimeoutMS: connectTimeoutMS ? connectTimeoutMS + ma***REMOVED***AwaitTimeMS : 0,
          e***REMOVED***haustAllowed: true
        }
    ...
    if (isAwaitable) {
      awaited = true;
      return connection.e***REMOVED***haustCommand(ns('admin.$cmd'), cmd, options, (error, hello) => {
...
function monitorServer(monitor: Monitor) {
  return (callback: Callback) => {
    if (monitor.s.state === STATE_MONITORING) {
      process.ne***REMOVED***tTick(callback);
      return;
    }
...
/** @internal */
e***REMOVED***port interface RTTPingerOptions e***REMOVED***tends ConnectionOptions {
  heartbeatFrequencyMS: number;
}

/** @internal */
e***REMOVED***port class RTTPinger {
 ...
 * @internal
 */
e***REMOVED***port interface MonitorIntervalOptions {
  /** The interval to e***REMOVED***ecute a method on */
  heartbeatFrequencyMS: number;
 ...
 * @internal
 */
e***REMOVED***port class MonitorInterval {
  fn: (callback: Callback) => void;
  timerId: NodeJS.Timeout | undefined;
  lastE***REMOVED***ecutionEnded: number;
  isE***REMOVED***peditedCallToFnScheduled = false;
  stopped = false;
  isE***REMOVED***ecutionInProgress = false;
  hasE***REMOVED***ecutedOnce = false;
  ...
  constructor(fn: (callback: Callback) => void, options: Partial<MonitorIntervalOptions> = {}) {
    this.fn = fn;
    this.lastE***REMOVED***ecutionEnded = -Infinity;

    this.heartbeatFrequencyMS = options.heartbeatFrequencyMS ?? 1000;
    this.minHeartbeatFrequencyMS = options.minHeartbeatFrequencyMS ?? 500;

    if (options.immediate) {
      this._e***REMOVED***ecuteAndReschedule();
    } else {
  ...
  wake() {
    ...
    const timeSinceLastCall = currentTime - this.lastE***REMOVED***ecutionEnded;

    // TODO(NODE-4674): Add error handling and logging to the monitor
    if (timeSinceLastCall < 0) {
      return this._e***REMOVED***ecuteAndReschedule();
    }

    if (this.isE***REMOVED***ecutionInProgress) {
      return;
    }

    // debounce multiple calls to wake within the `minInterval`
    if (this.isE***REMOVED***peditedCallToFnScheduled) {
      return;
    }
    ...
    if (timeSinceLastCall < this.minHeartbeatFrequencyMS) {
      this.isE***REMOVED***peditedCallToFnScheduled = true;
  ...
  stop() {
    ...
    this.lastE***REMOVED***ecutionEnded = -Infinity;
    this.isE***REMOVED***peditedCallToFnScheduled = false;
  }
  ...
  toJSON() {
    ...
    return {
      ...
      lastCallTime: this.lastE***REMOVED***ecutionEnded,
      isE***REMOVED***peditedCheckScheduled: this.isE***REMOVED***peditedCallToFnScheduled,
  ...
  private _reschedule(ms?: number) {
    ...
    this.timerId = setTimeout(this._e***REMOVED***ecuteAndReschedule, ms || this.heartbeatFrequencyMS);
  }

  private _e***REMOVED***ecuteAndReschedule = () => {
    if (this.stopped) return;
    if (this.timerId) {
      clearTimeout(this.timerId);
    }
    ...
    this.isE***REMOVED***peditedCallToFnScheduled = false;
    this.isE***REMOVED***ecutionInProgress = true;

    this.fn(() => {
      this.lastE***REMOVED***ecutionEnded = now();
      this.isE***REMOVED***ecutionInProgress = false;
 ...
 * */
e***REMOVED***port class RTTSampler {
  /** Inde***REMOVED*** of the ne***REMOVED***t slot to be overwritten */
  private writeInde***REMOVED***: number;
  ...
  addSample(sample: number) {
    this.rttSamples[this.writeInde***REMOVED***++] = sample;
    if (this.length < this.rttSamples.length) {
      this.length++;
    }

    this.writeInde***REMOVED*** %= this.rttSamples.length;
  }
  ...
  get last(): number | null {
    ...
    return this.rttSamples[this.writeInde***REMOVED*** === 0 ? this.length - 1 : this.writeInde***REMOVED*** - 1];
  }
