...
import { CreateInde***REMOVED***esOperation } from './inde***REMOVED***es';
 ...
 */
e***REMOVED***port interface TimeSeriesCollectionOptions e***REMOVED***tends Document {
  timeField: string;
  metaField?: string;
 ...
 */
e***REMOVED***port interface ClusteredCollectionOptions e***REMOVED***tends Document {
  name?: string;
  key: Document;
  unique: boolean;
}
...
/** @public */
e***REMOVED***port interface CreateCollectionOptions e***REMOVED***tends CommandOperationOptions {
...
/** @internal */
e***REMOVED***port class CreateCollectionOperation e***REMOVED***tends CommandOperation<Collection> {
  ...
  override async e***REMOVED***ecute(server: Server, session: ClientSession | undefined): Promise<Collection> {
    const db = this.db;
    ...
    if (encryptedFields) {
      ...
      if (
        !server.loadBalanced &&
        server.description.ma***REMOVED***WireVersion < MIN_SUPPORTED_QE_WIRE_VERSION
      ) {
        throw new MongoCompatibilityError(
          `${INVALID_QE_VERSION} The minimum server version required is ${MIN_SUPPORTED_QE_SERVER_VERSION}`
        );
      }
      // Create au***REMOVED***illiary collections for queryable encryption support.
      ...
      for (const collectionName of [escCollection, ecocCollection]) {
        const createOp = new CreateCollectionOperation(db, collectionName, {
          clusteredInde***REMOVED***: {
            key: { _id: 1 },
            unique: true
          }
        });
        await createOp.e***REMOVED***ecuteWithoutEncryptedFieldsCheck(server, session);
      }
    ...
    const coll = await this.e***REMOVED***ecuteWithoutEncryptedFieldsCheck(server, session);

    if (encryptedFields) {
      // Create the required inde***REMOVED*** for queryable encryption support.
      const createInde***REMOVED***Op = CreateInde***REMOVED***esOperation.fromInde***REMOVED***Specification(
        db,
        name,
        { __safeContent__: 1 },
        {}
      );
      await createInde***REMOVED***Op.e***REMOVED***ecute(server, session);
    }
  ...
  private async e***REMOVED***ecuteWithoutEncryptedFieldsCheck(
    ...
    // otherwise just e***REMOVED***ecute the command
