...
import type { GridFSBucket } from './inde***REMOVED***';

/** @public */
e***REMOVED***port interface GridFSChunk {
  _id: ObjectId;
  files_id: ObjectId;
  n: number;
...
/** @public */
e***REMOVED***port interface GridFSBucketWriteStreamOptions e***REMOVED***tends WriteConcernOptions {
 ...
 * @public
 */
e***REMOVED***port class GridFSBucketWriteStream e***REMOVED***tends Writable {
  bucket: GridFSBucket;
  ...
  constructor(bucket: GridFSBucket, filename: string, options?: GridFSBucketWriteStreamOptions) {
    ...
    if (!this.bucket.s.calledOpenUploadStream) {
      ...
      checkInde***REMOVED***es(this).then(() => {
        this.bucket.s.checkedInde***REMOVED***es = true;
        this.bucket.emit('inde***REMOVED***');
      }, squashError);
    }
  }
   ...
   * @internal
   *
   * The stream is considered constructed when the inde***REMOVED***es are done being created
   */
  override _construct(callback: (error?: Error | null) => void): void {
    if (this.bucket.s.checkedInde***REMOVED***es) {
      return process.ne***REMOVED***tTick(callback);
    }
    this.bucket.once('inde***REMOVED***', callback);
  }
...
function handleError(stream: GridFSBucketWriteStream, error: Error, callback: Callback): void {
  ...
  process.ne***REMOVED***tTick(callback, error);
}
...
async function checkChunksInde***REMOVED***(stream: GridFSBucketWriteStream): Promise<void> {
  const inde***REMOVED*** = { files_id: 1, n: 1 };

  let inde***REMOVED***es;
  try {
    inde***REMOVED***es = await stream.chunks.listInde***REMOVED***es().toArray();
  } catch (error) {
  ...
  const hasChunksInde***REMOVED*** = !!inde***REMOVED***es.find(inde***REMOVED*** => {
    const keys = Object.keys(inde***REMOVED***.key);
    if (keys.length === 2 && inde***REMOVED***.key.files_id === 1 && inde***REMOVED***.key.n === 1) {
      return true;
    }
    return false;
  });

  if (!hasChunksInde***REMOVED***) {
    await stream.chunks.createInde***REMOVED***(inde***REMOVED***, {
      ...stream.writeConcern,
      background: true,
      unique: true
    });
...
async function checkInde***REMOVED***es(stream: GridFSBucketWriteStream): Promise<void> {
  const doc = await stream.files.findOne({}, { projection: { _id: 1 } });
  if (doc != null) {
    // If at least one document e***REMOVED***ists assume the collection has the required inde***REMOVED***
    return;
  }

  const inde***REMOVED*** = { filename: 1, uploadDate: 1 };

  let inde***REMOVED***es;
  try {
    inde***REMOVED***es = await stream.files.listInde***REMOVED***es().toArray();
  } catch (error) {
    if (error instanceof MongoError && error.code === MONGODB_ERROR_CODES.NamespaceNotFound) {
      inde***REMOVED***es = [];
    } else {
  ...
  const hasFileInde***REMOVED*** = !!inde***REMOVED***es.find(inde***REMOVED*** => {
    const keys = Object.keys(inde***REMOVED***.key);
    if (keys.length === 2 && inde***REMOVED***.key.filename === 1 && inde***REMOVED***.key.uploadDate === 1) {
      return true;
    }
    return false;
  });

  if (!hasFileInde***REMOVED***) {
    await stream.files.createInde***REMOVED***(inde***REMOVED***, { background: false });
  }

  await checkChunksInde***REMOVED***(stream);
}
...
function isAborted(stream: GridFSBucketWriteStream, callback: Callback<void>): boolean {
  if (stream.state.aborted) {
    process.ne***REMOVED***tTick(callback, new MongoAPIError('Stream has been aborted'));
    return true;
  }
  return false;
}
