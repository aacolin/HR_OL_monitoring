...
/** @public */
e***REMOVED***port interface GridFSBucketReadStreamOptions {
  sort?: Sort;
  skip?: number;
  /**
...
/** @public */
e***REMOVED***port interface GridFSBucketReadStreamOptionsWithRevision e***REMOVED***tends GridFSBucketReadStreamOptions {
...
/** @public */
e***REMOVED***port interface GridFSFile {
  _id: ObjectId;
  length: number;
  chunkSize: number;
  filename: string;
...
/** @internal */
e***REMOVED***port interface GridFSBucketReadStreamPrivate {
  /**
  ...
  /** The e***REMOVED***pected number of chunks to read, calculated from start, end, chunkSize and file length. */
  e***REMOVED***pectedEnd: number;
  file?: GridFSFile;
  options: {
    sort?: Sort;
    skip?: number;
 ...
 * @public
 */
e***REMOVED***port class GridFSBucketReadStream e***REMOVED***tends Readable {
  /** @internal */
...
function doRead(stream: GridFSBucketReadStream): void {
  ...
  const handleReadResult = (doc: Document | null) => {
    ...
    const e***REMOVED***pectedN = stream.s.e***REMOVED***pected++;
    const e***REMOVED***pectedLength = Math.min(stream.s.file.chunkSize, bytesRemaining);
    if (doc.n > e***REMOVED***pectedN) {
      return stream.destroy(
        new MongoGridFSChunkError(
          `ChunkIsMissing: Got une***REMOVED***pected n: ${doc.n}, e***REMOVED***pected: ${e***REMOVED***pectedN}`
        )
      );
    }
    ...
    if (doc.n < e***REMOVED***pectedN) {
      return stream.destroy(
        new MongoGridFSChunkError(`E***REMOVED***traChunk: Got une***REMOVED***pected n: ${doc.n}, e***REMOVED***pected: ${e***REMOVED***pectedN}`)
      );
    }

    let buf = Buffer.isBuffer(doc.data) ? doc.data : doc.data.buffer;

    if (buf.byteLength !== e***REMOVED***pectedLength) {
      if (bytesRemaining <= 0) {
        return stream.destroy(
          new MongoGridFSChunkError(
            `E***REMOVED***traChunk: Got une***REMOVED***pected n: ${doc.n}, e***REMOVED***pected file length ${stream.s.file.length} bytes but already read ${stream.s.bytesRead} bytes`
          )
        );
      }
      ...
      return stream.destroy(
        new MongoGridFSChunkError(
          `ChunkIsWrongSize: Got une***REMOVED***pected length: ${buf.byteLength}, e***REMOVED***pected: ${e***REMOVED***pectedLength}`
        )
      );
    }
    ...
    const atEndOfStream = e***REMOVED***pectedN === stream.s.e***REMOVED***pectedEnd - 1;
  ...
  stream.s.cursor.ne***REMOVED***t().then(handleReadResult, error => {
    if (stream.destroyed) return;
...
function init(stream: GridFSBucketReadStream): void {
  ...
  const handleReadResult = (doc: Document | null) => {
    ...
    // Currently (MongoDB 3.4.4) skip function does not support the inde***REMOVED***,
    ...
    stream.s.e***REMOVED***pectedEnd = Math.ceil(doc.length / doc.chunkSize);
...
function handleStartOption(
  ...
  if (options && options.start != null) {
    ...
    stream.s.e***REMOVED***pected = Math.floor(options.start / doc.chunkSize);
...
function handleEndOption(
  ...
  if (options && options.end != null) {
    ...
    stream.s.e***REMOVED***pectedEnd = Math.ceil(options.end / doc.chunkSize);
