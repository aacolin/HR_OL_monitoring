 ...
 */
e***REMOVED***port async function resolveSRVRecord(options: MongoOptions): Promise<HostAddress[]> {
  ...
  t***REMOVED***tResolutionPromise.then(undefined, squashError); // rejections will be handled later
  ...
  if (record.length > 1) {
    throw new MongoParseError('Multiple te***REMOVED***t records not allowed');
  }

  const t***REMOVED***tRecordOptions = new URLSearchParams(record[0].join(''));
  ...
  if (t***REMOVED***tRecordOptionKeys.some(key => !VALID_TXT_RECORDS.includes(key))) {
    throw new MongoParseError(`Te***REMOVED***t record may only set any of: ${VALID_TXT_RECORDS.join(', ')}`);
  }
  ...
  if (options.replicaSet && options.srvMa***REMOVED***Hosts > 0) {
    throw new MongoParseError('Cannot combine replicaSet option with srvMa***REMOVED***Hosts');
  }
...
function getIntFromOptions(name: string, value: unknown): number {
  ...
  throw new MongoParseError(`E***REMOVED***pected ${name} to be stringified int value, got: ${value}`);
}
...
class CaseInsensitiveMap<Value = any> e***REMOVED***tends Map<string, Value> {
...
e***REMOVED***port function parseOptions(
  uri: string,
  ...
  if (mongoOptions.credentials) {
    ...
    if (
      ...
      throw new MongoParseError(
        `authMechanism ${mongoOptions.credentials.mechanism} requires an authSource of '$e***REMOVED***ternal'`
      );
    }
  ...
  if (isSRV) {
    ...
    if (mongoOptions.srvMa***REMOVED***Hosts > 0 && typeof mongoOptions.replicaSet === 'string') {
      throw new MongoParseError('Cannot use srvMa***REMOVED***Hosts option with replicaSet');
    }
    ...
    const userSpecifiedSrvOptions =
      urlOptions.has('srvMa***REMOVED***Hosts') ||
    ...
    if (userSpecifiedSrvOptions) {
      throw new MongoParseError(
        'Cannot use srvMa***REMOVED***Hosts or srvServiceName with a non-srv connection string'
      );
    }
  }

  if (mongoOptions.directConnection && mongoOptions.hosts.length !== 1) {
    throw new MongoParseError('directConnection option requires e***REMOVED***actly one host');
  }

  if (
    !mongoOptions.pro***REMOVED***yHost &&
    (mongoOptions.pro***REMOVED***yPort || mongoOptions.pro***REMOVED***yUsername || mongoOptions.pro***REMOVED***yPassword)
  ) {
    throw new MongoParseError('Must specify pro***REMOVED***yHost if other pro***REMOVED***y options are passed');
  }
  ...
  if (
    ...
    throw new MongoParseError('Can only specify both of pro***REMOVED***y username/password or neither');
  }

  const pro***REMOVED***yOptions = ['pro***REMOVED***yHost', 'pro***REMOVED***yPort', 'pro***REMOVED***yUsername', 'pro***REMOVED***yPassword'].map(
    key => urlOptions.get(key) ?? []
  );

  if (pro***REMOVED***yOptions.some(options => options.length > 1)) {
    throw new MongoParseError(
      'Pro***REMOVED***y options cannot be specified multiple times in the connection string'
    );
  }
  ...
  mongoOptions.e***REMOVED***tendedMetadata = addContainerMetadata(mongoOptions.metadata).then(
    undefined,
    squashError
...
e***REMOVED***port const OPTIONS = {
  ...
  ma***REMOVED***Connecting: {
    ...
    transform({ name, values: [value] }): number {
      ...
      if (ma***REMOVED***Connecting === 0) {
        throw new MongoInvalidArgumentError('ma***REMOVED***Connecting must be > 0 if specified');
      }
  ...
  ma***REMOVED***StalenessSeconds: {
    ...
    transform({ name, options, values: [value] }) {
      ...
      if (options.readPreference) {
        ...
        return new ReadPreference('secondary', undefined, { ma***REMOVED***StalenessSeconds });
      }
    }
  },
  ...
  useNewUrlParser: {
    type: 'boolean',
    deprecated:
      'useNewUrlParser has no effect since Node.js Driver version 4.0.0 and will be removed in the ne***REMOVED***t major version'
  } as OptionDescriptor,
  useUnifiedTopology: {
    type: 'boolean',
    deprecated:
      'useUnifiedTopology has no effect since Node.js Driver version 4.0.0 and will be removed in the ne***REMOVED***t major version'
  } as OptionDescriptor,
  // MongoLogger
  /**
   * @internal
...
e***REMOVED***port const DEFAULT_OPTIONS = new CaseInsensitiveMap(
 ...
 * @internal
 */
e***REMOVED***port const FEATURE_FLAGS = new Set([
