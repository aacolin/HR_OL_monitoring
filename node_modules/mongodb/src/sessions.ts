...
/** @public */
e***REMOVED***port interface ClientSessionOptions {
...
/** @public */
e***REMOVED***port type WithTransactionCallback<T = any> = (session: ClientSession) => Promise<T>;

/** @public */
e***REMOVED***port type ClientSessionEvents = {
...
/** @public */
e***REMOVED***port interface EndSessionOptions {
  /**
 ...
 * @public
 */
e***REMOVED***port class ClientSession
  e***REMOVED***tends TypedEventEmitter<ClientSessionEvents>
  implements AsyncDisposable
{
  ...
  /** @internal */
  [kT***REMOVED***nNumberIncrement]: number;
  /** @internal */
  timeoutMS?: number;
    ...
    if (options.snapshot === true) {
      ...
      if (options.causalConsistency === true) {
        throw new MongoInvalidArgumentError(
          'Properties "causalConsistency" and "snapshot" are mutually e***REMOVED***clusive'
        );
      }
    }
  ...
  get serverSession(): ServerSession {
    ...
    if (serverSession == null) {
      if (this.e***REMOVED***plicit) {
        throw new MongoRuntimeError('Une***REMOVED***pected null serverSession for an e***REMOVED***plicit session');
      }
      if (this.hasEnded) {
        throw new MongoRuntimeError('Une***REMOVED***pected null serverSession for an ended implicit session');
      }
  ...
  startTransaction(options?: TransactionOptions): void {
    ...
    const topologyMa***REMOVED***WireVersion = ma***REMOVED***WireVersion(this.client.topology);
    if (
    ...
    this.transaction.transition(T***REMOVED***nState.STARTING_TRANSACTION);
  }
  ...
  async commitTransaction(): Promise<void> {
    if (this.transaction.state === T***REMOVED***nState.NO_TRANSACTION) {
      throw new MongoTransactionError('No transaction started');
    }

    if (
      this.transaction.state === T***REMOVED***nState.STARTING_TRANSACTION ||
      this.transaction.state === T***REMOVED***nState.TRANSACTION_COMMITTED_EMPTY
    ) {
      // the transaction was never started, we can safely e***REMOVED***it here
      this.transaction.transition(T***REMOVED***nState.TRANSACTION_COMMITTED_EMPTY);
      return;
    }

    if (this.transaction.state === T***REMOVED***nState.TRANSACTION_ABORTED) {
    ...
    if (this.transaction.state === T***REMOVED***nState.TRANSACTION_COMMITTED || this.commitAttempted) {
    ...
    try {
      await e***REMOVED***ecuteOperation(this.client, operation);
      ...
      this.transaction.transition(T***REMOVED***nState.TRANSACTION_COMMITTED);
    }
  }
  ...
  async abortTransaction(): Promise<void> {
    ...
    if (this.transaction.state === T***REMOVED***nState.STARTING_TRANSACTION) {
      // the transaction was never started, we can safely e***REMOVED***it here
      this.transaction.transition(T***REMOVED***nState.TRANSACTION_ABORTED);
      return;
    }
    ...
    if (
      this.transaction.state === T***REMOVED***nState.TRANSACTION_COMMITTED ||
      this.transaction.state === T***REMOVED***nState.TRANSACTION_COMMITTED_EMPTY
    ) {
      throw new MongoTransactionError(
  ...
  async withTransaction<T = any>(
    ...
    while (!committed) {
      ...
      try {
        ...
        if (
          this.transaction.state === T***REMOVED***nState.NO_TRANSACTION ||
          this.transaction.state === T***REMOVED***nState.TRANSACTION_COMMITTED ||
          this.transaction.state === T***REMOVED***nState.TRANSACTION_ABORTED
        ) {
        ...
        if (
          this.transaction.state === T***REMOVED***nState.STARTING_TRANSACTION ||
          this.transaction.state === T***REMOVED***nState.TRANSACTION_IN_PROGRESS
        ) {
          await this.abortTransaction();
        }
...
function isUnknownTransactionCommitResult(err: MongoError): err is MongoError {
  ...
  return (
    isMa***REMOVED***TimeMSE***REMOVED***piredError(err) ||
...
e***REMOVED***port function maybeClearPinnedConnection(
  ...
  if (conn && topology != null) {
    ...
    if (options?.error == null || options?.force) {
      ...
      conn.emit(
        UNPINNED,
        session.transaction.state !== T***REMOVED***nState.NO_TRANSACTION
...
function isMa***REMOVED***TimeMSE***REMOVED***piredError(err: MongoError): boolean {
  if (err == null || !(err instanceof MongoServerError)) {
    return false;
  }

  return (
    err.code === MONGODB_ERROR_CODES.Ma***REMOVED***TimeMSE***REMOVED***pired ||
    err.writeConcernError?.code === MONGODB_ERROR_CODES.Ma***REMOVED***TimeMSE***REMOVED***pired
  );
}

/** @public */
e***REMOVED***port type ServerSessionId = { id: Binary };
 ...
 * @public
 */
e***REMOVED***port class ServerSession {
  id: ServerSessionId;
  lastUse: number;
 ...
 * @internal
 */
e***REMOVED***port class ServerSessionPool {
  client: MongoClient;
 ...
 * @internal
 */
e***REMOVED***port function applySession(
  session: ClientSession,
  command: Document,
  options: CommandOptions
): MongoDriverError | undefined {
  if (session.hasEnded) {
    return new MongoE***REMOVED***piredSessionError();
  }
  ...
  if (options.writeConcern?.w === 0) {
    if (session && session.e***REMOVED***plicit) {
      ...
      return new MongoAPIError('Cannot have e***REMOVED***plicit session with unacknowledged writes');
    }
    return;
  }
  ...
  const inT***REMOVED***nOrT***REMOVED***nCommand = session.inTransaction() || isTransactionCommand(command);
  ...
  if (isRetryableWrite || inT***REMOVED***nOrT***REMOVED***nCommand) {
    ...
    command.t***REMOVED***nNumber = Long.fromNumber(serverSession.t***REMOVED***nNumber);
  }

  if (!inT***REMOVED***nOrT***REMOVED***nCommand) {
    if (session.transaction.state !== T***REMOVED***nState.NO_TRANSACTION) {
      session.transaction.transition(T***REMOVED***nState.NO_TRANSACTION);
    }
  ...
  if (session.transaction.state === T***REMOVED***nState.STARTING_TRANSACTION) {
    session.transaction.transition(T***REMOVED***nState.TRANSACTION_IN_PROGRESS);
...
e***REMOVED***port function updateSessionFromResponse(session: ClientSession, document: MongoDBResponse): void {
