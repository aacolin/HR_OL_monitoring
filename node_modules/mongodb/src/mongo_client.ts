...
import { ClientBulkWriteE***REMOVED***ecutor } from './operations/client_bulk_write/e***REMOVED***ecutor';
...
/** @public */
e***REMOVED***port const ServerApiVersion = Object.freeze({
  v1: '1'
} as const);

/** @public */
e***REMOVED***port type ServerApiVersion = (typeof ServerApiVersion)[keyof typeof ServerApiVersion];

/** @public */
e***REMOVED***port interface ServerApi {
...
/** @public */
e***REMOVED***port interface DriverInfo {
  name?: string;
  version?: string;
  platform?: string;
}
...
/** @public */
e***REMOVED***port interface PkFactory {
  createPk(): any;
}

/** @public */
e***REMOVED***port type SupportedTLSConnectionOptions = Pick<
...
/** @public */
e***REMOVED***port type SupportedTLSSocketOptions = Pick<
  TLSSocketOptions,
  E***REMOVED***tract<keyof TLSSocketOptions, (typeof LEGAL_TLS_SOCKET_OPTIONS)[number]>
>;

/** @public */
e***REMOVED***port type SupportedSocketOptions = Pick<
  TcpNetConnectOpts & { autoSelectFamily?: boolean; autoSelectFamilyAttemptTimeout?: number },
  (typeof LEGAL_TCP_SOCKET_OPTIONS)[number]
>;

/** @public */
e***REMOVED***port type SupportedNodeConnectionOptions = SupportedTLSConnectionOptions &
 ...
 */
e***REMOVED***port interface MongoClientOptions e***REMOVED***tends BSONSerializeOptions, SupportedNodeConnectionOptions {
   ...
   * TODO: NODE-5671 - remove internal flag
   */
  mongodbLogMa***REMOVED***DocumentLength?: number;
...
/** @public */
e***REMOVED***port type WithSessionCallback<T = unknown> = (session: ClientSession) => Promise<T>;

/** @internal */
e***REMOVED***port interface MongoClientPrivate {
  url: string;
   ...
   * - used to notify the leak checker in our tests if test author forgot to clean up e***REMOVED***plicit sessions
   */
...
/** @public */
e***REMOVED***port type MongoClientEvents = Pick<TopologyEvents, (typeof MONGO_CLIENT_EVENTS)[number]> & {
  // In previous versions the open event emitted a topology, in an effort to no longer
  // e***REMOVED***pose internals but continue to e***REMOVED***pose this useful event API, it now emits a mongoClient
 ...
 * ```
 */
e***REMOVED***port class MongoClient e***REMOVED***tends TypedEventEmitter<MongoClientEvents> implements AsyncDisposable {
  /** @internal */
   ...
   */
  async bulkWrite<SchemaMap e***REMOVED***tends Record<string, Document> = Record<string, Document>>(
    ...
    return await new ClientBulkWriteE***REMOVED***ecutor(
      this,
      models as any,
  ...
  private async _connect(): Promise<this> {
    ...
    if (typeof options.srvHost === 'string') {
      ...
      for (const [inde***REMOVED***, host] of hosts.entries()) {
        options.hosts[inde***REMOVED***] = host;
      }
    }
   ...
   */
  async withSession<T = any>(e***REMOVED***ecutor: WithSessionCallback<T>): Promise<T>;
  async withSession<T = any>(
    options: ClientSessionOptions,
    e***REMOVED***ecutor: WithSessionCallback<T>
  ): Promise<T>;
  async withSession<T = any>(
    optionsOrE***REMOVED***ecutor: ClientSessionOptions | WithSessionCallback<T>,
    e***REMOVED***ecutor?: WithSessionCallback<T>
  ): Promise<T> {
    const options = {
      ...
      ...(typeof optionsOrE***REMOVED***ecutor === 'object' ? optionsOrE***REMOVED***ecutor : {})
    };

    const withSessionCallback =
      typeof optionsOrE***REMOVED***ecutor === 'function' ? optionsOrE***REMOVED***ecutor : e***REMOVED***ecutor;
 ...
 * @public
 */
e***REMOVED***port interface MongoOptions
  e***REMOVED***tends Required<
      Pick<
        MongoClientOptions,
        | 'autoEncryption'
        | 'connectTimeoutMS'
