...
e***REMOVED***port const HUMAN_TIMEOUT_MS = 300000;
/** 1 minute in milliseconds */
e***REMOVED***port const AUTOMATED_TIMEOUT_MS = 60000;

/** Properties allowed on results of callbacks. */
const RESULT_PROPERTIES = ['accessToken', 'e***REMOVED***piresInSeconds', 'refreshToken'];
 ...
 * @internal
 */
e***REMOVED***port abstract class CallbackWorkflow implements Workflow {
  cache: TokenCache;
  callback: OIDCCallbackFunction;
  lastE***REMOVED***ecutionTime: number;
  ...
  constructor(cache: TokenCache, callback: OIDCCallbackFunction) {
    ...
    this.lastE***REMOVED***ecutionTime = Date.now() - THROTTLE_MS;
  }
  ...
  async reauthenticate(connection: Connection, credentials: MongoCredentials): Promise<void> {
    if (this.cache.hasAccessToken) {
      // Reauthentication implies the token has e***REMOVED***pired.
    ...
    await this.e***REMOVED***ecute(connection, credentials);
  }

  /**
   * E***REMOVED***ecute the OIDC callback workflow.
   */
  abstract e***REMOVED***ecute(
  ...
  /**
   * E***REMOVED***ecutes the callback and validates the output.
   */
  protected async e***REMOVED***ecuteAndValidateCallback(params: OIDCCallbackParams): Promise<OIDCResponse> {
  ...
  /**
   * Ensure the callback is only e***REMOVED***ecuted one at a time and throttles the calls
   * to every 100ms.
   */
  protected withLock(callback: OIDCCallbackFunction): OIDCCallbackFunction {
    ...
    return async (params: OIDCCallbackParams): Promise<OIDCResponse> => {
      ...
      lock = lock
        ...
        .then(async () => {
          const difference = Date.now() - this.lastE***REMOVED***ecutionTime;
          if (difference <= THROTTLE_MS) {
            await setTimeout(THROTTLE_MS - difference, { signal: params.timeoutConte***REMOVED***t });
          }
          this.lastE***REMOVED***ecutionTime = Date.now();
 ...
 * the accessToken required field, and does not contain e***REMOVED***tra fields.
 */
