 ...
 * @internal
 */
e***REMOVED***port interface AccessToken {
  access_token: string;
  e***REMOVED***pires_in?: number;
}

/** @internal */
e***REMOVED***port type OIDCTokenFunction = (credentials: MongoCredentials) => Promise<AccessToken>;

/**
 * Common behaviour for OIDC machine workflows.
 * @internal
 */
e***REMOVED***port abstract class MachineWorkflow implements Workflow {
  cache: TokenCache;
  ...
  /**
   * E***REMOVED***ecute the workflow. Gets the token from the subclass implementation.
   */
  async e***REMOVED***ecute(connection: Connection, credentials: MongoCredentials): Promise<void> {
   ...
   * has said the current access token is invalid or e***REMOVED***pired.
   */
  ...
  private async getTokenFromCacheOrEnv(
    ...
    if (this.cache.hasAccessToken) {
      ...
      this.cache.put({ accessToken: token.access_token, e***REMOVED***piresInSeconds: token.e***REMOVED***pires_in });
  ...
  /**
   * Ensure the callback is only e***REMOVED***ecuted one at a time, and throttled to
   * only once per 100ms.
   */
