...
/** @public */
e***REMOVED***port const GSSAPICanonicalizationValue = Object.freeze({
  on: true,
  off: false,
  none: 'none',
  forward: 'forward',
...
/** @public */
e***REMOVED***port type GSSAPICanonicalizationValue =
...
e***REMOVED***port class GSSAPI e***REMOVED***tends AuthProvider {
  override async auth(authConte***REMOVED***t: AuthConte***REMOVED***t): Promise<void> {
    ...
    const client = await makeKerberosClient(authConte***REMOVED***t);

    const payload = await client.step('');

    const saslStartResponse = await e***REMOVED***ternalCommand(connection, saslStart(payload));

    const negotiatedPayload = await negotiate(client, 10, saslStartResponse.payload);

    const saslContinueResponse = await e***REMOVED***ternalCommand(
      connection,
    ...
    await e***REMOVED***ternalCommand(connection, {
      saslContinue: 1,
...
async function makeKerberosClient(authConte***REMOVED***t: AuthConte***REMOVED***t): Promise<KerberosClient> {
  const { hostAddress } = authConte***REMOVED***t.options;
...
async function negotiate(
  ...
  try {
    ...
    if (retries === 0) {
      // Retries e***REMOVED***hausted, raise error
      throw error;
    }
...
e***REMOVED***port async function performGSSAPICanonicalizeHostName(
  host: string,
  ...
  if (
    ...
    try {
      ...
      // This can error as ptr records may not e***REMOVED***ist for all ips. In this case
...
e***REMOVED***port async function resolveCname(host: string): Promise<string> {
