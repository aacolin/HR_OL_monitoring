...
import type { AuthConte***REMOVED***t } from './auth/auth_provider';
...
/** @internal */
e***REMOVED***port interface CommandOptions e***REMOVED***tends BSONSerializeOptions {
  secondaryOk?: boolean;
  ...
  // from e***REMOVED***ecuteOperation that the t***REMOVED***nNum should be applied to this command.
  // Applying a session to a command should happen as part of command construction,
  // most likely in the CommandOperation#e***REMOVED***ecuteCommand method, where we have access to
  // the details we need to determine if a t***REMOVED***nNum should also be applied.
...
/** @public */
e***REMOVED***port interface Pro***REMOVED***yOptions {
...
/** @public */
e***REMOVED***port interface ConnectionOptions
  e***REMOVED***tends SupportedNodeConnectionOptions,
  ...
  /** @internal */
  e***REMOVED***tendedMetadata: Promise<Document>;
  /** @internal */
  mongoLogger?: MongoLogger | undefined;
}

/** @public */
e***REMOVED***port type ConnectionEvents = {
...
/** @internal */
e***REMOVED***port function hasSessionSupport(conn: Connection): boolean {
  const description = conn.description;
  return description.logicalSessionTimeoutMinutes != null;
}

function streamIdentifier(stream: Stream, options: ConnectionOptions): string {
  if (options.pro***REMOVED***yHost) {
    // If pro***REMOVED***y options are specified, the properties of `stream` itself
  ...
  return uuidV4().toString('he***REMOVED***');
}

/** @internal */
e***REMOVED***port class Connection e***REMOVED***tends TypedEventEmitter<ConnectionEvents> {
  public id: number | '<monitor>';
  public address: string;
  public lastHelloMS = -1;
  public serverApi?: ServerApi;
  public helloOk = false;
  public authConte***REMOVED***t?: AuthConte***REMOVED***t;
  ...
  private get shouldEmitAndLogCommand(): boolean {
    return (
      (this.monitorCommands ||
        (this.established &&
          !this.authConte***REMOVED***t?.reauthenticating &&
  ...
  private prepareCommand(db: string, command: Document, options: CommandOptions) {
    ...
    } else if (session?.e***REMOVED***plicit) {
  ...
  private async *sendCommand(
    ...
    // Otherwise, support raw flag. Raw only works for cursors that hardcode firstBatch/ne***REMOVED***tBatch fields
  ...
  public async command<T e***REMOVED***tends MongoDBResponseConstructor>(
    ns: MongoDBNamespace,
    command: Document,
  ...
  public async command(
    ...
    throw new MongoUne***REMOVED***pectedServerResponseError('Unable to get response from server');
  }

  public e***REMOVED***haustCommand(
    ns: MongoDBNamespace,
    command: Document,
    options: CommandOptions,
    replyListener: Callback
  ) {
    const e***REMOVED***haustLoop = async () => {
      this.throwIfAborted();
      for await (const reply of this.sendCommand(ns, command, options)) {
        replyListener(undefined, reply);
        this.throwIfAborted();
      }
      throw new MongoUne***REMOVED***pectedServerResponseError('Server ended moreToCome une***REMOVED***pectedly');
    };

    e***REMOVED***haustLoop().then(undefined, replyListener);
  }
   ...
   *
   * Note that `for-await` loops call `return` automatically when the loop is e***REMOVED***ited.
   */
...
/** @internal */
e***REMOVED***port class SizedMessageTransform e***REMOVED***tends Transform {
...
/** @internal */
e***REMOVED***port class CryptoConnection e***REMOVED***tends Connection {
  /** @internal */
  autoEncrypter?: AutoEncrypter;
  ...
  public override async command<T e***REMOVED***tends MongoDBResponseConstructor>(
    ns: MongoDBNamespace,
    command: Document,
    options: CommandOptions | undefined,
    responseType: T
  ...
  override async command<T e***REMOVED***tends MongoDBResponseConstructor>(
    ns: MongoDBNamespace,
    cmd: Document,
    options?: CommandOptions,
    responseType?: T | undefined
    ...
    if (!autoEncrypter) {
      ...
      // @ts-e***REMOVED***pect-error No cause provided because there is no underlying error.
      throw new MongoMissingDependencyError('No AutoEncrypter available for encryption', {
        dependencyName: 'n/a'
      });
    }

    const serverWireVersion = ma***REMOVED***WireVersion(this);
    ...
    // Save sort or inde***REMOVED***Keys based on the command being run
    // the encrypt API serializes our JS objects to BSON to pass to the native code layer
    // and then deserializes the encrypted result, the protocol level components
    // of the command (e***REMOVED***. sort) are then converted to JS objects potentially losing
    // import key order information. These fields are never encrypted so we can save the values
    // from before the encryption and replace them after encryption has been performed
    const sort: Map<string, number> | null = cmd.find || cmd.findAndModify ? cmd.sort : null;
    const inde***REMOVED***Keys: Map<string, number>[] | null = cmd.createInde***REMOVED***es
      ? cmd.inde***REMOVED***es.map((inde***REMOVED***: { key: Map<string, number> }) => inde***REMOVED***.key)
      : null;
    ...
    if (inde***REMOVED***Keys != null && cmd.createInde***REMOVED***es) {
      for (const [offset, inde***REMOVED***] of inde***REMOVED***Keys.entries()) {
        // @ts-e***REMOVED***pect-error `encrypted` is a generic "command", but we've narrowed for only `createInde***REMOVED***es` commands here
        encrypted.inde***REMOVED***es[offset].key = inde***REMOVED***;
      }
    }
