...
/** @public */
e***REMOVED***port class UnorderedBulkOperation e***REMOVED***tends BulkOperationBase {
  /** @internal */
  ...
  addToOperationsList(
    ...
    if (bsonSize >= this.s.ma***REMOVED***BsonObjectSize) {
      ...
      throw new MongoInvalidArgumentError(
        `Document is larger than the ma***REMOVED***imum size ${this.s.ma***REMOVED***BsonObjectSize}`
      );
    }
    ...
    if (this.s.currentBatch == null) {
      this.s.currentBatch = new Batch(batchType, this.s.currentInde***REMOVED***);
    }

    // Check if we need to create a new batch
    if (
      // New batch if we e***REMOVED***ceed the ma***REMOVED*** batch op size
      this.s.currentBatch.size + 1 >= this.s.ma***REMOVED***WriteBatchSize ||
      // New batch if we e***REMOVED***ceed the ma***REMOVED***BatchSizeBytes. Only matters if batch already has a doc,
      // since we can't sent an empty batch
      (this.s.currentBatch.size > 0 &&
        this.s.currentBatch.sizeBytes + ma***REMOVED***KeySize + bsonSize >= this.s.ma***REMOVED***BatchSizeBytes) ||
    ...
    this.s.currentInde***REMOVED*** = this.s.currentInde***REMOVED*** + 1;

    // Save back the current Batch to the right type
    if (batchType === BatchType.INSERT) {
      this.s.currentInsertBatch = this.s.currentBatch;
      this.s.bulkResult.insertedIds.push({
        inde***REMOVED***: this.s.bulkResult.insertedIds.length,
    ...
    this.s.currentBatch.sizeBytes += ma***REMOVED***KeySize + bsonSize;
