...
const props = {
	...
	clientId: 'client-123',
	type: 'web',
	redirect_uri: 'https://e***REMOVED***ample.com',
	scope: '',
...
class Common {
	static e***REMOVED***pectCredentials({ form }){
		form.username.should.equal(props.username);
		form.password.should.equal(props.password);
	}
	static e***REMOVED***pectDeviceUrlAndToken(results){
...
describe('ParticleAPI', () => {
	...
	describe('operations', () => {
		...
		describe('trackingIdentity', () => {
			const conte***REMOVED***t = { tool: { name: 'cli', version:'1.2.3' } };

			it('full', () => {
				return api.trackingIdentity({ auth: 'X', full: true, conte***REMOVED***t })
					.then((req) => {
						e***REMOVED***pect(req).to.have.property('uri', '/v1/user/identify');
						e***REMOVED***pect(req).to.have.property('method', 'get');
						e***REMOVED***pect(req).to.have.property('query', undefined);
						e***REMOVED***pect(req).to.have.property('conte***REMOVED***t').that.is.eql(conte***REMOVED***t);
						e***REMOVED***pect(req).to.have.property('auth', 'X');
					});
			});

			it('id only', () => {
				return api.trackingIdentity().then(req => {
					e***REMOVED***pect(req).to.have.property('uri').eql('/v1/user/identify');
					e***REMOVED***pect(req).to.have.property('method').eql('get');
					e***REMOVED***pect(req).to.have.property('conte***REMOVED***t').eql({});
					e***REMOVED***pect(req).to.have.property('query').eql({ tracking: 1 });
				});
			});
		});
		...
		describe('.login', () => {
			it('sends credentials', () => {
				return api.login(props).then(Common.e***REMOVED***pectCredentials);
			});
		});
		...
		describe('.loginAsClientOwner', () => {
			it('sends client ID and secret', () => {
				...
				return clientApi.loginAsClientOwner({}).then(req => {
					e***REMOVED***pect(req.form).to.have.property('client_id').eql('foo');
					e***REMOVED***pect(req.form).to.have.property('client_secret').eql('bar');
					e***REMOVED***pect(req.form).to.have.property('grant_type').eql('client_credentials');
				});
			});
		});
		...
		describe('.sendOtp', () => {
			it('sends request to oauth token endpoint to verify the user login', () => {
				return api.sendOtp(props).then((results) => {
					e***REMOVED***pect(results).to.have.property('method').eql('post');
					e***REMOVED***pect(results).to.have.property('uri').eql('/oauth/token');
					e***REMOVED***pect(results.form).to.have.property('otp').eql(props.otp);
					e***REMOVED***pect(results.form).to.have.property('mfa_token').eql(props.mfaToken);
				});
			});
		});
		...
		describe('.listAccessTokens', () => {
			...
			it('sends credentials', () => {
				...
				return api.listAccessTokens(options)
					.then(({ auth, query }) => {
						e***REMOVED***pect(auth).to.be.an('object');
						e***REMOVED***pect(auth).to.have.property('username', options.username);
						e***REMOVED***pect(auth).to.have.property('password', options.password);
						e***REMOVED***pect(query).to.equal(undefined);
					});
			});
			...
			it('includes otp when provided', () => {
				return api.listAccessTokens(options)
					.then(({ auth, query }) => {
						...
						e***REMOVED***pect(auth).to.have.property('username', options.username);
						e***REMOVED***pect(auth).to.have.property('password', options.password);
						e***REMOVED***pect(query).to.be.an('object');
						e***REMOVED***pect(query).to.have.property('otp', props.otp);
						e***REMOVED***pect(props.otp).to.be.a('string').with.lengthOf(6);
					});
			});
		});
		...
		describe('.changeProduct', () => {
			it('generates request', () => {
				return api.changeProduct(props).then(Common.e***REMOVED***pectDeviceUrlAndToken);
			});
		...
		describe('.signalDevice', () => {
			it('generates request', () => {
				return api.signalDevice(props).then(Common.e***REMOVED***pectDeviceUrlAndToken);
			});
		...
		describe('.flashTinker', () => {
			it('generates request', () => {
				return api.flashTinker(props).then(Common.e***REMOVED***pectDeviceUrlAndToken);
			});
		...
		describe('.flashDevice', () => {
			describe('user scope', () => {
				it('generates request', () => {
					return api.flashDevice(props).then(Common.e***REMOVED***pectDeviceUrlAndToken);
				});
		...
		describe('.downloadFirmwareBinary', () => {
			it('generates request', () => {
				sinon.stub(api, '_provideFileData').callsFake(***REMOVED*** => Promise.resolve(***REMOVED***));
		...
		describe('.sendPublicKey', () => {
			it('generates request', () => {
				return api.sendPublicKey(props).then(Common.e***REMOVED***pectDeviceUrlAndToken);
			});
		...
		describe('.listBuildTargets', () => {
			it('generates request', () => {
				return api.listBuildTargets(props).then(({ auth, query }) => {
					...
					should.not.e***REMOVED***ist(query);
				});
			});
			...
			it('forwards query parameters', () => {
				return api.listLibraries({
					...
					results.query.should.eql({
						...
						limit: undefined,
						filter: 'abc',
						e***REMOVED***cludeScopes: undefined,
						sort: 'name',
						scope: undefined,
						architectures: 'spark-core,particle-photon',
						category: 'Other'
		...
		describe('.deleteLibrary', () => {
			it('generates request', () => {
				return api.deleteLibrary({ name: 'mylib', auth: 'X', force: '***REMOVED***yz' }).then((results) => {
					results.should.match({
						uri: '/v1/libraries/mylib',
						method: 'delete',
						auth: 'X',
		...
		describe('.downloadFile', () => {
			it('generates request', () => {
				sinon.stub(api, '_provideFileData').callsFake(***REMOVED*** => Promise.resolve(***REMOVED***));
				const uri = 'http://e***REMOVED***ample.com/path/to/file.png';
	...
	describe('.deviceUri', () => {
		...
		describe('product scope', () => {
			it('gets the product device uri', () => {
				const uri = api.deviceUri({ deviceId: 'abc', product: '***REMOVED***yz' });
				uri.should.equal('/v1/products/***REMOVED***yz/devices/abc');
			});
		});
	});
	...
	describe('conte***REMOVED***t', () => {
		describe('_isValidConte***REMOVED***t', () => {
			it('does not have conte***REMOVED***t items set after default construction', () => {
				const api = new Particle();
				api.should.have.property('conte***REMOVED***t').eql({});
			});

			it('is valid for known types and non-empty object', () => {
				api._isValidConte***REMOVED***t('tool', { abc:'123' }).should.be.ok;
				api._isValidConte***REMOVED***t('project', { abc:'123' }).should.be.ok;
			});

			it('is not valid for unknown types and non-empty object', () => {
				api._isValidConte***REMOVED***t('tool1', { abc:'123' }).should.not.be.ok;
				api._isValidConte***REMOVED***t('project1', { abc:'123' }).should.not.be.ok;
			});

			it('is not valid for known types and falsey object', () => {
				api._isValidConte***REMOVED***t('tool', {}).should.not.be.ok;
				api._isValidConte***REMOVED***t('tool', 0).should.not.be.ok;
				api._isValidConte***REMOVED***t('tool', null).should.not.be.ok;
				api._isValidConte***REMOVED***t('tool').should.not.be.ok;
			});

			it('sets a valid conte***REMOVED***t', () => {
				api.setConte***REMOVED***t('tool', { name:'spanner' });
				api.conte***REMOVED***t.should.have.property('tool').property('name').equal('spanner');
			});
		});

		describe('_buildConte***REMOVED***t', () => {
			it('uses the api conte***REMOVED***t when no conte***REMOVED***t provided', () => {
				const tool = { name:'spanner' };
				api.setConte***REMOVED***t('tool', tool);
				api._buildConte***REMOVED***t().should.eql({ tool });
			});

			it('overrides the api conte***REMOVED***t completely for a given conte***REMOVED***t item', () => {
				const tool = { name:'spanner', version:'1.2.3' };
				api.setConte***REMOVED***t('tool', tool);
				const newTool = { name:'pliers' };
				api._buildConte***REMOVED***t({ tool:newTool }).should.eql({ tool:newTool });
			});
		});

		describe('agent forwarders', () => {
			let uri, auth, headers, query, data, conte***REMOVED***t, conte***REMOVED***tResult, result;
			...
			beforeEach(() => {
				...
				conte***REMOVED***t = { abc: 123 };
				conte***REMOVED***tResult = { def: 456 };
				result = 'fake-result';
				api._buildConte***REMOVED***t = sinon.stub().returns(conte***REMOVED***tResult);
			});

			afterEach(() => {
				e***REMOVED***pect(api._buildConte***REMOVED***t).to.have.been.calledWith(conte***REMOVED***t);
			});

			it('calls _buildConte***REMOVED***t from get', () => {
				api.agent.get = sinon.stub().returns(result);
				const options = { uri, auth, headers, query, conte***REMOVED***t };
				const res = api.get(options);
				e***REMOVED***pect(res).to.equal(result);
				e***REMOVED***pect(api.agent.get).to.have.been.calledWith({
					uri,
					auth,
					headers,
					query,
					conte***REMOVED***t: conte***REMOVED***tResult
				});
			});

			it('calls _buildConte***REMOVED***t from head', () => {
				api.agent.head = sinon.stub().returns(result);
				const options = { uri, auth, headers, query, conte***REMOVED***t };
				const res = api.head(options);
				e***REMOVED***pect(res).to.equal(result);
				e***REMOVED***pect(api.agent.head).to.have.been.calledWith({
					uri,
					auth,
					headers,
					query,
			...
			it('calls _buildConte***REMOVED***t from post', () => {
				api.agent.post = sinon.stub().returns(result);
				const options = { uri, auth, headers, data, conte***REMOVED***t };
				const res = api.post(options);
				e***REMOVED***pect(res).to.equal(result);
				e***REMOVED***pect(api.agent.post).to.have.been.calledWith({
					uri,
					auth,
					headers,
					data,
			...
			it('calls _buildConte***REMOVED***t from put', () => {
				api.agent.put = sinon.stub().returns(result);
				const options = { uri, auth, headers, data, conte***REMOVED***t };
				const res = api.put(options);
				e***REMOVED***pect(res).to.equal(result);
				e***REMOVED***pect(api.agent.put).to.have.been.calledWith({
					uri,
					auth,
					headers,
					data,
			...
			it('calls _buildConte***REMOVED***t from delete', () => {
				api.agent.delete = sinon.stub().returns(result);
				const options = { uri, auth, headers, data, conte***REMOVED***t };
				const res = api.delete(options);
				e***REMOVED***pect(res).to.equal(result);
				e***REMOVED***pect(api.agent.delete).to.have.been.calledWith({
					uri,
					auth,
					headers,
					data,
			...
			it('calls _buildConte***REMOVED***t from request', () => {
				api.agent.request = sinon.stub().returns(result);
				api.request({ conte***REMOVED***t }).should.eql(result);
				e***REMOVED***pect(api.agent.request).to.have.been.calledWith({ conte***REMOVED***t:conte***REMOVED***tResult });
			});
		});
	});
});
