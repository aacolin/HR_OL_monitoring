...
* Fi***REMOVED***ed incomplete return types in TypeScript typings.
* Replaced COPYING.t***REMOVED***t with LICENSE file, which now has public domain dedication
  te***REMOVED***t from The Unlicense. License fields in package.json and bower.json have
...
Fi***REMOVED***ed a bug in the fast version of Poly1305 and brought it back.

Thanks to @floodyberry for promptly responding and fi***REMOVED***ing the original C code:
...
No code changes, just tweaked packaging and added COPYING.t***REMOVED***t.
...
* **Breaking change!** All functions from `nacl.util` have been removed. These
  ...
  `tweetnacl-util-js` (in the ne***REMOVED***t version this error message will be removed).
...
* Fi***REMOVED***ed typo in README.


v0.13.2
-------

* Fi***REMOVED***ed undefined variable bug in fast version of Poly1305. No worries, this
...
* E***REMOVED***clude `crypto` and `buffer` modules from browserify builds.
...
* Improved performance of curve operations, making `nacl.scalarMult`, `nacl.bo***REMOVED***`,
  `nacl.sign` and related functions up to 3***REMOVED*** faster in `nacl-fast` version.


v0.12.1
-------

* Significantly improved performance of Salsa20 (~1.5***REMOVED*** faster) and
  Poly1305 (~3.5***REMOVED*** faster) in `nacl-fast` version.
...
* Instead of using the given secret key directly, TweetNaCl.js now copies it to
  a new array in `nacl.bo***REMOVED***.keyPair.fromSecretKey` and
...
* Fast version now has an improved hash implementation that is 2***REMOVED***-5***REMOVED*** faster.

* Fi***REMOVED***ed benchmarks, which may have produced incorrect measurements.


v0.10.1
-------

* E***REMOVED***ported undocumented `nacl.lowlevel.crypto_core_hsalsa20`.
...
* Fast version: increased theoretical message size limit from 2^32-1 to 2^52
  bytes in Poly1305 (and thus, secretbo***REMOVED*** and bo***REMOVED***). However this has no impact
  in practice since JavaScript arrays or ArrayBuffers are limited to 32-bit
  inde***REMOVED***es, and most implementations won't allocate more than a gigabyte or so.
