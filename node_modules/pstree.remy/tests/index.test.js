...
test('tree for live env', async (t) => {
  const pid = 4079;
  const fi***REMOVED***ture = readFile(__dirname + '/fi***REMOVED***tures/out2', 'utf8');
  const ps = await tree(fi***REMOVED***ture);
  t.deepEqual(
...
function testTree(t, runCallCount) {
  const sub = spawn('node', [`${__dirname}/fi***REMOVED***tures/inde***REMOVED***.js`, runCallCount], {
    stdio: 'pipe',
  });
  setTimeout(() => {
    const pid = sub.pid;
    ...
    pstree(pid, (error, pids) => {
      ...
      // the fi***REMOVED***ture launches `sh` which launches node which is why we
