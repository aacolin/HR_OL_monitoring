...
var supportsNodePrefi***REMOVED*** = semver.satisfies(process.versions.node, '^14.18 || >= 16', { includePrerelease: true });
...
test('core modules', function (t) {
	...
	t.test('core via repl module', { skip: !data.repl }, function (st) {
		...
		if (!libs) {
			st.skip('repl._builtinLibs does not e***REMOVED***ist');
		} else {
	...
	t.test('core via builtinModules list', { skip: !data.module }, function (st) {
		...
		if (!libs) {
			st.skip('module.builtinModules does not e***REMOVED***ist');
		} else {
			var e***REMOVED***cludeList = [
				'_debug_agent',
			...
			for (var i = 0; i < libs.length; ++i) {
				var mod = libs[i];
				if (e***REMOVED***cludeList.inde***REMOVED***Of(mod) === -1) {
					st.ok(data[mod], mod + ' is a core module');
