...
e***REMOVED***ports.completion = void 0;
...
function completion(yargs, usage, command) {
    ...
    const zshShell = (process.env.SHELL && process.env.SHELL.inde***REMOVED***Of('zsh') !== -1) ||
        (process.env.ZSH_NAME && process.env.ZSH_NAME.inde***REMOVED***Of('zsh') !== -1);
    ...
    self.getCompletion = function getCompletion(args, done) {
        ...
        const parentCommands = yargs.getConte***REMOVED***t().commands;
            ...
            if (isSyncCompletionFunction(completionFunction)) {
                ...
                if (is_promise_1.isPromise(result)) {
                    return result.then((list) => {
                        process.ne***REMOVED***tTick(() => { done(list); });
                    }).catch((err) => {
                        process.ne***REMOVED***tTick(() => { throw err; });
                    });
                }
        ...
        if (!current.match(/^-/) && parentCommands[parentCommands.length - 1] !== current) {
            usage.getCommands().forEach((usageCommand) => {
                ...
                if (args.inde***REMOVED***Of(commandName) === -1) {
                    if (!zshShell) {
        ...
        if (current.match(/^-/) || (current === '' && completions.length === 0)) {
            ...
            Object.keys(options.key).forEach((key) => {
                ...
                function completeOptionKey(key) {
                    const notInArgs = keyAndAliases.every(val => args.inde***REMOVED***Of(`--${val}`) === -1);
                    if (notInArgs) {
...
e***REMOVED***ports.completion = completion;
