...
e***REMOVED***ports.isYargsInstance = e***REMOVED***ports.rebase = e***REMOVED***ports.Yargs = void 0;
...
const apply_e***REMOVED***tends_1 = require("./apply-e***REMOVED***tends");
...
function Yargs(processArgs = [], cwd = process.cwd(), parentRequire = require) {
    ...
    let default$0;
    if (/\b(node|iojs|electron)(\.e***REMOVED***e)?$/.test(process.argv[0])) {
    ...
    self.$0 = default$0
        .map(***REMOVED*** => {
        const b = rebase(cwd, ***REMOVED***);
        return ***REMOVED***.match(/^(\/|([a-zA-Z]:)?\\)/) && b.length < ***REMOVED***.length ? b : ***REMOVED***;
    })
        .join(' ').trim();
    if (process.env._ !== undefined && processArgv.getProcessArgvBin() === process.env._) {
        self.$0 = process.env._.replace(`${path.dirname(process.e***REMOVED***ecPath)}/`, '');
    }
    // use conte***REMOVED***t object to keep track of resets, subcommand e***REMOVED***ecution, etc
    // submodules should modify and check the state of conte***REMOVED***t as necessary
    const conte***REMOVED***t = { resets: -1, commands: [], fullCommands: [], files: [] };
    self.getConte***REMOVED***t = () => conte***REMOVED***t;
    // puts yargs back into an initial state. any keys
    // that have been set to "global" will not be reset
    // by this action.
    let options;
    self.resetOptions = self.reset = function resetOptions(aliases = {}) {
        conte***REMOVED***t.resets++;
        options = options || {};
        ...
        // if a key has been e***REMOVED***plicitly set as local,
        ...
        tmpOptions.envPrefi***REMOVED*** = options.envPrefi***REMOVED***;
        options = tmpOptions;
        // if this is the first time being e***REMOVED***ecuted, create
    ...
    function populateParserHintDictionary(builder, type, key, value, singleKeyHandler) {
        if (Array.isArray(key)) {
            // an array of keys with one value ['***REMOVED***', 'y', 'z'], function parse () {}
            key.forEach((k) => {
                builder(k, value);
            });
        }
        else if (((key) => typeof key === 'object')(key)) {
            // an object of key value pairs: {'***REMOVED***': parse () {}, 'y': parse() {}}
    ...
    function deleteFromParserHintObject(optionKey) {
        ...
        common_types_1.objectKeys(options).forEach((hintKey) => {
            ...
            if (Array.isArray(hint)) {
                if (~hint.inde***REMOVED***Of(optionKey))
                    hint.splice(hint.inde***REMOVED***Of(optionKey), 1);
            }
    ...
    self.config = function config(key = 'config', msg, parseFn) {
        ...
        if ((typeof key === 'object') && !Array.isArray(key)) {
            key = apply_e***REMOVED***tends_1.applyE***REMOVED***tends(key, cwd, self.getParserConfiguration()['deep-merge-config']);
    ...
    self.e***REMOVED***ample = function (cmd, description) {
        argsert_1.argsert('<string|array> [string]', [cmd, description], arguments.length);
        if (Array.isArray(cmd)) {
            cmd.forEach((e***REMOVED***ampleParams) => self.e***REMOVED***ample(...e***REMOVED***ampleParams));
        }
        else {
            usage.e***REMOVED***ample(cmd, description);
        }
        return self;
    };
    ...
    self.commandDir = function (dir, opts) {
        ...
        command.addDirectory(dir, self.getConte***REMOVED***t(), req, require('get-caller-file')(), opts);
        return self;
    };
    // TODO: deprecate self.demand in favor of
    // .demandCommand() .demandOption().
    self.demand = self.required = self.require = function demand(keys, ma***REMOVED***, msg) {
        // you can optionally provide a 'ma***REMOVED***' key,
        // which will raise an e***REMOVED***ception if too many '_'
        // options are provided.
        if (Array.isArray(ma***REMOVED***)) {
            ma***REMOVED***.forEach((key) => {
                common_types_1.assertNotStrictEqual(msg, true);
                demandOption(key, msg);
            });
            ma***REMOVED*** = Infinity;
        }
        else if (typeof ma***REMOVED*** !== 'number') {
            msg = ma***REMOVED***;
            ma***REMOVED*** = Infinity;
        }
        if (typeof keys === 'number') {
            common_types_1.assertNotStrictEqual(msg, true);
            self.demandCommand(keys, ma***REMOVED***, msg, msg);
        }
    ...
    self.demandCommand = function demandCommand(min = 1, ma***REMOVED***, minMsg, ma***REMOVED***Msg) {
        argsert_1.argsert('[number] [number|string] [string|null|undefined] [string|null|undefined]', [min, ma***REMOVED***, minMsg, ma***REMOVED***Msg], arguments.length);
        if (typeof ma***REMOVED*** !== 'number') {
            minMsg = ma***REMOVED***;
            ma***REMOVED*** = Infinity;
        }
        self.global('_', false);
    ...
    self.global = function global(globals, global) {
        ...
        if (global !== false) {
            options.local = options.local.filter(l => globals.inde***REMOVED***Of(l) === -1);
        }
        else {
            globals.forEach((g) => {
                if (options.local.inde***REMOVED***Of(g) === -1)
    ...
    self.pkgConf = function pkgConf(key, rootPath) {
        ...
        // If an object e***REMOVED***ists in the key, add it to options.configObjects
        if (obj[key] && typeof obj[key] === 'object') {
            conf = apply_e***REMOVED***tends_1.applyE***REMOVED***tends(obj[key], rootPath || cwd, self.getParserConfiguration()['deep-merge-config']);
    ...
    function pkgUp(rootPath) {
        ...
        try {
            ...
            // Whether or not it is _actually_ a directory (e.g., e***REMOVED***tensionless bin) is irrelevant, find-up handles it.
            if (!rootPath && path.e***REMOVED***tname(startDir)) {
    ...
    self.parse = function parse(args, shortCircuit, _parseFn) {
        ...
        if (typeof args === 'undefined') {
            ...
            unfreeze();
            // TODO: remove this compatibility hack when we release yargs@15.***REMOVED***:
            self.parsed = tmpParsed;
            return argv;
        }
        // a conte***REMOVED***t object can optionally be provided, this allows
        // additional information to be passed to a command handler.
        if (typeof shortCircuit === 'object') {
            parseConte***REMOVED***t = shortCircuit;
        ...
        if (!shortCircuit)
            processArgs = args;
        if (parseFn)
            e***REMOVED***itProcess = false;
        const parsed = self._parseArgs(args, !!shortCircuit);
        completion.setParsed(self.parsed);
        if (parseFn)
            parseFn(e***REMOVED***itError, parsed, output);
        unfreeze();
        return parsed;
    };
    self._getParseConte***REMOVED***t = () => parseConte***REMOVED***t || {};
    ...
    self.positional = function (key, opts) {
        ...
        opts = obj_filter_1.objFilter(opts, (k, v) => {
            let accept = supportedOpts.inde***REMOVED***Of(k) !== -1;
            // type can be one of string|number|boolean.
            if (k === 'type' && ['string', 'number', 'boolean'].inde***REMOVED***Of(v) === -1)
                accept = false;
            return accept;
        });
        // copy over any settings that can be inferred from the command string.
        const fullCommand = conte***REMOVED***t.fullCommands[conte***REMOVED***t.fullCommands.length - 1];
        ...
        common_types_1.objectKeys(parseOptions).forEach((pk) => {
            ...
            if (Array.isArray(parseOption)) {
                if (parseOption.inde***REMOVED***Of(key) !== -1)
                    opts[pk] = true;
            }
            else {
    ...
    self.group = function group(opts, groupName) {
        ...
        const e***REMOVED***isting = preservedGroups[groupName] || groups[groupName];
        if (preservedGroups[groupName]) {
            // we now only need to track this group name in groups.
            delete preservedGroups[groupName];
        }
        const seen = {};
        groups[groupName] = (e***REMOVED***isting || []).concat(opts).filter((key) => {
            if (seen[key])
                return false;
    ...
    // combine e***REMOVED***plicit and preserved groups. e***REMOVED***plicit groups should be first
    self.getGroups = () => Object.assign({}, groups, preservedGroups);
    // as long as options.envPrefi***REMOVED*** is not undefined,
    // parser will apply env vars matching prefi***REMOVED*** to argv
    self.env = function (prefi***REMOVED***) {
        argsert_1.argsert('[string|boolean]', [prefi***REMOVED***], arguments.length);
        if (prefi***REMOVED*** === false)
            delete options.envPrefi***REMOVED***;
        else
            options.envPrefi***REMOVED*** = prefi***REMOVED*** || '';
        return self;
    };
    ...
    self.showHelp = function (level) {
        ...
        if (!self.parsed)
            self._parseArgs(processArgs); // run parser, if it has not already been e***REMOVED***ecuted.
        if (command.hasDefaultCommand()) {
            conte***REMOVED***t.resets++; // override the restriction on top-level positoinals.
    ...
    var e***REMOVED***itProcess = true;
    self.e***REMOVED***itProcess = function (enabled = true) {
        argsert_1.argsert('[boolean]', [enabled], arguments.length);
        e***REMOVED***itProcess = enabled;
        return self;
    };
    self.getE***REMOVED***itProcess = () => e***REMOVED***itProcess;
    var completionCommand = null;
    self.completion = function (cmd, desc, fn) {
        argsert_1.argsert('[string] [string|boolean|function] [function]', [cmd, desc, fn], arguments.length);
        // a function to e***REMOVED***ecute when generating
    ...
    var hasOutput = false;
    var e***REMOVED***itError = null;
    // maybe e***REMOVED***it, always capture
    // conte***REMOVED***t about why we wanted to e***REMOVED***it.
    self.e***REMOVED***it = (code, err) => {
        hasOutput = true;
        e***REMOVED***itError = err;
        if (e***REMOVED***itProcess)
            process.e***REMOVED***it(code);
    };
    ...
    // message in the current e***REMOVED***ecution conte***REMOVED***t.
    ...
    self._parseArgs = function parseArgs(args, shortCircuit, _calledFromCommand, commandInde***REMOVED***) {
        ...
        if (parseConte***REMOVED***t)
            argv = Object.assign({}, argv, parseConte***REMOVED***t);
        ...
        try {
            ...
            if (helpOpt) {
                ...
                if (~helpCmds.inde***REMOVED***Of(argv._[argv._.length - 1])) {
                    argv._.pop();
                    argv[helpOpt] = true;
                }
            }
            ...
            if (argv._.length) {
                if (handlerKeys.length) {
                    ...
                    for (let i = (commandInde***REMOVED*** || 0), cmd; argv._[i] !== undefined; i++) {
                        cmd = String(argv._[i]);
                        if (~handlerKeys.inde***REMOVED***Of(cmd) && cmd !== completionCommand) {
                            // commands are e***REMOVED***ecuted using a recursive algorithm that e***REMOVED***ecutes
                            // the deepest command first; we keep track of the position in the
                            // argv._ array that is currently being e***REMOVED***ecuted.
                    ...
                    // been enabled, and no commands were found to e***REMOVED***ecute
                    if (recommendCommands && firstUnknownCommand && !skipRecommendation) {
                        validation.recommendCommands(firstUnknownCommand, handlerKeys);
                    }
                }
                // generate a completion script for adding to ~/.bashrc.
                if (completionCommand && ~argv._.inde***REMOVED***Of(completionCommand) && !requestCompletions) {
                    if (e***REMOVED***itProcess)
                        setBlocking(true);
            ...
            if (requestCompletions) {
                ...
                args = [].concat(args);
                const completionArgs = args.slice(args.inde***REMOVED***Of(`--${completion.completionKey}`) + 1);
            ...
            if (!skipValidation && options.skipValidation.length > 0) {
                skipValidation = Object.keys(argv).some(key => options.skipValidation.inde***REMOVED***Of(key) >= 0 && argv[key] === true);
            }
            // If the help or version options where used and e***REMOVED***itProcess is false,
            // or if e***REMOVED***plicitly skipped, we won't run validations.
            if (!skipValidation) {
                if (parsed.error)
                    throw new yerror_1.YError(parsed.error.message);
                // if we're e***REMOVED***ecuted via bash completion, don't
...
e***REMOVED***ports.Yargs = Yargs;
// rebase an absolute path to a relative one with respect to a base directory
// e***REMOVED***ported for tests
function rebase(base, dir) {
    return path.relative(base, dir);
}
e***REMOVED***ports.rebase = rebase;
function isYargsInstance(y) {
    return !!y && (typeof y._parseArgs === 'function');
}
e***REMOVED***ports.isYargsInstance = isYargsInstance;
