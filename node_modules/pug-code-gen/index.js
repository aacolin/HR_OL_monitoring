...
module.e***REMOVED***ports = generateCode;
module.e***REMOVED***ports.CodeGenerator = Compiler;
...
function Compiler(node, options) {
  ...
  this.terse = false;
  this.mi***REMOVED***ins = {};
  this.dynamicMi***REMOVED***ins = false;
  this.eachCount = 0;
...
Compiler.prototype = {
  ...
  compile: function() {
    ...
    this.lastBufferedId***REMOVED*** = -1;
    this.visit(this.node);
    if (!this.dynamicMi***REMOVED***ins) {
      // if there are no dynamic mi***REMOVED***ins we can remove any un-used mi***REMOVED***ins
      var mi***REMOVED***inNames = Object.keys(this.mi***REMOVED***ins);
      for (var i = 0; i < mi***REMOVED***inNames.length; i++) {
        var mi***REMOVED***in = this.mi***REMOVED***ins[mi***REMOVED***inNames[i]];
        if (!mi***REMOVED***in.used) {
          for (var ***REMOVED*** = 0; ***REMOVED*** < mi***REMOVED***in.instances.length; ***REMOVED***++) {
            for (
              var y = mi***REMOVED***in.instances[***REMOVED***].start;
              y < mi***REMOVED***in.instances[***REMOVED***].end;
              y++
            ) {
              this.buf[y] = '';
            }
    ...
    return (
      ...
      '(locals) {var pug_html = "", pug_mi***REMOVED***ins = {}, pug_interp;' +
      js +
      ';return pug_html;}'
    );
  },
  ...
  setDoctype: function(name) {
    ...
    this.***REMOVED***ml = 0 == this.doctype.inde***REMOVED***Of('<?***REMOVED***ml');
  },

  /**
   * Buffer the given `str` e***REMOVED***actly as is or with interpolation
   *
   * @param {String} str
  ...
  buffer: function(str) {
    ...
    if (
      this.lastBufferedId***REMOVED*** == this.buf.length &&
      ...
      this.lastBufferedType = 'te***REMOVED***t';
      this.lastBuffered += str;
      this.buf[this.lastBufferedId***REMOVED*** - 1] =
        'pug_html = pug_html + ' +
  ...
  bufferE***REMOVED***pression: function(src) {
    if (isConstant(src)) {
    ...
    if (
      ...
      this.bufferedConcatenationCount++;
      if (this.lastBufferedType === 'te***REMOVED***t') this.lastBuffered += '"';
  ...
  visit: function(node, parent) {
    ...
    if (!node) {
      ...
      msg += ' is ' + node + ', e***REMOVED***pected a Pug AST Node.';
  ...
  visitCase: function(node) {
    this.buf.push('switch (' + node.e***REMOVED***pr + '){');
  ...
  visitWhen: function(node) {
    if ('default' == node.e***REMOVED***pr) {
      this.buf.push('default:');
    } else {
      this.buf.push('case ' + node.e***REMOVED***pr + ':');
    }
    if (node.block) {
      this.visit(node.block, node);
      this.buf.push('  break;');
  ...
  visitBlock: function(block) {
    ...
    // Pretty print multi-line te***REMOVED***t
    if (
      pp &&
      block.nodes.length > 1 &&
      !escapePrettyMode &&
      block.nodes[0].type === 'Te***REMOVED***t' &&
      block.nodes[1].type === 'Te***REMOVED***t'
    ) {
      this.prettyIndent(1, true);
    }
    for (var i = 0; i < block.nodes.length; ++i) {
      // Pretty print te***REMOVED***t
      if (
        pp &&
        i > 0 &&
        !escapePrettyMode &&
        block.nodes[i].type === 'Te***REMOVED***t' &&
        block.nodes[i - 1].type === 'Te***REMOVED***t' &&
  ...
  /**
   * Visit a mi***REMOVED***in's `block` keyword.
   *
   * @param {Mi***REMOVED***inBlock} block
   * @api public
   */

  visitMi***REMOVED***inBlock: function(block) {
    if (this.pp)
      this.buf.push(
        'pug_indent.push(' +
  ...
  /**
   * Visit `mi***REMOVED***in`, generating a function that
   * may be called within the template.
   *
   * @param {Mi***REMOVED***in} mi***REMOVED***in
   * @api public
   */

  visitMi***REMOVED***in: function(mi***REMOVED***in) {
    var name = 'pug_mi***REMOVED***ins[';
    var args = mi***REMOVED***in.args || '';
    var block = mi***REMOVED***in.block;
    var attrs = mi***REMOVED***in.attrs;
    var attrsBlocks = this.attributeBlocks(mi***REMOVED***in.attributeBlocks);
    var pp = this.pp;
    var dynamic = mi***REMOVED***in.name[0] === '#';
    var key = mi***REMOVED***in.name;
    if (dynamic) this.dynamicMi***REMOVED***ins = true;
    name +=
      (dynamic
        ? mi***REMOVED***in.name.substr(2, mi***REMOVED***in.name.length - 3)
        : '"' + mi***REMOVED***in.name + '"') + ']';

    this.mi***REMOVED***ins[key] = this.mi***REMOVED***ins[key] || {used: false, instances: []};
    if (mi***REMOVED***in.call) {
      this.mi***REMOVED***ins[key].used = true;
      if (pp)
        this.buf.push(
          'pug_indent.push(' +
      ...
      if (block || attrs.length || attrsBlocks.length) {
        ...
        if (block) {
          ...
          this.indents = 0;
          this.visit(mi***REMOVED***in.block, mi***REMOVED***in);
      ...
      var mi***REMOVED***in_start = this.buf.length;
      ...
      // once fi***REMOVED***ed, use this: this.buf.push(name + ' = function(' + args.join(',') + '){');
      ...
      this.parentIndents++;
      this.visit(block, mi***REMOVED***in);
      this.parentIndents--;
      this.buf.push('};');
      var mi***REMOVED***in_end = this.buf.length;
      this.mi***REMOVED***ins[key].instances.push({start: mi***REMOVED***in_start, end: mi***REMOVED***in_end});
    }
  },
  ...
  visitTag: function(tag, interpolated) {
    ...
    function bufferName() {
      if (interpolated) self.bufferE***REMOVED***pression(tag.e***REMOVED***pr);
    ...
    if (tag.selfClosing || (!this.***REMOVED***ml && selfClosing[tag.name])) {
      this.buffer('<');
      bufferName();
      ...
      if (
        ...
        (tag.block &&
          ...
          tag.block.nodes.some(function(tag) {
            return tag.type !== 'Te***REMOVED***t' || !/^\s*$/.test(tag.val);
          }))
      ) {
        this.error(
          name +
  ...
  /**
   * Visit `te***REMOVED***t` node.
   *
   * @param {Te***REMOVED***t} te***REMOVED***t
   * @api public
   */

  visitTe***REMOVED***t: function(te***REMOVED***t) {
    this.buffer(te***REMOVED***t.val);
  },
  ...
  visitCode: function(code) {
    ...
    if (code.buffer) {
      ...
      this.bufferE***REMOVED***pression(val);
    } else {
  ...
  visitEach: function(each) {
    var inde***REMOVED***VarName = each.key || 'pug_inde***REMOVED***' + this.eachCount;
    this.eachCount++;
  ...
  attrs: function(attrs, buffer) {
    ...
    if (buffer) {
      this.bufferE***REMOVED***pression(res);
    }
    return res;
  },
...
function tagCanInline(tag) {
  function isInline(node) {
    ...
    // e***REMOVED***pected to be included but is not. If this is the case, the block
    // must be empty.
    if (node.type === 'YieldBlock') return true;
    return (node.type === 'Te***REMOVED***t' && !/\n/.test(node.val)) || node.isInline;
  }
