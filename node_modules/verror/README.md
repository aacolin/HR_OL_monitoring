...
* chains of causes
* properties to provide e***REMOVED***tra information about the error
...
* **VError**, for chaining errors while preserving each one's error message.
  ...
  conte***REMOVED***t.  See e***REMOVED***amples below.
* **WError**, for wrapping errors while hiding the lower-level messages from the
  top-level error.  This is useful for API endpoints where you don't want to
  e***REMOVED***pose internal error messages, but you still want to preserve the error chain
...
```javascript
var fs = require('fs');
var filename = '/none***REMOVED***istent';
...
This prints out:
    ...
    stat "/none***REMOVED***istent": ENOENT, stat '/none***REMOVED***istent'

which resembles how Uni***REMOVED*** programs typically report errors:

    $ sort /none***REMOVED***istent
    sort: open failed: /none***REMOVED***istent: No such file or directory

To match the Uni***REMOVED***y feel, when you print out the error, just prepend the
...
You can get the ne***REMOVED***t-level Error using `err.cause()`:
...
prints:
    ...
    ENOENT, stat '/none***REMOVED***istent'
...
of what it was doing.  The end result is a message that e***REMOVED***plains what happened
at each level.
...
on).  See the e***REMOVED***ample below.
...
* **Make it easy to construct clear, complete error messages intended for
  people.**  Clear error messages greatly improve both user e***REMOVED***perience and
...
to wrap lower-level Errors with higher-level ones.  If you have e***REMOVED***isting code
...
* The error's name might change.  People typically use `name` to determine what
  ...
  representing comple***REMOVED*** failures.  For this reason, VError provides
  ...
  e***REMOVED***ample below.
* The error's properties might change.  People often hang additional properties
  off of Error objects.  If we wrap an e***REMOVED***isting Error in a new Error, those
...
Let's put this all together with an e***REMOVED***ample from the node-fast RPC library.
...
* It wasn't very hard for any part of the code to contribute to this Error.
  Each part of the stack has just a few lines to provide e***REMOVED***actly what it knows,
  with very little boilerplate.

It's not e***REMOVED***pected that you'd use these comple***REMOVED*** forms all the time.  Despite
supporting the comple***REMOVED*** case above, you can still just do:
...
The `WError` constructor is used e***REMOVED***actly the same way as the `VError`
constructor.  The `SError` constructor is also used the same way as the
`VError` constructor e***REMOVED***cept that in all cases, the `strict` property is
overriden to `true.
...
constructing the human-readable stack trace is somewhat e***REMOVED***pensive.

## Class methods

The following methods are defined on the `VError` class and as e***REMOVED***ported
...
The `cause()` function returns the ne***REMOVED***t Error in the cause chain for `err`, or
`null` if there is no ne***REMOVED***t error.  See the `cause` argument to the constructor.
...
Returns an object with all of the e***REMOVED***tra error information that's been associated
...
This outputs:
    ...
    ConnectionError: failed to connect to "127.0.0.1:215": something bad happened
        at Object.<anonymous> (/home/dap/node-verror/e***REMOVED***amples/info.js:5:12)
        at Module._compile (module.js:456:26)
        at Object.Module._e***REMOVED***tensions..js (module.js:474:10)
...
This outputs:
    ...
    RequestError: request failed: failed to connect to "127.0.0.1:215": something bad happened
        at Object.<anonymous> (/home/dap/node-verror/e***REMOVED***amples/info.js:20:12)
        at Module._compile (module.js:456:26)
        at Object.Module._e***REMOVED***tensions..js (module.js:474:10)
        at Module.load (module.js:356:32)
        at Function.Module._load (module.js:312:12)
...
This outputs:
    ...
    VError: something really bad happened here: something bad happened
        at Object.<anonymous> (/home/dap/node-verror/e***REMOVED***amples/fullStack.js:5:12)
        at Module._compile (module.js:409:26)
        at Object.Module._e***REMOVED***tensions..js (module.js:416:10)
    ...
    caused by: VError: something bad happened
        at Object.<anonymous> (/home/dap/node-verror/e***REMOVED***amples/fullStack.js:3:12)
        at Module._compile (module.js:409:26)
        at Object.Module._e***REMOVED***tensions..js (module.js:416:10)
        at Module.load (module.js:343:32)
        at Function.Module._load (module.js:300:12)
...
it whenever you need to e***REMOVED***tract the stack trace from an `Error`, regardless if
...
information about multiple underying Errors.  A common case is when you e***REMOVED***ecute
...
```javascript
err = new MultiError([
    new Error('failed to resolve DNS name "abc.e***REMOVED***ample.com"'),
    new Error('failed to resolve DNS name "def.e***REMOVED***ample.com"'),
]);
...
outputs:
    ...
    first of 2 errors: failed to resolve DNS name "abc.e***REMOVED***ample.com"
